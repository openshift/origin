...
func (r *SourceRef) BuildSource() (*build.BuildSource, []build.BuildTriggerPolicy) {
	return &build.BuildSource{
			...
			{
				Type: build.GithubWebHookBuildTriggerType,
				GithubWebHook: &build.WebHookTrigger{
					Secret: generateSecret(),
				},
			},
			{
				Type: build.GenericWebHookBuildTriggerType,
...
func (r *ImageInfo) Services() ([]*kapi.Service, error) {
	services := []*kapi.Service{}
	suggestedName, ok := r.SuggestName()
	if !ok {
		return nil, fmt.Errorf("Unable to determine a name from image reference")
	}
	name := suggestedName[0:min(55, len(suggestedName))]
	if r.Info != nil {
		for p := range r.Info.Config.ExposedPorts {
			serviceName := fmt.Sprintf("%s-%s", name, p.Port())
			port, err := strconv.Atoi(p.Port())
			if err != nil {
				log.Fatalf("failed to parse port %q: %v", p.Port(), err)
			}
			services = append(services, &kapi.Service{
				ObjectMeta: kapi.ObjectMeta{
					Name: serviceName,
				},
				Spec: kapi.ServiceSpec{
					Port:     port,
					Protocol: kapi.Protocol(strings.ToUpper(p.Proto())),
					Selector: map[string]string{"deploymentconfig": suggestedName},
				},
			})
		}
	}
...
func (r *DeploymentConfigRef) DeploymentConfig() (*deploy.DeploymentConfig, error) {
	...
	return &deploy.DeploymentConfig{
		...
		Template: deploy.DeploymentTemplate{
			Strategy: deploy.DeploymentStrategy{
				Type: deploy.DeploymentStrategyTypeRecreate,
			},
