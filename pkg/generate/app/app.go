...
func (r *ImageRef) ImageRepository() (*image.ImageRepository, error) {
	...
	return &image.ImageRepository{
		...
		DockerImageRepository: r.pullSpec(),
	}, nil
}

func (r *ImageRef) Container() (*kapi.Container, []deploy.DeploymentTriggerPolicy, error) {
	name, ok := r.SuggestName()
	if !ok {
...
func (r *DeploymentConfigRef) DeploymentConfig() (*deploy.DeploymentConfig, error) {
	...
	for i := range r.Images {
		c, containerTriggers, err := r.Images[i].Container()
		if err != nil {
			return nil, err
		}
