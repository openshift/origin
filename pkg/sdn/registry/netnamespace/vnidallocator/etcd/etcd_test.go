...
func newStorage(t *testing.T) (*Etcd, *tools.FakeEtcdClient, allocator.Interface) {
	...
	etcd := NewEtcd(mem, "/ranges/namespacevnids", "namespacevnidallocation", etcdStorage)
...
func TestEmpty(t *testing.T) {
	...
	if _, err := storage.Allocate(1); !strings.Contains(err.Error(), "cannot allocate resources of type namespacevnidallocation at this time") {
		t.Fatal(err)
	}
}
...
func TestStore(t *testing.T) {
	...
	if !other.Has(2) {
		t.Fatalf("could not restore allocated VNID: %#v", other)
	}

	other = allocator.NewAllocationMap(100, "rangeSpecValue")
	otherStorage := NewEtcd(other, "/ranges/namespacevnids", "namespacevnidallocation", storage.storage)
