...
func Source(log logr.Logger) (ObservationSource, error) {
	...
	resourcesToWatch := []schema.GroupVersionResource{
		...
		resource("groupsnapshot.storage.k8s", "v1beta2", "volumegroupsnapshots"),
		resource("groupsnapshot.storage.k8s", "v1beta2", "volumegroupsnapshotcontents"),
		resource("snapshot.storage.k8s", "v1", "volumesnapshots"),
		resource("snapshot.storage.k8s", "v1", "volumesnapshotcontents"),
	}
