...
func OkStrategy() appsapi.DeploymentStrategy {
	return appsapi.DeploymentStrategy{
		...
		ActiveDeadlineSeconds: mkintp(int(appsinternal.MaxDeploymentDurationSeconds)),
	}
}
...
func RoundTripConfig(t *testing.T, config *appsapi.DeploymentConfig) *appsapi.DeploymentConfig {
	versioned, err := legacyscheme.Scheme.ConvertToVersion(config, appsapi.SchemeGroupVersion)
	if err != nil {
