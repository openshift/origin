...
type hookExecutorImpl struct {
	executeFunc func(hook *appsapi.LifecycleHook, deployment *corev1.ReplicationController, stage string, timeout time.Duration) error
}

func (h *hookExecutorImpl) RunHook(hook *appsapi.LifecycleHook, rc *corev1.ReplicationController, stage string, timeout time.Duration) error {
	return h.executeFunc(hook, rc, stage, timeout)
}
...
func TestRecreate_initialDeployment(t *testing.T) {
	...
	deployment, _ = appsutil.MakeDeploymentV1(config, legacyscheme.Codecs.LegacyCodec(legacyscheme.Registry.GroupOrDie(kapi.GroupName).
		GroupVersions[0]))
...
func TestRecreate_acceptorSuccess(t *testing.T) {
	...
	oldDeployment, _ := appsutil.MakeDeploymentV1(appstest.OkDeploymentConfig(1), legacyscheme.Codecs.LegacyCodec(legacyscheme.Registry.GroupOrDie(
		corev1.GroupName).GroupVersions[0]))
	deployment, _ = appsutil.MakeDeploymentV1(appstest.OkDeploymentConfig(2), legacyscheme.Codecs.LegacyCodec(legacyscheme.Registry.GroupOrDie(corev1.
		GroupName).GroupVersions[0]))
	controllerClient := newFakeControllerClient(deployment)
	strategy.rcClient = controllerClient
	strategy.scaleClient = controllerClient.fakeScaleClient()
	strategy.podClient = &fakePodClient{deployerName: appsutil.DeployerPodNameForDeployment(deployment.Name)}
...
func TestRecreate_acceptorSuccessWithColdCaches(t *testing.T) {
	...
	oldDeployment, _ := appsutil.MakeDeploymentV1(appstest.OkDeploymentConfig(1), legacyscheme.Codecs.LegacyCodec(legacyscheme.Registry.GroupOrDie(
		kapi.GroupName).GroupVersions[0]))
	deployment, _ = appsutil.MakeDeploymentV1(appstest.OkDeploymentConfig(2), legacyscheme.Codecs.LegacyCodec(legacyscheme.Registry.GroupOrDie(kapi.
		GroupName).GroupVersions[0]))
	controllerClient := newFakeControllerClient(deployment)
