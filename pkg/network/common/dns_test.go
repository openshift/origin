...
func TestAddDNS(t *testing.T) {
	...
	for _, test := range tests {
		...
		dns, err := NewDNS(fexec, "")
		if err != nil {
			t.Fatalf("Test case: %s failed, unexpected error: %v", test.testCase, err)
		}
...
func TestGetDNSResolvers(t *testing.T) {
	configFile, err := ioutil.TempFile("/tmp/", "resolv")
	if err != nil {
		t.Fatalf("cannot create DNS resolver config file: %v", err)
	}
...
search sjc.redhat.com redhat.com
...
options rotate timeout:1 attempts:1`
	if _, err := configFile.WriteString(data); err != nil {
		t.Fatalf("unable to write data to resolver config file: %v", err)
	}

	nameservers, err := getDNSResolvers(configFile.Name())
	if err != nil {
		t.Fatalf("failed to extract dns resolvers: %v", err)
	}

	expectedNameservers := []string{"10.11.12.1", "10.11.12.2"}
	if !reflect.DeepEqual(nameservers, expectedNameservers) {
		t.Fatalf("incorrect dns resolvers, expected: %v but got %v", expectedNameservers, nameservers)
	}
}
