...
func (c *crdVersionChecker) EvaluateTestsFromConstructedIntervals(ctx context.Context, finalIntervals monitorapi.Intervals) ([]*junitapi.JUnitTestCase, error) {
	...
	// Only run checks if an upgrade actually happened
	if !platformidentification.DidUpgradeHappenDuringCollection(finalIntervals, time.Time{}, time.Time{}) {
		logrus.Info("No upgrade detected, skipping CRD version checks")
		return nil, nil
	}
...
//
// This check FAILS if:
// - A CRD has a new version added
// - The new version is marked as the storage version
// - The old version is no longer the storage version
//
// This check PASSES if:
// - New versions are added but the old storage version is preserved
// - Both old and new versions are served
func (c *crdVersionChecker) checkNewVersionsNotStoredImmediately() []*junitapi.JUnitTestCase {
	...
	for crdName, beforeCRD := range c.beforeSnapshot.CRDs {
		...
		for _, v := range beforeCRD.Versions {
			beforeVersionSet[v.Name] = true
		}

		newVersions := []string{}
		for _, v := range afterCRD.Versions {
			if !beforeVersionSet[v.Name] {
