...
func unexpectedNodeNotReadyJunit(finalIntervals monitorapi.Intervals) []*junitapi.JUnitTestCase {
	...
	for _, event := range finalIntervals {
		if didIntervalHappenBetweenMachinePhaseChange(event, finalIntervals) && event.Message.Reason == monitorapi.NodeUnexpectedReadyReason {
...
func unreachableNodeTaint(finalIntervals monitorapi.Intervals) []*junitapi.JUnitTestCase {
	...
	var failures []string
	for _, event := range finalIntervals {
		if didIntervalHappenBetweenMachinePhaseChange(event, finalIntervals) && event.Message.Reason == monitorapi.NodeUnexpectedUnreachableReason {
...
func didIntervalHappenBetweenMachinePhaseChange(interval monitorapi.Interval, machineIntervals monitorapi.Intervals) bool {
	isIntervalBetweenAMachinePhase := false
	...
	return isIntervalBetweenAMachinePhase
}
