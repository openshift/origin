...
func scanAllOperatorPods(ctx context.Context, kubeClient kubernetes.Interface, logHandlers ...podaccess.LogHandler) error {
	...
	for _, pod := range pods.Items {
		if !(strings.HasPrefix(pod.Namespace, "openshift-kni-infra") || strings.HasPrefix(pod.Namespace, "openshift-openstack-infra") || strings.HasPrefix(pod.Namespace, "openshift-vsphere-infra")) {
			continue
		}
		// Pods in cloud provider platforms do not have nice names, e.g. "haproxy-i7rd6ssm-ac890-q5tzv-master-0" in OpenStack
		if !strings.Contains(pod.Name, "haproxy") || !strings.Contains(pod.Name, "master") {
			continue
		}
