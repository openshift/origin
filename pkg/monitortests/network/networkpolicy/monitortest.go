...
type networkPolicyMonitorTest struct {
	...
	watchedNamespaces sets.Set[string]
}

// NewNetworkPolicyMonitorTest creates a monitor test that requires that every watched namespace
...
func NewNetworkPolicyMonitorTest() monitortestframework.MonitorTest {
	return &networkPolicyMonitorTest{
		watchedNamespaces: sets.New(
			// Control plane namespaces (see https://issues.redhat.com/browse/OCPSTRAT-1046)
			"openshift-apiserver",
...
func (n *networkPolicyMonitorTest) CollectData(ctx context.Context, storageDir string, beginning, end time.Time) (monitorapi.Intervals, []*junitapi.JUnitTestCase, error) {
	if n.kubeClient == nil {
		return nil, nil, fmt.Errorf("kube client not initialized")
	}
	...
	for _, ns := range nsList.Items {
		...
		// skip namespaces that aren't explicitly watched by this test
		if !n.watchedNamespaces.Has(ns.Name) {
			continue
		}
