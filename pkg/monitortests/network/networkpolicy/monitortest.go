...
func (n *networkPolicyMonitorTest) CollectData(ctx context.Context, storageDir string, beginning, end time.Time) (monitorapi.Intervals, []*junitapi.JUnitTestCase, error) {
	...
	for _, ns := range nsList.Items {
		...
		// skip namespaces that aren't core openshift or kube
		if !isOpenShiftCoreNamespace(ns.Name) && !isKubeSystemNamespace(ns.Name) {
			continue
		}
...
func isOpenShiftCoreNamespace(ns string) bool {
	return ns == "openshift" || strings.HasPrefix(ns, "openshift-")
}

func isKubeSystemNamespace(ns string) bool {
