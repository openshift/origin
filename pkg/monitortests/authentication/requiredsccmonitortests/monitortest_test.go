...
func TestRequiredSCCResults(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "validated pod in SCC-exempt namespace skipped", namespace: "kube-system",
...
func TestRequiredSCCExemptNamespaceDoesNotHideOtherFailure(t *testing.T) {
	t.Parallel()
	...
	// kube-system is SCC-exempt, so no JUnit results for it
	for _, result := range results {
		if strings.Contains(result.Name, "kube-system") {
			t.Errorf("SCC-exempt namespace kube-system should not produce JUnit results, got: %s", result.Name)
		}
	}
	if state := runSCCMonitor(t, checker); state != monitor.Failed {
		t.Fatalf("monitor result = %s, want %s: openshift-authentication failure must still surface", state, monitor.Failed)
	}
}
...
func TestRequiredSCCCollectionErrors(t *testing.T) {
	...
	for _, resource := range []string{"namespaces", "pods"} {
		t.Run(resource, func(t *testing.T) {
			...
			client := fake.NewSimpleClientset(&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "openshift-authentication"}})
