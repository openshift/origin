...
func TestValidateFailures(t *testing.T) {
	...
	var uid int64 = 999
	var badUID int64 = 1
	failUserSCC.Spec.RunAsUser = api.RunAsUserStrategyOptions{
		Type: api.RunAsUserStrategyMustRunAs,
		UID:  &uid,
	}
	...
	failSELinuxSCC.Spec.SELinuxContext = api.SELinuxContextStrategyOptions{
		Type: api.SELinuxStrategyMustRunAs,
	...
	errorCases := map[string]struct {
		...
		"failHostPortSCC": {
			pod:           failHostPortPod,
			scc:           defaultSCC(),
			expectedError: "Host port 1 is not allowed to be used",
		},
	}
...
func TestValidateSuccess(t *testing.T) {
	...
	userSCC := defaultSCC()
	var uid int64 = 999
	userSCC.Spec.RunAsUser = api.RunAsUserStrategyOptions{
		Type: api.RunAsUserStrategyMustRunAs,
		UID:  &uid,
	}
	userPod := defaultPod()
	...
	seLinuxSCC.Spec.SELinuxContext = api.SELinuxContextStrategyOptions{
		Type: api.SELinuxStrategyMustRunAs,
		SELinuxOptions: &kapi.SELinuxOptions{
			Level: "foo",
		},
	...
	capsSCC := defaultSCC()
	capsSCC.Spec.Capabilities = []kapi.Capability{"foo"}
	capsPod := defaultPod()
	...
	hostDirSCC.Spec.Volumes.HostPath = true
	...
	hostPortSCC.Spec.HostPorts = []api.HostPortRange{{Start: 1, End: 65535}}
