...
func (c CSVConverter) Convert(data []byte) ([]byte, error) {
	...
	if c.converterFunc != nil {
		...
		data = convertedBytes
		glog.V(4).Infof("CSV converter transformed data bytes: %s", string(data))
	}
	...
	glog.V(4).Infof("CSV converter fixing up csv header ...")
	data, _ = c.fixupHeaders(data)
	glog.V(4).Infof("CSV converter fixed up data bytes: %s", string(data))
...
func (c CSVConverter) fixupHeaders(data []byte) ([]byte, error) {
	prefix := []byte("#")
	if len(c.headers) > 0 && !bytes.HasPrefix(data, prefix) {
		// No header, so insert one.
		line := bytes.Join([][]byte{prefix, c.headers}, []byte(" "))
