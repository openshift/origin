...
func updateConditions(config *deployapi.DeploymentConfig, newStatus *deployapi.DeploymentConfigStatus, latestRC *kapi.ReplicationController) {
	...
	if latestRC != nil {
		...
		case deployapi.DeploymentStatusFailed:
			if deployutil.DeploymentConditionExists(*newStatus, deployapi.DeploymentProgressing, kapi.ConditionFalse, deployutil.TimedOutReason) {
				break
			}
		...
		case deployapi.DeploymentStatusComplete:
			if deployutil.DeploymentConditionExists(*newStatus, deployapi.DeploymentProgressing, kapi.ConditionTrue, deployutil.NewRcAvailableReason) {
				break
			}
			msg := fmt.Sprintf("Replication controller %q has complete progressing", latestRC.Name)
	...
	if latestRC == nil || deployutil.IsTerminatedDeployment(latestRC) {
		pausedCondExists := deployutil.DeploymentConditionExists(*newStatus, deployapi.DeploymentProgressing, kapi.ConditionUnknown, deployutil.PausedDeployReason)
		if config.Spec.Paused && !pausedCondExists {
			condition := deployutil.NewDeploymentCondition(deployapi.DeploymentProgressing, kapi.ConditionUnknown, deployutil.PausedDeployReason, "Deployment is paused")
			deployutil.SetDeploymentCondition(newStatus, *condition)
		} else if !config.Spec.Paused && pausedCondExists {
			condition := deployutil.NewDeploymentCondition(deployapi.DeploymentProgressing, kapi.ConditionUnknown, deployutil.ResumedDeployReason, "Deployment is resumed")
			deployutil.SetDeploymentCondition(newStatus, *condition)
		}
	}
}
