...
func TestStop(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			kexpected: []clientgotesting.Action{
				clientgotesting.NewListAction(replicationControllersResource, "default", metav1.ListOptions{LabelSelector: labels.SelectorFromSet(map[string]string{"openshift.io/deployment-config.name": "config"}).String()}),
				clientgotesting.NewGetAction(replicationControllersResource, "default", "config-1"),
				clientgotesting.NewListAction(replicationControllersResource, "default", metav1.ListOptions{}),
				clientgotesting.NewGetAction(replicationControllersResource, "default", "config-1"),
