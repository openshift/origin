...
func (g *configRESTOptionsGetter) GetRESTOptions(resource schema.GroupResource) (generic.RESTOptions, error) {
	...
	decorator := func(
		...
		if requestedSize == nil {
			requestedSize = &configuredCacheSize
		}

		if *requestedSize == 0 || !g.cacheEnabled {
			glog.V(5).Infof("using uncached watch storage for %s", resource.String())
			return generic.UndecoratedStorage(copier, storageConfig, requestedSize, objectType, resourcePrefix, keyFunc, newListFn, getAttrsFunc, triggerFn)
		}

		glog.V(5).Infof("using watch cache storage (capacity=%v) for %s %#v", *requestedSize, resource.String(), storageConfig)
		return storageWithCacher(copier, storageConfig, requestedSize, objectType, resourcePrefix, keyFunc, newListFn, getAttrsFunc, triggerFn)
	}
