...
// findDuplicateTests returns a map of test names that appear more than once
...
func detectPreconditionSkips(tests []*testCase) []*junitapi.JUnitTestCase {
	...
	for _, test := range tests {
		...
		results = append(results, &junitapi.JUnitTestCase{
			Name: "[sig-trt] Precondition Skip Failure",
			FailureOutput: &junitapi.FailureOutput{
				Message: fmt.Sprintf("Test '%s' was skipped due to unmet cluster preconditions", test.name),
				Output: fmt.Sprintf("The following test was skipped because the cluster did not meet required preconditions:\n\n"+
					"Test: %s\n\n"+
					"Reason: %s\n\n"+
					"This failure indicates the cluster was not in a healthy state when the test attempted to run.\n"+
					"Please investigate and resolve the cluster health issues.",
					test.name, skipReason),
			},
		})
