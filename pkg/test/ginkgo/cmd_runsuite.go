...
func (o *GinkgoRunSuiteOptions) Run(suite *TestSuite, junitSuiteName string, monitorTestInfo monitortestframework.MonitorTestInitializationInfo, upgrade bool) error {
	...
	if len(os.Getenv("OPENSHIFT_SKIP_EXTERNAL_TESTS")) == 0 {
		...
		// a payload references (which are private, regardless of the registry).
		...
		// on a cluster.
		// For any situation in which the convention is violated, they must set
		// the REGISTRY_AUTH_FILE environment variable to an auths file with
