...
func (opt *Options) Run(suite *TestSuite, junitSuiteName string) error {
	...
	// Create a place to store events added by synthetic tests.
	synthetictests.SyntheticTestIntervals = monitor.NewMonitorWithInterval(time.Second)
	...
	syntheticTestIntervalsLength := len(synthetictests.SyntheticTestIntervals.GetUnsortedEvents())
	if syntheticTestIntervalsLength > 0 {
		fmt.Println("Adding events generated by synthetic tests:", syntheticTestIntervalsLength)
		if err := opt.MonitorEventsOptions.AppendSyntheticTestIntervals(synthetictests.SyntheticTestIntervals.GetUnsortedEvents()); err != nil {
			return err
		}
	}
