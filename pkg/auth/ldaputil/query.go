...
// RetrieveUniqueEntry queries for an LDAP entry with the given searchRequest. The query is expected
// to return one unqiue result. If this is not the case, errors are raised
func RetrieveUniqueEntry(clientConfig LDAPClientConfig, query *ldap.SearchRequest) (*ldap.Entry, error) {
	result, err := IssueLDAPQuery(clientConfig, query)
	if err != nil {
		return nil, err
	}
	if len(result) > 1 {
