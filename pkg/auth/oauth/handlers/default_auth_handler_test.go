...
func (h *mockChallengeHandler) AuthenticationChallengeNeeded(req *http.Request) (http.Header, error) {
	headers := http.Header{}
...
func TestWithOnlyChallengeErrors(t *testing.T) {
	...
	failingChallengeHandler1 := &mockChallengeHandler{err: errors.New(expectedError1)}
	failingChallengeHandler2 := &mockChallengeHandler{err: errors.New(expectedError2)}
	authHandler := NewUnionAuthenticationHandler(
		map[string]ChallengeAuthHandler{"first": failingChallengeHandler1, "second": failingChallengeHandler2},
		nil, nil)
	client := &testClient{&oauthapi.Client{RespondWithChallenges: true}}
	req, _ := http.NewRequest("GET", "http://example.org", nil)
	responseRecorder := httptest.NewRecorder()
...
func TestWithChallengeErrorsAndMergedSuccess(t *testing.T) {
	...
	failingChallengeHandler := &mockChallengeHandler{err: errors.New(expectedError)}
	workingChallengeHandler1 := &mockChallengeHandler{headerName: "Charlie", headerValue: "delta"}
	workingChallengeHandler2 := &mockChallengeHandler{headerName: "Echo", headerValue: "foxtrot"}
	workingChallengeHandler3 := &mockChallengeHandler{headerName: "Charlie", headerValue: "golf"}
	...
	authHandler := NewUnionAuthenticationHandler(
		map[string]ChallengeAuthHandler{
