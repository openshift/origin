...
func CollectIntervalsForPods(ctx context.Context, kubeClient kubernetes.Interface, namespace string, labelSelector labels.Selector) (monitorapi.Intervals, []*junitapi.JUnitTestCase, []error) {
	...
	logJunit := &junitapi.JUnitTestCase{
		Name:      fmt.Sprintf("[sig-network] can collect %v poller pod logs", labelSelector),
