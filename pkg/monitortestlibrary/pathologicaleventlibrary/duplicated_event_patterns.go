...
func newVsphereConfigurationTestsRollOutTooOftenEventMatcher(finalIntervals monitorapi.Intervals) EventMatcher {
	...
	for i := range configurationTestIntervals {
		configurationTestIntervals[i].To = configurationTestIntervals[i].To.Add(time.Minute)
		configurationTestIntervals[i].From = configurationTestIntervals[i].From.Add(time.Minute * -1)
	}
