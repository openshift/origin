...
func NewCmdNewApplication(fullName string, f *clientcmd.Factory, out io.Writer) *cobra.Command {
	...
	cmd.Flags().VarP(&config.BuildConfigEnv, "bc-env", "", "Specify key value pairs of environment variables to set into all build configs.")
	cmd.Flags().VarP(&config.DeploymentConfigEnv, "dc-env", "", "Specify key value pairs of environment variables to set into all deploymnet configs.")
