...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (*persistConn, error) {
	...
	// we always use https for this purpose
	if true || cm.targetScheme == "https" && !tlsDial {
...
func (pc *persistConn) writeLoop() {
	...
	for {
		...
		case wr := <-pc.writech:
			...
			var err error
			if pc.isProxy {
				wr.req.Request.WriteProxy(pc.bw)
			} else {
				wr.req.Request.Write(pc.bw)
			}
