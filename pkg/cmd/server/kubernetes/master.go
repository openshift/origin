...
func (c *MasterConfig) RunNamespaceController() {
	...
	// TODO: build this list from config or hook up to discovery
	groupVersionResources := []unversioned.GroupVersionResource{}

	// Find the list of namespaced resources via discovery that the namespace controller must manage
	namespaceKubeClient := internalclientset.NewForConfigOrDie(restclient.AddUserAgent(c.KubeConfig, "namespace-controller"))
	namespaceClientPool := dynamic.NewClientPool(restclient.AddUserAgent(c.KubeConfig, "namespace-controller"), dynamic.LegacyAPIPathResolverFunc)
	...
	namespaceController := namespacecontroller.NewNamespaceController(namespaceKubeClient, namespaceClientPool, groupVersionResources, c.ControllerManager.NamespaceSyncPeriod.Duration, kapi.FinalizerKubernetes)
...
func (c *MasterConfig) RunHPAController(oc *osclient.Client, kc *client.Client, heapsterNamespace string) {
	...
	podautoscaler.Run(utilwait.NeverStop)
}
