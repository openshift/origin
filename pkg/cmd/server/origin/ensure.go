...
func (c *MasterConfig) ensureOpenShiftSharedResourcesNamespace() {
	if _, err := c.KubeClientsetInternal().Namespaces().Get(c.Options.PolicyConfig.OpenShiftSharedResourcesNamespace, metav1.GetOptions{}); kapierror.IsNotFound(err) {
		namespace, createErr := c.KubeClientsetInternal().Namespaces().Create(&kapi.Namespace{ObjectMeta: metav1.ObjectMeta{Name: c.Options.PolicyConfig.OpenShiftSharedResourcesNamespace}})
		if createErr != nil {
...
func (c *MasterConfig) ensureOpenShiftInfraNamespace() {
	...
	namespace, err := c.KubeClientsetInternal().Namespaces().Create(&kapi.Namespace{ObjectMeta: metav1.ObjectMeta{Name: ns}})
	if kapierror.IsAlreadyExists(err) {
		// Get the persisted namespace
		namespace, err = c.KubeClientsetInternal().Namespaces().Get(ns, metav1.GetOptions{})
		if err != nil {
	...
	for _, saName := range bootstrappolicy.InfraSAs.GetServiceAccounts() {
		_, err := c.KubeClientsetInternal().ServiceAccounts(ns).Create(&kapi.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: saName}})
...
func (c *MasterConfig) ensureDefaultNamespaceServiceAccountRoles() {
	...
	for i := 0; i < 30; i++ {
		ns, err := c.KubeClientsetInternal().Namespaces().Get(metav1.NamespaceDefault, metav1.GetOptions{})
		if err == nil {
			namespace = ns
			break
		}
...
func (c *MasterConfig) ensureNamespaceServiceAccountRoleBindings(namespace *kapi.Namespace) {
	...
	if _, err := c.KubeClientsetInternal().Namespaces().Update(namespace); err != nil && !kapierror.IsConflict(err) {
...
func (c *MasterConfig) securityContextConstraintsSupported() (bool, error) {
	...
	serverGroupList, err := c.KubeClientsetInternal().ServerGroups()
	if err != nil {
		return false, err
	}
	...
	apiResourceList, err := c.KubeClientsetInternal().ServerResourcesForGroupVersion(legacyGroup.PreferredVersion.GroupVersion)
	if err != nil {
		return false, err
	}
...
func (c *MasterConfig) ensureDefaultSecurityContextConstraints() {
	...
	for _, scc := range bootstrappolicy.GetBootstrapSecurityContextConstraints(bootstrapSCCGroups, bootstrapSCCUsers) {
		_, err := c.KubeClientsetInternal().SecurityContextConstraints().Create(&scc)
		if kapierror.IsAlreadyExists(err) {
			continue
		}
		if err != nil {
