...
`
  ...
  $ %s openshift.local.config/master/node-config.yaml`
	...
	validateNodeConfigDeprecationMessage = `This command is deprecated and will be removed. Use 'openshift ex diagnostics --diagnostic NodeConfigCheck --node-config=<path/to/config.yaml' instead.`
)
