...
func (cfg *config) RunDeployer() error {
	...
	kubeExternalClient, err := kubernetes.NewForConfig(kcfg)
	if err != nil {
		return err
	}
	...
	restMapper.Reset()
	scaleKindResolver := scale.NewDiscoveryScaleKindResolver(kubeExternalClient.Discovery())
	scaleClient, err := scale.NewForConfig(kcfg, restMapper, dynamic.LegacyAPIPathResolverFunc, scaleKindResolver)
	if err != nil {
		return err
	}

	deployer := NewDeployer(kubeInternalClient, scaleClient, openshiftInternalImageClient, cfg.Out, cfg.ErrOut, cfg.Until)
	return deployer.Deploy(cfg.Namespace, cfg.rcName)
}

// NewDeployer makes a new Deployer from a kube client.
func NewDeployer(client kclientset.Interface, scaleClient scale.ScalesGetter, images imageclientinternal.Interface, out, errOut io.Writer,
	...
	return &Deployer{
		...
		strategyFor: func(config *appsapi.DeploymentConfig) (strategy.DeploymentStrategy, error) {
			...
			case appsapi.DeploymentStrategyTypeRecreate:
				return recreate.NewRecreateDeploymentStrategy(client, scaleClient, images.Image(),
			...
			case appsapi.DeploymentStrategyTypeRolling:
				recreateDeploymentStrategy := recreate.NewRecreateDeploymentStrategy(client, scaleClient, images.Image(),
