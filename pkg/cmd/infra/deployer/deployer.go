...
func (cfg *config) RunDeployer() error {
	...
	deployer := NewDeployer(kubeClient, kubeInternalClient, openshiftImageClient, cfg.Out, cfg.ErrOut, cfg.Until)
	return deployer.Deploy(cfg.Namespace, cfg.rcName)
}

// NewDeployer makes a new Deployer from a kube client.
func NewDeployer(kubeClient kubernetes.Interface, client kclientset.Interface, images imageclientv1.Interface, out, errOut io.Writer,
	...
	return &Deployer{
		...
		getDeployment: func(namespace, name string) (*corev1.ReplicationController, error) {
			return kubeClient.Core().ReplicationControllers(namespace).Get(name, metav1.GetOptions{})
		},
		getDeployments: func(namespace, configName string) (*corev1.ReplicationControllerList, error) {
			return kubeClient.Core().ReplicationControllers(namespace).List(metav1.ListOptions{LabelSelector: appsutil.ConfigSelector(configName).
				String()})
		},
		scaler: appsutil.NewReplicationControllerV1Scaler(kubeClient),
		strategyFor: func(config *appsapi.DeploymentConfig) (strategy.DeploymentStrategy, error) {
			...
			case appsapi.DeploymentStrategyTypeRolling:
				...
				return rolling.NewRollingDeploymentStrategy(config.Namespace, client, kubeClient, images,
					legacyscheme.Codecs.UniversalDecoder(), recreateDeploymentStrategy, out, errOut, until), nil
			default:
...
func (d *Deployer) Deploy(namespace, rcName string) error {
	...
	sort.Sort(appsutil.ByLatestVersionDescV1(deployments))
