...
func ensurePodSpec(modified *bool, existing *corev1.PodSpec, required corev1.PodSpec) {
	...
	// TODO this should be relaxed.  If someone wants to mount them from somewhere else, we should allow it
...
func ensureContainer(modified *bool, existing *corev1.Container, required corev1.Container) {
	...
	if len(required.WorkingDir) > 0 {
		SetString(modified, &existing.WorkingDir, required.WorkingDir)
	}
	...
	if required.LivenessProbe != nil {
		ensureProbePtr(modified, &existing.ReadinessProbe, required.ReadinessProbe.Handler)
	}
...
func ensureProbePtr(modified *bool, existing **corev1.Probe, required corev1.Handler) {
	if *existing == nil {
		*modified = true
		*existing = &corev1.Probe{Handler: required}
		return
	}
	ensureProbe(modified, *existing, required)
}

func ensureProbe(modified *bool, existing *corev1.Probe, required corev1.Handler) {
	ensureProbeHandler(modified, &existing.Handler, required)
}
