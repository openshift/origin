package docker_registry_operator
...
type DockerRegistryOperatorComponentOptions struct {
	InstallContext componentinstall.Context
}

func (c *DockerRegistryOperatorComponentOptions) Name() string {
	return "openshift-docker-registry-operator"
}

func (c *DockerRegistryOperatorComponentOptions) Install(dockerClient dockerhelper.Interface) error {
	...
	params := map[string]string{
		...
		"COMPONENT_IMAGE":       imageTemplate.ExpandOrDie("registry"),
	...
	glog.V(2).Infof("instantiating registry-operator template with parameters %v", params)

	component := componentinstall.Template{
		Name:            "openshift-registry-operator",
		Namespace:       namespace,
		RBACTemplate:    manifests.MustAsset("install/openshift-registry-operator/install-rbac.yaml"),
		InstallTemplate: manifests.MustAsset("install/openshift-registry-operator/install.yaml"),

		// wait until there's an available endpoint of registry
		WaitCondition: func() (bool, error) {
			glog.V(2).Infof("polling for registry availability ...")
			deployment, err := kubeAdminClient.AppsV1().Deployments("openshift-registry").Get("registry", metav1.GetOptions{})
	...
	operatorConfig, err := operatorClient.DockerregistryV1alpha1().OpenShiftDockerRegistryConfigs().Get("instance", metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	*/
	// TODO: set REGISTRY_OPENSHIFT_SERVER_ADDR
	if _, err := operatorClient.DockerregistryV1alpha1().OpenShiftDockerRegistryConfigs().Update(operatorConfig); err != nil {
		return err
	}
