...
func NewCmdExpose(fullName string, f *clientcmd.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	o := NewExposeOptions()

	cmd := kcmd.NewCmdExposeService(f, streams.Out)
...
func (o *ExposeOptions) Complete(cmd *cobra.Command, f *clientcmd.Factory, args []string) error {
	var err error
...
func (o *ExposeOptions) Validate(cmd *cobra.Command) error {
	r := o.Builder().
		...
		FilenameParam(o.EnforceNamespace, &oldresource.FilenameOptions{Recursive: false, Filenames: o.Filenames}).
