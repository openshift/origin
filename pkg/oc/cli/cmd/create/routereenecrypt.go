...
func NewCmdCreateReencryptRoute(fullName string, f *clientcmd.Factory, streams genericclioptions.IOStreams) *cobra.Command {
...
func (o *CreateReencryptRouteOptions) Complete(f *clientcmd.Factory, cmd *cobra.Command, args []string) error {
	return o.CreateRouteSubcommandOptions.Complete(f, cmd, args)
}

func (o *CreateReencryptRouteOptions) Run() error {
	serviceName, err := resolveServiceName(o.Mapper, o.Service)
	if err != nil {
		return err
	}
	route, err := route.UnsecuredRoute(o.KubeClient, o.Namespace, o.Name, serviceName, o.Port, false)
	if err != nil {
		return err
	}
	...
	if !o.DryRun {
		route, err = o.Client.Routes(o.Namespace).Create(route)
		if err != nil {
			return err
		}
	}

	return o.PrintObj(route)
}
