...
import (
	...
	"github.com/openshift/origin/pkg/kubectl"
...
func (f *Factory) NewCmdGet(out io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "get [(-o|--output=)table|json|yaml|template|templatefile] [-t <file>|--template=<file>] <resource> [<id>]",
...
Examples:
  ...
  <list single pod in json output format>`,
	...
	cmd.Flags().StringP("output", "o", "console", "Output format: console|json|yaml|template|templatefile")
