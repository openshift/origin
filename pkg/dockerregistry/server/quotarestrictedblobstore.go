// This module defines a wrapper for upstream blob store that does an image quota check before commiting image
// layer to a registry. Master server contains admission check that will refuse the manifest if the image
// exceeds whatever quota set. But the check occurs too late (after the layers are written). This addition
// allows us to refuse the layers and thus keep the storage clean.
//
