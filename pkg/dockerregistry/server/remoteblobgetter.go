...
type digestToStoreCache struct {
	mu   sync.RWMutex
	data map[string]distribution.BlobStore
}

func newDigestToStoreCache() digestToStoreCache {
	return digestToStoreCache{
		data: make(map[string]distribution.BlobStore),
	}
}

func (d *digestToStoreCache) Load(key string) (value distribution.BlobStore, ok bool) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	value, ok = d.data[key]
	return
...
func (d *digestToStoreCache) Store(key string, value distribution.BlobStore) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.data[key] = value
}
...
type remoteBlobGetterService struct {
	...
	digestToStore       digestToStoreCache
}
...
func NewBlobGetterService(
	...
	return &remoteBlobGetterService{
		...
		digestToStore:       newDigestToStoreCache(),
	}
}
...
func (rbgs *remoteBlobGetterService) Open(ctx context.Context, dgst digest.Digest) (distribution.ReadSeekCloser, error) {
	...
	store, ok := rbgs.digestToStore.Load(dgst.String())
	if ok {
	...
	store, ok = rbgs.digestToStore.Load(desc.Digest.String())
	if !ok {
...
func (rbgs *remoteBlobGetterService) proxyStat(
	...
	rbgs.digestToStore.Store(dgst.String(), pullthroughBlobStore)
	return desc, nil
}
