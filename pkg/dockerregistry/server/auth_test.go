...
func TestAccessController(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"valid openshift token but invalid namespace": {
			...
			openshiftResponses: []response{
				{200, runtime.EncodeOrDie(kapi.Codecs.LegacyCodec(registered.GroupOrDie(kapi.GroupName).GroupVersions[0]), &userapi.User{ObjectMeta: metav1.ObjectMeta{Name: "usr1"}})},
			},
