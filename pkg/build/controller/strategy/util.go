...
// Each CA is referenced by key within the ConfigMap, ensuring that the pod is not scheduled
// until all CA keys are present.
//
// The following certificate authorities are mounted:
//
// 1. The cluster service signing CA, as generated by the service-ca-operator's controllers.
				...
				VolumeSource: corev1.VolumeSource{
					ConfigMap: &corev1.ConfigMapVolumeSource{
						...
						Items: []corev1.KeyToPath{
							// The BuildServiceCAKey is not initially present in the referenced
							// ConfigMap - it is created by the service signing CA controller.
							// We rely on the kubelet's behavior to prevent the pod from proceeding
							// past the ContainerCreating phase until this key is injected.
							{
								Key:  buildutil.BuildServiceCAKey,
