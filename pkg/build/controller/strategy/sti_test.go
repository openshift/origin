...
func testSTICreateBuildPod(t *testing.T, rootAllowed bool) {
	...
	// the pod has 10 volumes but the git source secret is not mounted into the main container.
	if len(container.VolumeMounts) != 9 {
		t.Fatalf("Expected 9 volumes in container, got %d", len(container.VolumeMounts))
	}
	for i, expected := range []string{buildutil.BuildWorkDirMount, dockerSocketPath, "/var/run/crio/crio.sock", DockerPushSecretMountPath, DockerPullSecretMountPath, filepath.Join(SecretBuildSourceBaseMountPath, "secret"), filepath.Join(ConfigMapBuildSourceBaseMountPath, "configmap"), ConfigMapBuildSystemConfigsMountPath, DaemonlessGraphRoot} {
