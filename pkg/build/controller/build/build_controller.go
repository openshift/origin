...
// createBuildCAConfigMapSpec creates a ConfigMap template containing the following:
//
// 1. The service signing CA that is injected by the service cert signer controller.
//
...
func (bc *BuildController) createBuildCAConfigMapSpec(build *buildv1.Build, buildPod *corev1.Pod) *corev1.ConfigMap {
	cm := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			...
			// Tell the service signer CA controller to inject the CA into this ConfigMap.
			// Since the build pod references the injected key `service-ca.crt`, it will
			// not proceed past ContainerCreating until the CA is added to this ConfigMap.
