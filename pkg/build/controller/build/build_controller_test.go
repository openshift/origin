...
func TestCreateBuildCAConfigMap(t *testing.T) {
	...
	for key, value := range caMap.Annotations {
		if key == "service.alpha.openshift.io/inject-cabundle" && value == "true" {
			hasInjectAnnotation = true
			break
		}
	}
	if !hasInjectAnnotation {
		t.Errorf("build CA configMap is missing service signing CA annotation, got %v", caMap.Annotations)
	}
