...
var (
	// DaemonlessIsolation is the default isolation type to use when building
	// without using a daemon as the build engine.
	DaemonlessIsolation = buildah.IsolationChroot
	// DaemonlessStoreOptions provide settings for storage that we'll use
	// when building without using a daemon as the build engine.
	DaemonlessStoreOptions = storage.StoreOptions{
		GraphRoot:          storage.DefaultStoreOptions.GraphRoot,
		RunRoot:            storage.DefaultStoreOptions.RunRoot,
		GraphDriverName:    "vfs",
		GraphDriverOptions: nil,
	}
	// ClientTypeUnknown is an error returned when we can't figure out which
	// type of "client" we're using.
...
func inspectDaemonlessImage(name string) (*docker.Image, error) {
	store, err := storage.GetStore(DaemonlessStoreOptions)
	if err != nil {
		return nil, err
	}
...
func daemonlessRun(ctx context.Context, createOpts docker.CreateContainerOptions, attachOpts docker.AttachToContainerOptions) error {
	store, err := storage.GetStore(DaemonlessStoreOptions)
	if err != nil {
		return err
	}
...
// GetDaemonless returns a valid implemenatation of the DockerClient interface, or an error
// if the implementation couldn't be created.
func GetDaemonless(store storage.Store) (client DockerClient, err error) {
...
func (d *DaemonlessClient) BuildImage(opts docker.BuildImageOptions) error {
	return buildDaemonlessImage(opts.ContextDir, buildapiv1.ImageOptimizationNone, &opts)
}
...
func (d *DaemonlessClient) PushImage(opts docker.PushImageOptions, auth docker.AuthConfiguration) error {
	...
	return pushDaemonlessImage(imageName, auth)
}
...
func (d *DaemonlessClient) DownloadFromContainer(id string, opts docker.DownloadFromContainerOptions) error {
	...
	rc, err := archive.TarWithOptions(filepath.Join(mp, opts.Path), &options)
	if err != nil {
		return err
	}
	defer rc.Close()
	_, err = io.Copy(opts.OutputStream, rc)
	return err
}
...
func (d *DaemonlessClient) PullImage(opts docker.PullImageOptions, auth docker.AuthConfiguration) error {
	...
	return pullDaemonlessImage(imageName, auth)
}
...
func (d *DaemonlessClient) TagImage(name string, opts docker.TagImageOptions) error {
	...
	return tagDaemonlessImage(name, imageName)
}

func (d *DaemonlessClient) InspectImage(name string) (*docker.Image, error) {
	return inspectDaemonlessImage(name)
}
