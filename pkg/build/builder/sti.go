...
// injectConfigs creates an s2i `VolumeSpec` from each provided `ConfigMapBuildSource`
func injectConfigs(configs []buildapiv1.ConfigMapBuildSource) []s2iapi.VolumeSpec {
	vols := make([]s2iapi.VolumeSpec, len(configs))
	for i := range configs {
		vols[i] = makeVolumeSpec(configs[i], strategy.ConfigMapBuildSourceBaseMountPath)
	}
	return vols
}
...
func injectSecrets(secrets []buildapiv1.SecretBuildSource) []s2iapi.VolumeSpec {
	...
	for i := range secrets {
		vols[i] = makeVolumeSpec(secrets[i], strategy.SecretBuildSourceBaseMountPath)
	}
	return vols
}

func makeVolumeSpec(src buildapiv1.LocalObjectBuildSource, mountPath string) s2iapi.VolumeSpec {
...
func (s *S2IBuilder) Build() error {
	...
	injections = append(injections, injectConfigs(s.build.Spec.Source.Configs)...)
