...
var (
	testDefaultRegistry = imagestream.DefaultRegistryFunc(func() (string, bool) { return "defaultregistry:5000", true })
	testImage           = &api.Image{
...
func setup(t *testing.T) (*tools.FakeEtcdClient, kstorage.Interface, *REST) {
	...
	imageStorage, imageStatusStorage, imageFinalizeStorage := imageetcd.NewREST(helper)
...
func TestList(t *testing.T) {
	...
	fakeEtcdClient.Data["/images/id"] = tools.EtcdResponseWithError{
		R: &etcd.Response{
			Node: &etcd.Node{
				Value: runtime.EncodeOrDie(latest.Codec, testImage),
			},
		},
	}

	fakeEtcdClient.Data["/images/unreferenced"] = tools.EtcdResponseWithError{
		R: &etcd.Response{
			Node: &etcd.Node{
	...
	isiList := list.(*api.ImageStreamImageList)
	if len(isiList.Items) != 1 {
		t.Fatalf("expected 1 item, got %d", len(isiList.Items))
	}
	isi := isiList.Items[0]
	// validate a couple of the fields
	if isi.Namespace != "ns" {
		t.Errorf("got unexpected namespace %q", isi.Namespace, "ns")
	}
	expectedImageName := "repo@id"
	if isi.Name != expectedImageName {
		t.Errorf("got unexpected name (%q != %q)", isi.Name, expectedImageName)
	}
	expectedMID := "2d24f826cb16146e2016ff349a8a33ed5830f3b938d45c0f82943f4ab8c097e7"
	if isi.Image.DockerImageMetadata.ID != expectedMID {
		t.Errorf("got unexpected image id (%q != %q)", isi.Image.DockerImageMetadata.ID, expectedMID)
	}
}
