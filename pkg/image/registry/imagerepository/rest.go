...
func (s *REST) fillRepository(repo *api.ImageRepository) {
	...
	if len(repo.DockerImageRepository) != 0 {
		registry = repo.DockerImageRepository
	} else {
		registry = api.JoinDockerPullSpec(s.defaultRegistry, repo.Namespace, repo.Name, "")
	}
	repo.Status.DockerImageRepository = registry
}
