...
type imageResolutionPolicy interface {
	...
	RequestsResolution(imagepolicy.GroupResource) bool
	// FailOnResolutionFailure returns true if you should fail when resolution fails
	FailOnResolutionFailure(imagepolicy.GroupResource) bool
	// RewriteImagePullSpec returns true if you should rewrite image pull specs when resolution succeeds
	RewriteImagePullSpec(attr *rules.ImagePolicyAttributes, isUpdate bool, gr imagepolicy.GroupResource) bool
}
...
func (a *imagePolicyPlugin) admit(attr admission.Attributes, mutationAllowed bool) error {
	...
	apigr := attr.GetResource().GroupResource()
	gr := imagepolicy.GroupResource{Resource: apigr.Resource, Group: apigr.Group}
	...
	if err != nil {
		return apierrs.NewForbidden(apigr, attr.GetName(), fmt.Errorf("unable to apply image policy against objects of type %T: %v", attr.GetObject(), err))
	}
...
func (config resolutionConfig) Covers(gr imagepolicy.GroupResource) bool {
...
func (config resolutionConfig) RequestsResolution(gr imagepolicy.GroupResource) bool {
...
func (config resolutionConfig) FailOnResolutionFailure(gr imagepolicy.GroupResource) bool {
	return FailOnResolutionFailure(config.config.ResolveImages)
}

var skipImageRewriteOnUpdate = map[imagepolicy.GroupResource]struct{}{
...
func (config resolutionConfig) RewriteImagePullSpec(attr *rules.ImagePolicyAttributes, isUpdate bool, gr imagepolicy.GroupResource) bool {
	if isUpdate {
...
func resolutionRuleCoversResource(rule imagepolicy.GroupResource, gr imagepolicy.GroupResource) bool {
