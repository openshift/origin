...
func TestAccept(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"when all rules are deny, match everything else": {
			...
			rules: []api.ImageExecutionPolicyRule{
				{ImageCondition: api.ImageCondition{Reject: true, OnResources: []unversioned.GroupResource{podResource}, MatchIntegratedRegistry: true, AllowResolutionFailure: true}},
			},
			accepts: []acceptResult{
				{ImagePolicyAttributes{}, true},
		...
		"exclude a deny rule": {
			matcher: NewRegistryMatcher([]string{"myregistry:5000"}),
			rules: []api.ImageExecutionPolicyRule{
				{ImageCondition: api.ImageCondition{Name: "excluded-rule", Reject: true, OnResources: []unversioned.GroupResource{podResource}, MatchIntegratedRegistry: true, AllowResolutionFailure: true}},
			},
			accepts: []acceptResult{
		...
		"accepts matching image labels": {
			matcher: NewRegistryMatcher([]string{"myregistry:5000"}),
			rules: []api.ImageExecutionPolicyRule{
				{ImageCondition: api.ImageCondition{OnResources: []unversioned.GroupResource{podResource}, MatchImageLabels: []api.ValueCondition{{Key: "label1", Value: "value1"}}}},
			},
			accepts: []acceptResult{
		...
		"accepts matching multiple image label values": {
			...
			rules: []api.ImageExecutionPolicyRule{
				{ImageCondition: api.ImageCondition{OnResources: []unversioned.GroupResource{podResource}, MatchImageLabels: []api.ValueCondition{{Key: "label1", Value: "value1"}}}},
				{ImageCondition: api.ImageCondition{OnResources: []unversioned.GroupResource{podResource}, MatchImageLabels: []api.ValueCondition{{Key: "label1", Value: "value2"}}}},
			},
			accepts: []acceptResult{
				{ImagePolicyAttributes{Resource: podResource}, false},
				{ImagePolicyAttributes{Resource: podResource, Image: &imageapi.Image{ObjectMeta: kapi.ObjectMeta{Labels: map[string]string{"label1": "value1"}}}}, true},
		...
		"accepts matching image labels by key": {
			matcher: NewRegistryMatcher([]string{"myregistry:5000"}),
			rules: []api.ImageExecutionPolicyRule{
				{ImageCondition: api.ImageCondition{OnResources: []unversioned.GroupResource{podResource}, MatchImageLabels: []api.ValueCondition{{Key: "label1", Set: true}}}},
			},
			accepts: []acceptResult{
				{ImagePolicyAttributes{Resource: podResource}, false},
				{ImagePolicyAttributes{Resource: podResource, Image: &imageapi.Image{ObjectMeta: kapi.ObjectMeta{Labels: map[string]string{"label1": "value1"}}}}, true},
	...
	for test, testCase := range testCases {
		a := NewExecutionRulesAccepter(testCase.rules, testCase.matcher)
