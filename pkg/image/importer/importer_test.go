	...
	testCases := []struct {
		...
		},
		{
			...
			expect: func(isi *api.ImageStreamImport, t *testing.T) {
				...
				if !expectStatusError(isi.Status.Images[2].Status, ".image.openshift.io \"\" is invalid: from.name: Invalid value: \"test///un/parse/able/image\": invalid name: invalid reference format") {
