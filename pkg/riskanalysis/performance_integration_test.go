...
// An uncached call can take many seconds so run go test with e.g. -test.benchtime=10x
func BenchmarkRequestRiskAnalysis(b *testing.B) {
	...
	if sippyUrl == "" {
		...
		b.Skip("set TEST_RA_SIPPY_URL to run this test; e.g. http://localhost:8080/ or https://sippy.dptools.openshift.org/")
	}
	...
	client := &http.Client{}
	tmp := b.TempDir()
	uri, err := url.JoinPath(sippyUrl, raApiUrl)
	if err != nil {
		b.Fatalf("could not join %q and %q: %v", sippyUrl, raApiUrl, err)
	}
	opt := &Options{SippyURL: uri, JUnitDir: tmp}
	for b.Loop() {
