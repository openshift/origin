...
func (p *HybridProxier) OnServiceAdd(service *corev1.Service) {
	...
	if isUsingUserspace, ok := p.usingUserspace[svcName]; ok && isUsingUserspace {
		klog.V(2).Infof("hybrid proxy: add svc %s/%s in unidling proxy", service.Namespace, service.Name)
		p.unidlingServiceHandler.OnServiceAdd(service)
	} else {
		klog.V(2).Infof("hybrid proxy: add svc %s/%s in main proxy", service.Namespace, service.Name)
...
func (p *HybridProxier) OnServiceUpdate(oldService, service *corev1.Service) {
	...
	if isUsingUserspace, ok := p.usingUserspace[svcName]; ok && isUsingUserspace {
		klog.V(2).Infof("hybrid proxy: update svc %s/%s in unidling proxy", service.Namespace, service.Name)
		p.unidlingServiceHandler.OnServiceUpdate(oldService, service)
	} else {
		klog.V(2).Infof("hybrid proxy: update svc %s/%s in main proxy", service.Namespace, service.Name)
...
func (p *HybridProxier) OnServiceDelete(service *corev1.Service) {
	...
	if isUsingUserspace, ok := p.usingUserspace[svcName]; ok && isUsingUserspace {
		klog.V(2).Infof("hybrid proxy: del svc %s/%s in unidling proxy", service.Namespace, service.Name)
		p.unidlingServiceHandler.OnServiceDelete(service)
	} else {
		klog.V(2).Infof("hybrid proxy: del svc %s/%s in main proxy", service.Namespace, service.Name)
...
func (p *HybridProxier) switchService(name types.NamespacedName) {
	...
	if p.usingUserspace[name] {
		klog.V(2).Infof("hybrid proxy: switching svc %s/%s to unidling proxy", svc.Namespace, svc.Name)
		p.unidlingServiceHandler.OnServiceAdd(svc)
		p.mainServicesHandler.OnServiceDelete(svc)
	} else {
		klog.V(2).Infof("hybrid proxy: switching svc %s/%s to main proxy", svc.Namespace, svc.Name)
...
func (p *HybridProxier) OnEndpointsAdd(endpoints *corev1.Endpoints) {
	...
	klog.V(2).Infof("hybrid proxy: (always) add ep %s/%s in unidling proxy", endpoints.Namespace, endpoints.Name)
	...
	if !p.usingUserspace[svcName] {
		klog.V(2).Infof("hybrid proxy: add ep %s/%s in main proxy", endpoints.Namespace, endpoints.Name)
...
func (p *HybridProxier) OnEndpointsUpdate(oldEndpoints, endpoints *corev1.Endpoints) {
	// we track all endpoints in the unidling endpoints handler so that we can succesfully
	// detect when a service become unidling
	klog.V(2).Infof("hybrid proxy: (always) update ep %s/%s in unidling proxy", endpoints.Namespace, endpoints.Name)
	...
	if !isSwitch && !p.usingUserspace[svcName] {
		klog.V(2).Infof("hybrid proxy: update ep %s/%s in main proxy", endpoints.Namespace, endpoints.Name)
		p.mainEndpointsHandler.OnEndpointsUpdate(oldEndpoints, endpoints)
		return
	}

	if p.usingUserspace[svcName] {
		klog.V(2).Infof("hybrid proxy: del ep %s/%s in main proxy", endpoints.Namespace, endpoints.Name)
...
func (p *HybridProxier) OnEndpointsDelete(endpoints *corev1.Endpoints) {
	// we track all endpoints in the unidling endpoints handler so that we can succesfully
	// detect when a service become unidling
	klog.V(2).Infof("hybrid proxy: (always) del ep %s/%s in unidling proxy", endpoints.Namespace, endpoints.Name)
