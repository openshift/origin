...
- Create the [template configuration file](gluster-nginx-template.yaml) with the reference to the PVC embedded in the configuration to utilize the “glusterfs” plugin and our gluster volume.
...
```
        ...
        oc create -f gluster-nginx-template.yaml



        [root@ose1 nginx_gluster_pvc_template]# oc create -f gluster-nginx-template.yaml
