...
# having this set causes some spurious errors when testing traps
unset OS_DESCRIBE_RETURN_CODE
...
os::cmd::expect_code_and_text './err_with_errexit.sh' '127' '\[DEBUG\] Error handler executing with return code `127`, last command `not_a_function`, and errexit set `true`'
## ensure EXIT handler doesn't fire in above test
os::cmd::expect_code_and_not_text './err_with_errexit.sh' '127' '\[DEBUG\] Exit handler executing'

## ensure ERR handler returns failure code correctly and ERR handler runs
os::cmd::expect_code_and_text './err_without_errexit.sh' '127' '\[DEBUG\] Error handler executing with return code `127`, last command `not_a_function`, and errexit set ``'
## ensure EXIT handler doesn't fire in above test
os::cmd::expect_code_and_not_text './err_without_errexit.sh' '127' '\[DEBUG\] Exit handler executing'

## ensure EXIT handler returns failure code correctly and EXIT handler runs
os::cmd::expect_code_and_text './err_exit.sh' '55' '\[DEBUG\] Exit handler executing with return code `55`'
## ensure ERR handler doesn't fire in above test
os::cmd::expect_code_and_not_text './err_exit.sh' '55' '\[DEBUG\] Error handler executing with return code'

## ensure EXIT handler returns success code correctly and EXIT handler runs
os::cmd::expect_code_and_text './success_exit.sh' '0' '\[DEBUG\] Exit handler executing with return code `0`'
## ensure ERR handler doesn't fire in above test
os::cmd::expect_code_and_not_text './success_exit.sh' '0' '\[DEBUG\] Error handler executing with return code'
