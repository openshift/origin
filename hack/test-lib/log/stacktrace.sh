...
os::cmd::expect_code_and_text './err_in_func.sh' '2' '\[ERROR\] ./err_in_func:29: `grep > /dev/null 2>&1` exited with status 2.
\[INFO\] 		Stack Trace: 
...
os::cmd::expect_code_and_not_text './err_in_func_no_errexit.sh' '2' '.'

# an error in a script causes a stacktrace with errexit set
os::cmd::expect_code_and_text './err_in_script.sh' '2' '\[ERROR\] ./err_in_script:16: `grep > /dev/null 2>&1` exited with status 2.
\[INFO\] 		Stack Trace: 
\[INFO\] 		  1: ./err_in_script:16: `grep > /dev/null 2>&1`
\[INFO\]   Exiting with code 2.'

# an error in a script causes no stacktrace with errexit unset
os::cmd::expect_code_and_not_text './err_in_script_no_errexit.sh' '2' '.'
