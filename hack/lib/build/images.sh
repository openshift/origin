...
function os::build::cross_images() {
	...
	while (( "$#" )); do
		...
		local arch="${1}"
		[[ "${OS_BUILD_SUPPORTED_ARCHES[@]}" =~ "${arch}" ]] || break
	...
	if [[ -z ${arches:-} ]]; then
		os::log::error "At least one arch needs to be specified, supported arches: ${OS_BUILD_SUPPORTED_ARCHES[@]}"
		exit 1
	fi
	...
	for arch in ${arches[@]}; do
		local sys_arch
		sys_arch=$(os::build::goarch_to_sysarch $arch)
		OS_BUILD_IMAGE_ARGS=""
		if [[ "${arch}" != "${host_arch}" ]]; then
			...
			qemu_binary_path=$(os::util::find::system_binary $qemu_binary)
		...
		for tag in ${tags_to_apply[@]}; do
			...
			# If the build is for the amd64, add a tag without the architecture
			if [[ "${arch}" == "amd64" ]]; then
...
function os::build::image::internal::get_arch_specific_from() {
	...
	case "${from_image}" in
		...
		openshift/origin-*)
			...
			echo "${from_image}-$(os::build::goarch_to_sysarch $arch)"
			;;
		*)
			# the image as is
			echo "${from_image}"
