...
if [[ -n "${junit_report}" ]]; then
    ...
    go test -p 1 -json ${gotest_flags} ${test_packages} 2>"${test_error_file}" | tee "${JUNIT_REPORT_OUTPUT}" | gotest2junit > "${report_file}"
...
$( cat "${test_error_file}") "
    ...
    for test_package in ${test_packages}; do
        ...
        go test -p 1 ${local_gotest_flags} ${test_package}
    done
