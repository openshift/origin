...
if [[ -z "$(which iptables)" ]]; then
  ...
  echo "The end-to-end test requires a system with iptables for Kubernetes services."
  exit 1
fi
...
if [ $? -ne 0 ]; then
  ...
  if [ $? -ne 0 ]; then
    echo "You do not have iptables or sudo privileges."
    echo "Kubernetes services will not work without iptables access."
    echo "See https://github.com/GoogleCloudPlatform/kubernetes/issues/1859"
    echo "--> Try 'sudo hack/test-end-to-end.sh'."
    exit 1
  fi
fi
...
# Use the Docker bridge ip address until there is a good way to get the auto-selected
# address from master this address is considered stable.
...
echo "[INFO] OpenShift and Kubernetes versions:"
...
echo "[INFO] Extended tests:            ${EXTENDED}"
echo
...
# Process the Template and create
...
# Run extended tests if the EXTENDED is set
if [[ ! -z "${EXTENDED-}" ]]; then
  source ${OS_ROOT}/test/extended/run.sh
fi
