...
import (
	...
	"k8s.io/kubernetes/pkg/runtime/unversioned"
...
func (f *fakeClientAccessFactory) ClientSetForVersion(requiredVersion *unversioned.GroupVersion) (internalclientset.Interface, error) {
...
var (
	podsResource = unversioned.GroupVersionResource{Resource: "pods"}
)
...
func TestLogsForObject(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "pod logs",
			obj: &api.Pod{
				ObjectMeta: api.ObjectMeta{Name: "hello", Namespace: "test"},
			},
		...
		},
		{
			...
			pods: []runtime.Object{testPod()},
			actions: []core.Action{
				core.NewListAction(podsResource, "test", metav1.ListOptions{LabelSelector: "foo=bar"}),
				getLogsAction("test", nil),
			},
		},
		{
...
func getLogsAction(namespace string, opts *api.PodLogOptions) core.Action {
