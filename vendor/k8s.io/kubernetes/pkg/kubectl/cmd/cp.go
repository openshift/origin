...
func copyFromPod(f cmdutil.Factory, cmd *cobra.Command, cmderr io.Writer, src, dest fileSpec) error {
	...
	return untarAll(cmderr, reader, dest.File, prefix)
}
...
func untarAll(cmderr io.Writer, reader io.Reader, destFile, prefix string) error {
	entrySeq := -1
	...
	for {
		...
		if mode&os.ModeSymlink != 0 {
			...
			if path.IsAbs(linkname) && (err != nil || relative != stripPathShortcuts(relative)) {
				fmt.Fprintf(cmderr, "warning: link %q is pointing to %q which is outside target destination, skipping\n", outFileName, header.Linkname)
				continue
			}
