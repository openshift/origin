...
func TestNonNullSerializedSlices(t *testing.T) {
	for gvk, knownType := range api.Scheme.AllKnownTypes() {
		if gvk.Version != runtime.APIVersionInternal {
			ensureNonNullSerializedSlices(t, gvk, knownType, nil)
		}
	}
}
...
func ensureNonNullSerializedSlices(t *testing.T, gvk schema.GroupVersionKind, tp reflect.Type, parents []reflect.Type) {
	parents = append(parents, tp)

	switch tp.Kind() {
	case reflect.Map, reflect.Slice, reflect.Ptr:
		ensureNonNullSerializedSlices(t, gvk, tp.Elem(), parents)
...
If the field will never be returned from the API server while null, add to falsePositiveEmptySliceFields with a comment explaining why.`,
			...
			ensureNonNullSerializedSlices(t, gvk, f.Type, parents)
		}
