...
func createProxier(loadBalancer LoadBalancer, listenIP net.IP, iptables iptables.Interface, exec utilexec.Interface, hostIP net.IP, proxyPorts PortAllocator, syncPeriod, minSyncPeriod, udpIdleTimeout time.Duration, makeProxySocket ProxySocketFunc) (*Proxier, error) {
	...
	glog.V(3).Infof("minSyncPeriod: %v, syncPeriod: %v, burstSyncs: %d", minSyncPeriod, syncPeriod, numBurstSyncs)
...
func (proxier *Proxier) syncProxyRules() {
	start := time.Now()
	defer func() {
		glog.V(2).Infof("userspace syncProxyRules took %v", time.Since(start))
	}()

	// don't sync rules till we've received services and endpoints
	if !proxier.isInitialized() {
		glog.V(2).Info("Not syncing userspace proxy until Services and Endpoints have been received from master")
		return
	}
	...
	glog.V(2).Infof("userspace proxy: processing %d service events", len(changes))
...
// connections, for now.
func (proxier *Proxier) addServiceOnPortInternal(service proxy.ServicePortName, protocol api.Protocol, proxyPort int, timeout time.Duration) (*ServiceInfo, error) {
...
func (proxier *Proxier) serviceChange(previous, current *api.Service, detail string) {
	...
	glog.V(4).Infof("userspace proxy: %s for %s", detail, svcName)
...
func (proxier *Proxier) OnServiceSynced() {
	glog.V(2).Infof("userspace OnServiceSynced")
...
func (proxier *Proxier) OnEndpointsSynced() {
	glog.V(2).Infof("userspace OnEndpointsSynced")
