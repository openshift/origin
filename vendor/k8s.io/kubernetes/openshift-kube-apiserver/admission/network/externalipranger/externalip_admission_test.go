...
func TestAdmission(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			admit:    true,
			op:       admission.Update,
			testName: "No external IPs on update for test:ordinary-user user",
			userinfo: serviceaccount.UserInfo("test", "ordinary-user", ""),
		},
		{
			admit:       false,
		...
		},
		{
			...
			externalIPs: []string{"1.2.3.4"},
			op:          admission.Update,
			testName:    "No external IPs allowed on update for test:ordinary-user user",
			errFn:       func(err error) bool { return strings.Contains(err.Error(), "externalIPs have been disabled") },
			userinfo:    serviceaccount.UserInfo("test", "ordinary-user", ""),
		},
		{
	...
	for _, test := range tests {
		...
		err = handler.Validate(admission.NewAttributesRecord(svc, oldSvc, kapi.Kind("Service").WithVersion("version"), "namespace", svc.ObjectMeta.Name, kapi.Resource("services").WithVersion("version"), "", test.op, false, test.userinfo), nil)
