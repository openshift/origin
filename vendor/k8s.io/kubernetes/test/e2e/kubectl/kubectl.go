...
var _ = SIGDescribe("Kubectl client", func() {
	...
	ginkgo.Describe("Simple pod", func() {
		...
		ginkgo.It("should contain last line of the log", func() {
			...
			framework.RunKubectlOrDie("run", podName, "--image="+busyboxImage, "--restart=OnFailure", nsFlag, "--", "sh", "-c", "sleep 10; seq 100 | while read i; do echo $i; sleep 0.01; done; echo EOF")
...
`), os.FileMode(0755)))
	...
	ginkgo.Describe("Kubectl logs", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			framework.RunKubectlOrDie("run", podName, "--image="+agnhostImage, nsFlag, "--", "logs-generator", "--log-lines-total", "100", "--run-duration", "20s")
		})
		ginkgo.AfterEach(func() {
	...
	ginkgo.Describe("Kubectl rolling-update", func() {
		...
		*/
		framework.ConformanceIt("should support rolling-update to same image ", func() {
			...
			framework.RunKubectlOrDieInput(httpdRC, "create", "-f", "-", fmt.Sprintf("--namespace=%v", ns))
