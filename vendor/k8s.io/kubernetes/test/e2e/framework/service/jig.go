...
func (j *TestJig) CreatePDB(rc *v1.ReplicationController) error {
	pdb := j.newPDBTemplate(rc)
	_, err := j.Client.PolicyV1beta1().PodDisruptionBudgets(j.Namespace).Create(context.TODO(), pdb, metav1.CreateOptions{})
	if err != nil {
		return fmt.Errorf("failed to create PDB %q %v", pdb.Name, err)
	}
	if err := j.waitForPdbReady(); err != nil {
		return fmt.Errorf("failed waiting for PDB to be ready: %v", err)
	}
