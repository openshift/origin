...
func (client APIProductsClient) ListByAPI(ctx context.Context, resourceGroupName string, serviceName string, apiid string, filter string, top *int32, skip *int32) (result ProductCollectionPage, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/APIProductsClient.ListByAPI")
		defer func() {
			sc := -1
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("apimanagement.APIProductsClient", "ListByAPI", err.Error())
	}
	...
	result.pc, err = client.ListByAPIResponder(resp)
	if err != nil {
...
func (client APIProductsClient) listByAPINextResults(ctx context.Context, lastResults ProductCollection) (result ProductCollection, err error) {
	req, err := lastResults.productCollectionPreparer(ctx)
	if err != nil {
		return result, autorest.NewErrorWithError(err, "apimanagement.APIProductsClient", "listByAPINextResults", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "apimanagement.APIProductsClient", "listByAPINextResults", resp, "Failure sending next results request")
	}
	result, err = client.ListByAPIResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apimanagement.APIProductsClient", "listByAPINextResults", resp, "Failure responding to next results request")
	}
	return
}
