...
Any data written an emptyDir volume by *ns1/pod-a* or *ns1/pod-b* share a 100 MB
...
* The XFS quota tools don't provide any means to delete a specific group's quota
configuration. The rules are stored on disk, and so the maximum number of quota
entries is limited by inode/disk space on the node. Even with a high rate of
quota-enabled pod churn it seems unlikely these limits will be reached during
