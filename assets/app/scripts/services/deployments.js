...
angular.module("openshiftConsole")
  .factory("DeploymentsService", function(DataService, $filter, LabelFilter){
    return {
      startLatestDeployment: function(deploymentConfig, context, $scope) {
      ...
      retryFailedDeployment: function(deployment, context, $scope) {
      ...
      rollbackToDeployment: function(deployment, changeScaleSettings, changeStrategy, changeTriggers, context, $scope) {
        var deploymentName = deployment.metadata.name;
        var deploymentConfigName = $filter('annotation')(deployment, 'deploymentConfig');
      ...
      cancelRunningDeployment: function(deployment, context, $scope) {
        var deploymentName = deployment.metadata.name;
        var deploymentConfigName = $filter('annotation')(deployment, 'deploymentConfig');
        var req = angular.copy(deployment);
      ...
      associateDeploymentsToDeploymentConfig: function(deployments, deploymentConfigs, filter) {
      ...
      deploymentBelongsToConfig: function(deployment, deploymentConfigName) {
        if (!deployment || !deploymentConfigName) {
          return false;
        }
        return deploymentConfigName === $filter('annotation')(deployment, 'deploymentConfig');
      },
      associateRunningDeploymentToDeploymentConfig: function(deploymentsByDeploymentConfig) {
      ...
      scale: function(deployment, replicas) {
        var req = angular.copy(deployment);
