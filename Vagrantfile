...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if dind_dev_cluster
    ...
    config.vm.define "#{VM_NAME_PREFIX}master" do |config|
      config.vm.box = kube_box[kube_os]["name"]
      config.vm.box_url = kube_box[kube_os]["box_url"]
      config.vm.provision "shell", inline: "/vagrant/contrib/vagrant/provision-master.sh #{master_ip} #{num_minion} #{minion_ips_str} #{instance_prefix} #{ENV['OPENSHIFT_SDN']}"
    ...
    num_minion.times do |n|
      config.vm.define "#{VM_NAME_PREFIX}minion-#{n+1}" do |minion|
        ...
        minion.vm.provision "shell", inline: "/vagrant/contrib/vagrant/provision-minion.sh #{master_ip} #{num_minion} #{minion_ips_str} #{instance_prefix} #{minion_ip} #{minion_index}"
