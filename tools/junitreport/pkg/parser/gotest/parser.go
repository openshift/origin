...
func (p *testOutputParser) Parse(input *bufio.Scanner) (*api.TestSuites, error) {
	...
	for input.Scan() {
		...
		case stateResults:
			...
			// if this is a result AND we have already declared this as a test, parse it, otherwise it's output
			if result, name, _, duration, ok := ExtractResult(output); ok && tests[name] != nil {
				...
				switch {
				case depth >= len(currentTestName):
					currentTestName = append(currentTestName, name)
				default:
					if depth < len(currentTestName)-1 {
						currentTestName = currentTestName[:depth]
					}
					currentTestName[len(currentTestName)-1] = name
				}
				continue
			}

			// treat as regular output at the appropriate depth
			log("  found message line %d %v\n", depth, currentTestName)
			// BUG: in go test, double nested output is double indented for some reason
			if depth >= len(currentTestName) {
				depth = len(currentTestName) - 1
			}
			name := currentTestName[depth]
			log("    name %s\n", name)
		...
		case stateComplete:
			// suite exit line
			if name, duration, coverage, ok := ExtractSuite(line); ok {
