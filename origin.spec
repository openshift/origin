...
%{!?commit:
%global commit cf7da2e57514e8ae09a651143dbe11d130bb4ceb
}
%global shortcommit %(c=%{commit}; echo ${c:0:7})
# os_git_vars needed to run hack scripts during rpm builds
%{!?os_git_vars:
%global os_git_vars OS_GIT_MINOR=10+ OS_GIT_MAJOR=3 OS_GIT_VERSION=v3.10.38 OS_GIT_TREE_STATE=clean OS_BUILD_LDFLAGS_DEFAULT_IMAGE_STREAMS=rhel7 KUBE_GIT_VERSION=v1.10.0+b81c8f8 OS_GIT_PATCH=38 KUBE_GIT_COMMIT=b81c8f8 KUBE_GIT_MINOR=10+ OS_GIT_COMMIT=48d5ab5 KUBE_GIT_MAJOR=1 OS_IMAGE_PREFIX=registry.access.redhat.com/openshift3/ose ETCD_GIT_VERSION=v3.2.16-0-g121edf0 ETCD_GIT_COMMIT=121edf0
}
...
- use v3.10 version of Service Catalog (jaboyd@redhat.com)
...
- wait longer for the builder service account (bparees@redhat.com)
...
- set full user info for SAR check on template readiness checks
  (bparees@redhat.com)
- UPSTREAM: 64971: Cherry pick of #64255 fix field removal in mutating
  admission webhooks (pmorie@redhat.com)
...
- UPSTREAM: 64882: Fix UnmountDevice with deleted pod (jsafrane@redhat.com)
- UPSTREAM: 64658: CSI fix for gRPC conn leak (jsafrane@redhat.com)
...
- UPSTREAM: 66350: Start cloudResourceSyncsManager before getNodeAnyWay
  ...
  addresses from a cloudprovider (avesh.ncsu@gmail.com)
- UPSTREAM: 65226: Store the latest cloud provider node addresses
  (avesh.ncsu@gmail.com)
- origin.spec: work around multi-arch build bug (adammhaile@gmail.com)
- UPSTREAM: 62943: set updated replicas in statefulsets
  (kargakis@protonmail.ch)
...
* Sun Jul 15 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.18-1
...
* Thu Jul 12 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.17-1
- disable all the problematic db replica tests on 3.10 only
  (bparees@redhat.com)
- bump(*) (deads@redhat.com)
- update glide for latest imagebuilder (deads@redhat.com)
- pin all 3.10 deps (deads@redhat.com)

* Tue Jul 10 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.16-1
- 

* Sun Jul 08 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.15-1
- 

* Fri Jul 06 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.14-1
...
* Sun Jul 01 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.11-1
- Bind mount /etc/origin/kubelet-plugins for flex volumes (hekumar@redhat.com)

* Fri Jun 29 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.10-1
...
* Tue Jun 26 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.9-1
- bump(imagebuilder): Some `COPY --from` statements would fail
  (ccoleman@redhat.com)

* Mon Jun 25 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.8-1
- UPSTREAM: 65223: Fixed detection of inaccessible AWS encryption key.
  (jsafrane@redhat.com)
...
* Thu Jun 21 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.5-1
- 

* Thu Jun 21 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.4-1
- Revert "Make NewCommandStartNode() handle SIGTERM and SIGINT"
  (amcdermo@redhat.com)

* Thu Jun 21 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.3-1
- 

* Wed Jun 20 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.2-1
- 

* Mon Jun 18 2018 Tim Bielawa <tbielawa@redhat.com> 3.10.1-1
- Updating branch release-3.10 (ccoleman@redhat.com)
- Branching 3.10 (tbielawa@redhat.com)
...
- Prevent a router panic when ROUTER_BIND_PORTS_AFTER_SYNC is set
  (bbennett@redhat.com)
- UPSTREAM: 64969: volume: decrease memory allocations for debugging messages
  (mfojtik@redhat.com)
- add log healthz check (jliggitt@redhat.com)
- UPSTREAM: 64946: log healthz check (jliggitt@redhat.com)
- bump(*) (sjenning@redhat.com)

* Mon Jun 11 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.66.0
...
- Update nextQueryTime field when getIPsAndMinTTL() fails (rpenta@redhat.com)
- Do not throw spurious error when minTTL=0 for the domain in egress network
  policy (rpenta@redhat.com)
- Add Casey Callendrello (squeed) to the relevant OWNERS files
  (bbennett@redhat.com)
...
- Add Dan Mace (ironcladlou) to relevant OWNERS (bbennett@redhat.com)
- Hide deprecated commands (maszulik@redhat.com)
- Use fully qualified pod test image names in network diagnostics
  (rpenta@redhat.com)
- support the --output flag in 'oc create route' (jtanenba@redhat.com)
- Add extended tests for oc image mirror (agladkov@redhat.com)
- Catch error from NewForConfig() as a new assigned error value
  (nakayamakenjiro@gmail.com)

* Wed Jun 06 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.63.0
- Make API storage migration less sensitive to conflicts (mkhan@redhat.com)

* Wed Jun 06 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.62.0
- UPSTREAM: 63650: Never clean backoff in job controller (maszulik@redhat.com)
- UPSTREAM: 58972: Fix job's backoff limit for restart policy OnFailure
  (maszulik@redhat.com)
...
- UPSTREAM: 64573: remove extra "../" when copying from pod to local
  (jvallejo@redhat.com)

* Fri Jun 01 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.58.0
- add --tag flag (jvallejo@redhat.com)
...
* Thu May 31 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.55.0
- Generated changes (maszulik@redhat.com)
- Wire up api-versions and api-resources to oc (maszulik@redhat.com)
- Allow egress-router to connect to its node's IP, via the SDN
  (danw@redhat.com)
- bump(*) (deads@redhat.com)
- update fork bump (deads@redhat.com)
- ovn-k8s-overlay executable  is deprecated (rchopra@redhat.com)
- UPSTREAM: 64516: Fix error message to be consistent with others
  (maszulik@redhat.com)
- UPSTREAM: 63421: Cache preferred resources, use in kubectl resource name
  autocomplete (single commit) (maszulik@redhat.com)
- UPSTREAM: 62254: Add name output and verb filtering to api-resources
  (maszulik@redhat.com)
- UPSTREAM: 42873: add kubectl api-resources command (maszulik@redhat.com)
- up: remove support for remote docker host (mfojtik@redhat.com)
- generated (deads@redhat.com)
- use library-go methods for webconsole operator (deads@redhat.com)
- bump(*) (deads@redhat.com)
...
- Turn off client rate limiting in integration tests (mkhan@redhat.com)
- Increase buffer for synchronization (agladkov@redhat.com)
- UPSTREAM: 62152: Keep node.kubeconfig correct during rotation
  (ccoleman@redhat.com)
- Drop template wrapping from bootstrap policy file (mrogers@redhat.com)
...
- Initialize defaultImageStreams (obulatov@redhat.com)
- Revert "Add extended tests for oc image mirror" (bparees@redhat.com)
- bump(*) reverts previous bump (bparees@redhat.com)
- UPSTREAM: <carry>: change config file location and restore perFSGroup to
  quantity (sjenning@redhat.com)
- UPSTREAM: 63977: pkg: kubelet: remote: increase grpc client default size
  (sjenning@redhat.com)
- catalog: start controller with --cluster-id-configmap-namespace=kube-service-
  catalog (jaboyd@redhat.com)
...
* Tue May 15 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.45.0
- UPSTREAM: 63875: make TestGetServerGroupsWithTimeout more reliable
  (deads@redhat.com)
- Update oc wrappers.go to remove default RunCommand opt (mgugino@redhat.com)
...
- skip publishing BUILD files to forks (jliggitt@redhat.com)
- retarget emptydirquota import (sjenning@redhat.com)
- remove XFS quota patch from origin (sjenning@redhat.com)
- UPSTREAM: <carry>: XFS quota for emptyDir volumes (sjenning@redhat.com)

* Mon May 14 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.43.0
- switch to using vendored version of SSCS and library-go (deads@redhat.com)
- bump(*) (deads@redhat.com)
- Set git configuration for test image (ccoleman@redhat.com)
- R.I.P Policy Objects (simo@redhat.com)
...
- apps: use API call instead of CLI when creating deployment configs from
  fixtures (mfojtik@redhat.com)
- apps: convert extended fixtures to groupified api (mfojtik@redhat.com)
- Generated completions (mkhan@redhat.com)
- Parallelize storage migration (mkhan@redhat.com)
- Add git to the test image for build tests (ccoleman@redhat.com)
- bump(*): kubernetes v1.10.2 (jliggitt@redhat.com)
...
- Remove old service dependencies from node system container
  (sdodson@redhat.com)
- up: use YAML instead of JSON for configs (mfojtik@redhat.com)
...
- update publishing rules for 1.10.2 (jliggitt@redhat.com)
- image: use deletion handling metanamespace key in import scheduler
  (mfojtik@redhat.com)
- remove final troublesome deps from SSCS (deads@redhat.com)
- snip links to libraries that cannot be included in library-go
  (deads@redhat.com)
- update external examples (bparees@redhat.com)
- start collapsing onto upstream env vars (deads@redhat.com)
- remove legacy resource and kind checks from the CLI (deads@redhat.com)

* Thu May 10 2018 Justin Pierce <jupierce@redhat.com> 3.10.0-0.39.0
- deflake TSB deprovision test (bparees@redhat.com)
- up: add pull policy to context and pass it properly to cluster add
  (mfojtik@redhat.com)
- add migration for templateinstance status object references
  (bparees@redhat.com)
- use webconsole operator in oc cluster up (deads@redhat.com)
- add logic for openshift webconsole operator (deads@redhat.com)
- disable recently problematic mongo/mysql replica tests (will diagnose in PRs)
  (gmontero@redhat.com)
- use a finalizer instead of GC to clean up templateinstances
  (bparees@redhat.com)
- update external examples (bparees@redhat.com)
- generated (deads@redhat.com)
- add oc adm prune role command to replace the existing reapear
  (deads@redhat.com)
- catalog: remove OS_GIT_CATALOG_VERSION (jpeeler@redhat.com)
...
- Notice multiple NetNamespace EgressIPs, but only use the first one
  (danw@redhat.com)
- Reorganize egress IP updating code, add some debug. (danw@redhat.com)
- UPSTREAM: 62874: dockershim/sandbox: clean up pod network even if SetUpPod()
  failed (dcbw@redhat.com)
- UPSTREAM: <carry>: enable critical pod support by default (decarr@redhat.com)
...
- UPSTREAM: 63339: kubelet: volume: do not create event on mount success
  (sjenning@redhat.com)
- When we conflict, back off further (ccoleman@redhat.com)
- Ignore writes on router stress test from other routes (ccoleman@redhat.com)
- Fix inconsistency in StringSource MarshalJSON (mkhan@redhat.com)
- Fix inconsistency in authorization fuzzer (mkhan@redhat.com)
...
- Pluralize SCC Kind for /oapi group map (mkhan@redhat.com)
- up: consolidate usage of out and remove tasks (mfojtik@redhat.com)
- Fix UpdateEgressNetworkPolicyRules() to return both input and ovs transaction
  errors (rpenta@redhat.com)
- move jenkins_plugin.go from image_eco to builds (gmontero@redhat.com)
- React to imagebuilder changes (ccoleman@redhat.com)
- bump(github.com/openshift/imagebuilder) (ccoleman@redhat.com)
- Allow imagestreamtag delete to retry conflicts (ccoleman@redhat.com)
...
- dind: make ovn-kubernetes units depend on OVN (dcbw@redhat.com)
- Correct a bad dependency on direct staging packages (ccoleman@redhat.com)
- Snip more oauth dependencies from the oc binary (ccoleman@redhat.com)
- Snip kubelet and oauth from oc binary (ccoleman@redhat.com)
- Creating an image stream tag is too likely to conflict (ccoleman@redhat.com)
- get build logs through the kube-apiserver, not the kubelet (deads@redhat.com)
- remove directly kubelet dependency from deploymentconfigs (deads@redhat.com)
- separate single use helpers and actual shared code (deads@redhat.com)
- switch process to produce groupified openshift api resources
  (deads@redhat.com)
- re-enable jenkins client plugin ext tests (gmontero@redhat.com)
- UPSTREAM: <carry>: disable local storage isolation feature gate
  (sjenning@redhat.com)
- ipfailover - add missing ipset/libipset to image (pcameron@redhat.com)
- UPSTREAM: 62394: Revert "git: Use VolumeHost.GetExec() to execute stuff in
  volume plugins" (jsafrane@redhat.com)
...
- UPSTREAM: 62914: kubelet: fix flake in TestUpdateExistingNodeStatusTimeout
  (sjenning@redhat.com)
- temporarily disable client plugin while we adjust to new kind/apiVersion
  (gmontero@redhat.com)
- hack: use control-plan in build local images (mfojtik@redhat.com)
- Break a loop when config file found (nakayamakenjiro@gmail.com)
- remove cruft (deads@redhat.com)
- Don't install template-service-broker broker resource unless tsb component is
  enabled (marko.luksa@gmail.com)
- up: remove --env flag (mfojtik@redhat.com)
- extended tests: use leaner image build in valuefrom.go (jwozniak@redhat.com)
- oc new-app: propagate cmdline env to pipeline/git builds
  (jwozniak@redhat.com)
- Prefix with unix protocol for docker shim socket (rpenta@redhat.com)
- Add CL and CM readme (sejug@redhat.com)
- Initial cluster mirror commit (sejug@redhat.com)
...
- generated (deads@redhat.com)
- webconsole operator (deads@redhat.com)
- Fix LDAP test to catch the correct output. (simo@redhat.com)
- Add custom JSON marshaling implementations, metrics broken into standalone
  lib (sejug@redhat.com)
...
- Remove decode code related to PSP Subject Reviews (mkhan@redhat.com)
- Remove duplicate WantsAuthorizer interface (mkhan@redhat.com)
- Fix a panic in the networking tests (ccoleman@redhat.com)
- Use image template instead of allowing arbitary prefix (ccoleman@redhat.com)
- Rename origin references to origin-control-plane (ccoleman@redhat.com)
- Remove node.service from RPM content (ccoleman@redhat.com)
- Rename base origin images and remove unused content (ccoleman@redhat.com)
- Remove federation from origin (ccoleman@redhat.com)
- Purge old and unused examples (ccoleman@redhat.com)
- switch the sdn to use external types (deads@redhat.com)
- stop shimming the upstream factory inputs (deads@redhat.com)
- switch glide to https (jliggitt@redhat.com)
- Add a smoke test suite (ccoleman@redhat.com)
- up: copy configs into remote host (mfojtik@redhat.com)
- Improve patching ovs flow rules in UpdateEgressNetworkPolicyRules
  (rpenta@redhat.com)
- Switch conformance tests to release-1.10 (ccoleman@redhat.com)
- remove layer for creating event broadcaster (deads@redhat.com)
- UPSTREAM: <carry>: rewrite unstructured objects on the CLI to avoid oapi
  (deads@redhat.com)
- UPSTREAM: 62461: allow higher burst for discovery (deads@redhat.com)
- update oc commands to build their own clients (deads@redhat.com)
- remove legacy auth cli actions that no longer work on current servers
  (deads@redhat.com)
- UPSTREAM: 62572: Prevent virtual infinite loop in volume controller
  (joesmith@redhat.com)
- inject new parameter for image resolution into kubectl set image
  (deads@redhat.com)
- UPSTREAM: <carry>: inject new parameter for image resolution into kubectl set
  image (deads@redhat.com)
- Add .NET Core templates (tom.deseyn@gmail.com)
- stop using factory.OpenshiftClientConfig (deads@redhat.com)
- UPSTREAM: <carry>: add RawConfig to factory for commands modifying raw
  kubeconfig files (deads@redhat.com)
- UPSTREAM: 62584: Make x-kubernetes-print-column print handling opt-in
  (jliggitt@redhat.com)
- up: write static pod definitions on --write-config (mfojtik@redhat.com)
- Mount device manager socket dir path (vichoudh@redhat.com)
- tag a basic test as a smoke test (bparees@redhat.com)
...
- Tests for jenkins-sync-plugin pull request to delete jenkins job runs when
  the associated build is deleted. (cdaley@redhat.com)
- Enable router metrics preservation test (ccoleman@redhat.com)
...
- remove unnecessary struct layering (deads@redhat.com)
...
- Grab the sa token per namespace. (mrsiano@gmail.com)
