...
func (t *UpgradeTest) Test(f *framework.Framework, done <-chan struct{}, upgrade upgrades.UpgradeType) {
	...
	newBroadcaster := record.NewBroadcaster()
	newBroadcaster.StartRecordingToSink(&clientcorev1.EventSinkImpl{Interface: client.CoreV1().Events("")})
	r := newBroadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "openshift.io/upgrade-test-service"})
...
func startEndpointMonitoring(ctx context.Context, m *monitor.Monitor, svc *v1.Service, r record.EventRecorder) error {
	tcpIngressIP := framework.GetIngressPoint(&svc.Status.LoadBalancer.Ingress[0])
	...
	m.AddSampler(
		monitor.StartSampling(ctx, m, time.Second, func(previous bool) (condition *monitor.Condition, next bool) {
			...
			case err != nil && previous:
				...
				r.Eventf(&v1.ObjectReference{Kind: "Service", Namespace: "kube-system", Name: "service-upgrade-test"}, v1.EventTypeWarning, "Unreachable", "detected on reused connections")
				condition = &monitor.Condition{
					Level:   monitor.Error,
					Locator: locateService(svc),
	...
	m.AddSampler(
		monitor.StartSampling(ctx, m, time.Second, func(previous bool) (condition *monitor.Condition, next bool) {
			...
			case err != nil && previous:
				...
				r.Eventf(&v1.ObjectReference{Kind: "Service", Namespace: "kube-system", Name: "service-upgrade-test"}, v1.EventTypeWarning, "Unreachable", "detected on new connections")
				condition = &monitor.Condition{
					Level:   monitor.Error,
					Locator: locateService(svc),
