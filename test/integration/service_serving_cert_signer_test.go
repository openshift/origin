...
func TestServiceServingCertSigner(t *testing.T) {
	...
	secretWatcher1, err := clusterAdminKubeClientset.Core().Secrets(ns).Watch(kapi.ListOptions{ResourceVersion: actualService.ResourceVersion})
	if err != nil {
		t.Fatal(err)
	}
	...
	secretWatcher2, err := clusterAdminKubeClientset.Core().Secrets(ns).Watch(kapi.ListOptions{ResourceVersion: actualSecondSecret.ResourceVersion})
	if err != nil {
		t.Fatal(err)
	}
	...
	secretWatcher3, err := clusterAdminKubeClientset.Core().Secrets(ns).Watch(kapi.ListOptions{ResourceVersion: actualFourthSecret.ResourceVersion})
	if err != nil {
		t.Fatal(err)
	}
