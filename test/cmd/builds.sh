...
os::cmd::expect_success 'oc new-build centos/ruby-27-centos7 https://github.com/openshift/ruby-hello-world.git'
...
os::cmd::try_until_text 'oc get is ruby-27-centos7' 'latest'
os::cmd::expect_failure_and_text 'oc new-build ruby-27-centos7~https://github.com/sclorg/ruby-ex ruby-27-centos7~https://github.com/sclorg/ruby-ex --to invalid/argument' 'error: only one component with source can be used when specifying an output image reference'
...
os::cmd::expect_success_and_text "oc describe build ${started}" 'centos/ruby-27-centos7$'
frombuild="$(basename $(oc start-build -o=name --from-build="${started}"))"
os::cmd::expect_success_and_text "oc describe build ${frombuild}" 'centos/ruby-27-centos7$'
