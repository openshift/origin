...
# verify login, project, get and config view using deprecated --config flag
old_kubeconfig="${ARTIFACT_DIR}/old.kubeconfig"
os::cmd::expect_success "oc login --server=${KUBERNETES_MASTER} --certificate-authority='${MASTER_CONFIG_DIR}/ca.crt' --config='${old_kubeconfig}' -u oldconfig -p anything"
os::cmd::expect_success "oc new-project project-old --config='${old_kubeconfig}'"
os::cmd::expect_success "oc get services --config='${old_kubeconfig}'"
os::cmd::expect_success_and_text 'oc config view --config=${old_kubeconfig}' "current-context.+project-old/${API_HOST}:${API_PORT}/oldconfig"

# verify login, project, get and config view using the recommended --kubeconfig flag
new_kubeconfig="${ARTIFACT_DIR}/new.kubeconfig"
os::cmd::expect_success "oc login --server=${KUBERNETES_MASTER} --certificate-authority='${MASTER_CONFIG_DIR}/ca.crt' --kubeconfig='${new_kubeconfig}' -u newconfig -p anything"
os::cmd::expect_success "oc new-project project-new --kubeconfig='${new_kubeconfig}'"
os::cmd::expect_success "oc get services --kubeconfig='${new_kubeconfig}'"
os::cmd::expect_success_and_text 'oc config view --kubeconfig=${new_kubeconfig}' "current-context.+project-new/${API_HOST}:${API_PORT}/newconfig"
