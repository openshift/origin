...
var _ = g.Describe("[Feature:Performance][Serial][Slow] Mirror cluster", func() {
	...
	g.It("it should read the cluster apps", func() {
		...
		for _, ns := range nsList.Items {
			...
			dcs, err := oc.AdminInternalAppsClient().Apps().DeploymentConfigs(ns.Name).List(metav1.ListOptions{})
			if err != nil {
				e2e.Failf("Error listing DeploymentConfigs: %v\n", err)
			}

			for _, dc := range dcs.Items {
				dc, err := oc.AdminInternalAppsClient().Apps().DeploymentConfigs(ns.Name).Get(dc.Name, metav1.GetOptions{})
				if err != nil {
