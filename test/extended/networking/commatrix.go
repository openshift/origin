...
// excludeNodeRoleStaticEntries excludes from comDetails, static entries from staticEntriesMatrix with the given nodeRole
// The function returns filtered ComDetails without the excluded entries.
func excludeNodeRoleStaticEntries(comDetails []types.ComDetails, staticEntriesMatrix *types.ComMatrix, nodeRole string) []types.ComDetails {
	filteredComDetails := []types.ComDetails{}
	for _, cd := range comDetails {
		if cd.NodeRole != nodeRole || !staticEntriesMatrix.Contains(cd) {
...
func filterDocumentedCommatrixByDeploymentAndInfraTypes(docComDetailsList []types.ComDetails, isSNO, isBM bool) *types.ComMatrix {
	if isSNO {
		...
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: docComDetailsList}, workerNodeRole)
		// Exclude static entries of standard deployment type.
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: types.StandardStaticEntries}, masterNodeRole)
	} else {
		// Exclude specific master entries (see StandardExcludedMasterComDetails var description)
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: StandardExcludedMasterComDetails}, masterNodeRole)
	}
	...
	if isBM {
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: types.CloudStaticEntriesWorker}, workerNodeRole)
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: types.CloudStaticEntriesMaster}, masterNodeRole)
	} else {
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: types.BaremetalStaticEntriesWorker}, workerNodeRole)
		docComDetailsList = excludeNodeRoleStaticEntries(docComDetailsList, &types.ComMatrix{Matrix: types.BaremetalStaticEntriesMaster}, masterNodeRole)
	}
