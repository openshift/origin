...
var excludedPorts = []int{10250, 6385}
...
var _ = g.Describe("[sig-network][Feature:commatrix][apigroup:config.openshift.io][Serial]", func() {
	g.It("Static entries should not overlap with those in the EndpointSlice; any shared entries must be removed", func() {
		...
		if err != nil {
			g.Skip(fmt.Sprintf("Skipping test: failed to get platform type: %v", err))
		}
		...
		staticEntryNeedToRemove := removePortsFromMatrix(sharedEntries)
...
// removePortsFromMatrix removes ports from matrix that just on the origin ci have eps
func removePortsFromMatrix(mat *types.ComMatrix) *types.ComMatrix {
	...
	for _, cd := range mat.Matrix {
		if _, found := exclude[cd.Port]; found {
			continue
		}
		res = append(res, cd)
	}

	return &types.ComMatrix{Matrix: res}
}
