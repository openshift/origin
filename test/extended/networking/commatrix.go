...
var _ = g.Describe("[sig-network][Feature:commatrix][Serial]", func() {
	g.It("should validate the communication matrix ports match the node's listening ports", func() {
		...
		listeningCheck, err := listeningsockets.NewCheck(cs, utilsHelpers, "", "", types.FormatCSV, artifactsDir, env, deployment)
		o.Expect(err).ToNot(o.HaveOccurred())
		...
		diff, err := matrix.GenerateMatrixDiff(ssFilteredMat)
		o.Expect(err).ToNot(o.HaveOccurred())

		err = utilsHelpers.WriteFile(filepath.Join(artifactsDir, "matrix-diff-ss"), []byte(diff))
		o.Expect(err).ToNot(o.HaveOccurred())

		notUsed, missing := validateMatrix(diff)
		if notUsed != "" {
...
// Review the diff to validate the ports in the generated communication matrix match the node's listening ports
func validateMatrix(diff string) (notUsed, missing string) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "+") {
			details := strings.TrimPrefix(line, "+")
			notUsed += details + "\n"
		}
