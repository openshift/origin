...
import (
	...
	ssutil "github.com/openshift/origin/test/extended/util/ss"
)
...
var _ = g.Describe("[sig-network][Feature:commatrix][Serial]", func() {
	...
	g.It("should cover all ports that the nodes are actually listening on", func() {
		...
		err = os.WriteFile(filepath.Join(artifactsDir, "communication-matrix"), []byte(comMatrix.String()), 0644)
		o.Expect(err).ToNot(o.HaveOccurred())

		err = os.WriteFile(filepath.Join(artifactsDir, "ss-output"), []byte(expectedComMat.String()), 0644)
		o.Expect(err).ToNot(o.HaveOccurred())
