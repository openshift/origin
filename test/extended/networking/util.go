...
func allocateIPs(oc *exutil.CLI, namespace, name, key, t4, t6 string, n4, n6, min, max int) ([]string, []string, error) {
	...
	err = retry.OnError(
		...
		func() error {
			...
			if err != nil {
				return fmt.Errorf("could not allocate %d IPv4 from pool %s with existin allocations %v: %w", n4, key, existing, err)
			}
			ipsv6, err = allocate(t6, n6, min, max, existingSet)
			if err != nil {
				return fmt.Errorf("could not allocate %d IPv6 from pool %s with existin allocations %v: %w", n6, key, existing, err)
			}
