	...
	InOVNKubernetesContext(func() {
		...
		g.Context("[PodNetwork] Advertising a cluster user defined network [apigroup:user.openshift.io][apigroup:security.openshift.io]", func() {
			...
			g.BeforeEach(func() {
				...
				raManifest := newRouteAdvertisementsManifest(cudnName, false)
...
func checkExternalResponse(oc *exutil.CLI, proberPod *corev1.Pod, podIP, ExternalIP string, serverPort int, packetSnifferDaemonSet *v1.DaemonSet, targetProtocol string) {
	...
	g.By("Making sure that external server IP presents in the response")
	var resp response
	err = json.Unmarshal([]byte(output), &resp)
	o.Expect(err).NotTo(o.HaveOccurred())
	o.Expect(len(resp.Responses)).To(o.BeNumerically(">", 0))
	o.Expect(resp.Responses[0]).To(o.ContainSubstring(ExternalIP))

	g.By("Making sure that external host IP presents in the sniffer packet log")
	o.Eventually(func() int {
		found, _ := scanPacketSnifferDaemonSetPodLogs(oc, packetSnifferDaemonSet, targetProtocol, ExternalIP)
		return len(found)
	}, timeOut, interval).Should(o.BeNumerically(">", 0))
}
