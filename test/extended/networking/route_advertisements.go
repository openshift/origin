...
var _ = g.Describe("[sig-network][OCPFeatureGate:RouteAdvertisements][Feature:RouteAdvertisements][apigroup:operator.openshift.io]", func() {
	...
	InOVNKubernetesContext(func() {
		...
		g.Describe("[PodNetwork] Advertising a cluster user defined network [apigroup:user.openshift.io][apigroup:security.openshift.io]", func() {
			...
			afterEach := func() {
				g.GinkgoHelper()
				o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "deploy", deployName)).To(o.Succeed())
				o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "pod", "--all")).To(o.Succeed())
				o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin().WithoutNamespace(), "delete", "ra", testCUDNName)).To(o.Succeed())
				o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin().WithoutNamespace(), "delete", "clusteruserdefinednetwork", testCUDNName)).To(o.Succeed())
				o.Expect(deallocateSubnets(oc, userDefinedNetworkIPv4Subnet, userDefinedNetworkIPv6Subnet)).To(o.Succeed())
				cleanup()
			}
			...
			g.Describe("Over a VRF-Lite configuration", func() {
				...
				g.AfterEach(func() {
					o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "nncp", "extranet")).To(o.Succeed())
					afterEach()
				})
		...
		g.Describe("[EgressIP] Advertising EgressIP [apigroup:user.openshift.io][apigroup:security.openshift.io]", func() {
			...
			g.AfterEach(func() {
				o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "egressip", egressIPObjectName, "--cascade=foreground", "--ignore-not-found=true")).To(o.Succeed())
				o.Expect(deallocateEgressIPSets(oc, egressIPSet, newEgressIPSet)).To(o.Succeed())
			})
			...
			g.DescribeTableSubtree("For cluster user defined networks",
				func(udnTopology string) {
					...
					g.AfterEach(func() {
						o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "deploy", deployName)).To(o.Succeed())
						o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "ra", testCUDNName)).To(o.Succeed())
						o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "pod", "--all")).To(o.Succeed())
						o.Expect(runOcWithRetryIgnoreOutput(oc.AsAdmin(), "delete", "clusteruserdefinednetwork", testCUDNName)).To(o.Succeed())
						o.Expect(deallocateSubnets(oc, userDefinedNetworkIPv4Subnet, userDefinedNetworkIPv6Subnet)).To(o.Succeed())
						cleanup()
					})
