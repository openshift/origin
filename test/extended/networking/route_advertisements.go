	...
	InOVNKubernetesContext(func() {
		...
		verifyUdnRaFunc := func(udnTopology string) {
			...
			g.BeforeEach(func() {
				...
				o.Expect(err).NotTo(o.HaveOccurred())
				if network.Spec.DefaultNetwork.OVNKubernetesConfig.GatewayConfig != nil && network.Spec.DefaultNetwork.OVNKubernetesConfig.GatewayConfig.RoutingViaHost {
					skipper.Skipf("Skipping test for local gateway mode")
				}
