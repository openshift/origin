...
var _ = Describe("[sig-network][OCPFeatureGate:NetworkSegmentation][Feature:UserDefinedPrimaryNetworks]", func() {
	...
	InOVNKubernetesContext(func() {
		...
		It("when primary network exist, UserDefinedNetwork status should report not-ready", func() {
			...
			primaryNetNad := generateNAD(oc, newNetworkAttachmentConfig(networkAttachmentConfigParams{
				role:        "primary",
				topology:    "layer3",
				name:        primaryNadName,
...
func generateUserDefinedNetworkManifest(oc *exutil.CLI, params *networkAttachmentConfigParams) string {
	...
	if params.role == "primary" {
		joinSubnets = "joinSubnets: " + generateJoinSubnetsYaml(oc)
	}
	return `
...
func generateNAD(oc *exutil.CLI, config networkAttachmentConfig) *nadapi.NetworkAttachmentDefinition {
	joinSubnets := ""
	if config.role == "primary" {
		joinSubnet := correctCIDRFamily(oc, userDefinedNetworkIPv4JoinSubnet, userDefinedNetworkIPv6JoinSubnet)
		joinSubnets = fmt.Sprintf(`,"joinSubnets": %q`, joinSubnet)
	}
	nadSpec := fmt.Sprintf(
		`
{
