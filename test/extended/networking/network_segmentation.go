			...
			func(createNetworkFn func(c *networkAttachmentConfigParams) error) {
				...
				DescribeTable(
					...
					func(
						...
						if restartCount > 0 {
							fmt.Printf("SURYA %v", restartCount)
							// If pod has restarted, check events to understand why
							stdout, err := e2ekubectl.RunKubectl(udnPod.Namespace, "logs", udnPod.Name, "-c",
								"agnhost-container",
							)
							fmt.Printf("SURYA %v/ %v", stdout, err)
							stdout, err = e2ekubectl.RunKubectl(udnPod.Namespace, "logs", udnPod.Name, "-c",
								"agnhost-container", "--previous",
							)
							fmt.Printf("SURYA %v/ %v", stdout, err)
							stdout, err = e2ekubectl.RunKubectl(udnPod.Namespace, "get", "pods", udnPod.Name, "-oyaml")
							fmt.Printf("SURYA %v/ %v", stdout, err)
