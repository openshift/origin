...
var _ = g.Describe("[sig-network][OCPFeatureGate:NetworkObservabilityInstall][Feature:NetObserv]", func() {
	...
	g.It("Network Observability should not be auto-deployed on single node clusters", func(ctx context.Context) {
		...
		g.By("checking that no Network Observability pods are running on SNO")
		pods, err := oc.AdminKubeClient().CoreV1().Pods(netobservNamespace).List(ctx, metav1.ListOptions{})
		if err != nil && !apierrors.IsNotFound(err) {
			o.Expect(err).NotTo(o.HaveOccurred(), "failed to list pods in %s", netobservNamespace)
		}
		o.Expect(pods.Items).To(o.BeEmpty(),
			"Network Observability should not have any pods running on single node clusters")
	})
	...
	g.Context("health checks", func() {
		g.BeforeEach(func(ctx context.Context) {
			...
			if isMicroShift {
				g.Skip("FlowCollector API group should not be available on MicroShift clusters")
			}
		})

		g.It("Network Observability should have FlowCollector CR in Ready state", func(ctx context.Context) {
		...
		g.It("Network Observability should have operator pod running", func(ctx context.Context) {
		...
		g.It("Network Observability should have FLP pods running", func(ctx context.Context) {
		...
		g.It("Network Observability should have eBPF agent DaemonSet fully ready", func(ctx context.Context) {
		...
		g.It("Network Observability should have console plugin healthy [apigroup:console.openshift.io]", func(ctx context.Context) {
			...
			o.Expect(err).NotTo(o.HaveOccurred(), "failed to list console plugin pods in %s", netobservNamespace)
		...
		g.It("Network Observability should have static plugin pods running", func(ctx context.Context) {
			staticPluginPods, err := oc.AdminKubeClient().CoreV1().Pods(netobservOperatorNamespace).List(ctx, metav1.ListOptions{
				LabelSelector: "app=netobserv-plugin-static",
			})
			o.Expect(err).NotTo(o.HaveOccurred(), "failed to list static plugin pods in %s", netobservOperatorNamespace)
			o.Expect(staticPluginPods.Items).NotTo(o.BeEmpty(), "expected static plugin pods in %s", netobservOperatorNamespace)

			for _, pod := range staticPluginPods.Items {
				o.Expect(string(pod.Status.Phase)).To(o.Equal("Running"),
					"static plugin pod %s should be Running, got %s", pod.Name, pod.Status.Phase)
			}
		})

		g.It("Network Observability should not have excessive errors in operator logs", func(ctx context.Context) {
			...
			o.Expect(errorCount).To(o.BeNumerically("<=", 5),
				"found %d error-level log entries in the last 50 operator log lines (threshold: 5):\n%s",
				errorCount, strings.Join(errorLines, "\n"))
		})

		g.It("Network Observability should have Prometheus scraping non-zero NetObserv metrics", func(ctx context.Context) {
			...
			g.By("querying Prometheus for non-zero flow metrics")
...
func uninstallNetObserv(ctx context.Context, oc *exutil.CLI, client kubernetes.Interface) {
	...
	g.By("waiting for Network Observability pods to terminate")
	o.Eventually(func() bool {
		...
		"Network Observability pods should terminate after uninstall")
...
var _ = g.Describe("[sig-network][OCPFeatureGate:NetworkObservabilityInstall][Feature:NetObserv][Serial][Disruptive]", g.Ordered, func() {
	...
	g.It("Network Observability should have NetObserv installed and healthy with InstallAndEnable policy", func(ctx context.Context) {
