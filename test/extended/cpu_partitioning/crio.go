	...
	g.It("should have correct cpuset and cpushare set in crio containers", func() {
		...
		for _, node := range nodes.Items {
			...
			for _, containerInfo := range crioData {
				failLocation := fmt.Sprintf("node: %s pod: %s/%s", node.Name, containerInfo.PodNamespace, containerInfo.PodName)
				...
				if expectedCPUSet.IsEmpty() {
					o.Expect(parsedHost.Equals(parsedFullNodeCPUSet)).To(o.BeTrue(), fmt.Sprintf("%s : expected host CPUSet: %s got: %s", failLocation, parsedHost, parsedFullNodeCPUSet))
				} else {
					o.Expect(parsedContainer.Equals(parsedHost)).To(o.BeTrue(), fmt.Sprintf("%s : expected: %s got: %s", failLocation, parsedContainer, parsedHost))
				}
