...
func createTestingNS(baseName string, c kclientset.Interface, labels map[string]string) (*kapiv1.Namespace, error) {
	...
	if isKubeE2ENamespace {
		...
		addRoleToE2EServiceAccounts(rbacClient, []kapiv1.Namespace{*ns}, "system:openshift:scc:privileged")
		// add the "anyuid" scc to ensure pods that don't specify a
		// uid don't get forced into a range (mimics upstream
		// behavior)
		addRoleToE2EServiceAccounts(rbacClient, []kapiv1.Namespace{*ns}, "system:openshift:scc:anyuid")
		// add the "hostmount-anyuid" scc to ensure pods using hostPath
		// can execute tests
		addRoleToE2EServiceAccounts(rbacClient, []kapiv1.Namespace{*ns}, "system:openshift:scc:hostmount-anyuid")
