...
func (t *AvailableTest) Test(f *framework.Framework, done <-chan struct{}, upgrade upgrades.UpgradeType) {
	...
	for _, interval := range m.Events(time.Time{}, time.Time{}) {
		framework.Logf("Event: %s", interval)
	...
	if duration > 30*time.Second {
		framework.Failf("API was unreachable during upgrade for at least %s", duration.Truncate(time.Second))
	} else if duration > 0 {
		framework.Logf("API was unreachable during upgrade for at least %s", duration.Truncate(time.Second))
	}
}
