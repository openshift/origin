...
func ProcessLogURLAnnotations(oc *exutil.CLI, t *exutil.BuildResult, includeBlueOcean bool) error {
	...
	_, err := url.Parse(t.Build.Annotations[buildapi.BuildJenkinsLogURLAnnotation])
	if err != nil {
	...
	if len(t.Build.Annotations[buildapi.BuildJenkinsConsoleLogURLAnnotation]) == 0 {
		return fmt.Errorf("build %s does not contain a Jenkins Console URL annotation", t.BuildName)
	}
	_, err = url.Parse(t.Build.Annotations[buildapi.BuildJenkinsConsoleLogURLAnnotation])
	if err != nil {
		return fmt.Errorf("cannot parse jenkins console log URL (%s): %v", t.Build.Annotations[buildapi.BuildJenkinsConsoleLogURLAnnotation], err)
	}
	if includeBlueOcean {
		if len(t.Build.Annotations[buildapi.BuildJenkinsBlueOceanLogURLAnnotation]) == 0 {
			return fmt.Errorf("build %s does not contain a Jenkins BlueOcean URL annotation", t.BuildName)
		}
		_, err = url.Parse(t.Build.Annotations[buildapi.BuildJenkinsBlueOceanLogURLAnnotation])
		if err != nil {
			return fmt.Errorf("cannot parse jenkins log blueocean URL (%s): %v", t.Build.Annotations[buildapi.BuildJenkinsBlueOceanLogURLAnnotation], err)
		}
	}
	return nil
}
