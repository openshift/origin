...
func (t *AdminAckTest) Test(ctx context.Context) {
	...
	current, err := exutil.GetCurrentVersion(postUpdateCtx, t.Config)
	if err != nil {
		framework.Failf("Error getting current version, err=%v", err)
	}
	if current != "" && !exercisedVersions.Has(current) {
...
func adminAckRequiredWithMessage(ctx context.Context, config *restclient.Config, message string) bool {
	...
	if err != nil {
		framework.Failf("Error getting cluster version, err=%v", err)
	}
