...
func init() {
	allowedImages = map[string]k8simage.ImageID{
		...
		// has been replaced by 2.39 in upstream k8s, but as this image still exists
		// it seems reasonable to continue to allow it.  Tests should be updated to
		// use 2.39 in the future though.
		"k8s.gcr.io/e2e-test-images/agnhost:2.36": -1,

		// has been replaced by registry.k8s.io/e2e-test-images/nginx:1.15-2 in upstream k8s, but as
		// this image still exists it seems reasonable to continue to allow it.  Tests should
		// be updated to use the new location in the future though
		"k8s.gcr.io/e2e-test-images/nginx:1.15-2": -1,
		...
		// TODO: why do we need to list upstream images at all?  shouldn't it always be valid
		// to use an image that upstream tests use?  Afterall, the upstream tests already have
