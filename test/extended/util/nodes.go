...
// hostNetworkMode represents the networking mode for disruption pods
type hostNetworkMode int

const (
	// hostNetwork enables host networking for the disruption pod
	hostNetwork hostNetworkMode = iota
	// podNetwork disables host networking for the disruption pod
	podNetwork
)
...
func TriggerNodeRebootGraceful(kubeClient kubernetes.Interface, nodeName string) error {
	...
	return createNodeDisruptionPod(kubeClient, nodeName, 0, podNetwork, command)
}
...
func TriggerNetworkDisruption(kubeClient kubernetes.Interface, target, peer *corev1.Node, disruptionDuration time.Duration) (string, error) {
	...
	return disruptionCmd, createNodeDisruptionPod(kubeClient, target.Name, 0, hostNetwork, disruptionCmd)
}
...
func createNodeDisruptionPod(kubeClient kubernetes.Interface, nodeName string, attempt int, networkMode hostNetworkMode, command string) error {
	isTrue := true
	zero := int64(0)
	...
	_, err := kubeClient.CoreV1().Pods("kube-system").Create(context.Background(), &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			HostNetwork:   networkMode == hostNetwork,
			HostPID:       true,
	...
	if errors.IsAlreadyExists(err) {
		return createNodeDisruptionPod(kubeClient, nodeName, attempt+1, hostNetwork, command)
	}
	return err
}
