...
// ── Parallel read-only tests ────────────────────────────────────────────
// These tests only read cluster state (ObservedConfig, ConfigMaps,
// Deployment env vars, wire-level TLS) and can safely run in parallel.
var _ = g.Describe("[sig-api-machinery][Feature:TLSObservedConfig]", func() {
...
var _ = g.Describe("[sig-api-machinery][Feature:TLSObservedConfig][Serial]", func() {
	defer g.GinkgoRecover()
	...
	for _, target := range targets {
		...
		g.It(fmt.Sprintf("should restore inject-tls annotation after deletion - %s [Disruptive]", target.namespace), func() {
			if isHyperShiftCluster && target.controlPlane {
				g.Skip(fmt.Sprintf("Skipping control-plane target %s on HyperShift (runs on management cluster)", target.namespace))
			}
		...
		g.It(fmt.Sprintf("should restore inject-tls annotation when set to false - %s [Disruptive]", target.namespace), func() {
			if isHyperShiftCluster && target.controlPlane {
				g.Skip(fmt.Sprintf("Skipping control-plane target %s on HyperShift (runs on management cluster)", target.namespace))
			}
		...
		g.It(fmt.Sprintf("should restore servingInfo after removal - %s [Disruptive]", target.namespace), func() {
			if isHyperShiftCluster && target.controlPlane {
				g.Skip(fmt.Sprintf("Skipping control-plane target %s on HyperShift (runs on management cluster)", target.namespace))
			}
		...
		g.It(fmt.Sprintf("should restore servingInfo after modification - %s [Disruptive]", target.namespace), func() {
			if isHyperShiftCluster && target.controlPlane {
				g.Skip(fmt.Sprintf("Skipping control-plane target %s on HyperShift (runs on management cluster)", target.namespace))
			}
	...
	g.It("should enforce Modern TLS profile after cluster-wide config change [Slow] [Disruptive] [Timeout:60m]", func() {
		...
		if isHyperShiftCluster {
			...
			e2e.Logf("PASS: Phase 1 - Modern TLS profile verified on HyperShift")

			// Phase 2: Intermediate
			g.By("patching HostedCluster back to Intermediate")
			setTLSProfileOnHyperShift(mgmtOC, hostedClusterName, hostedClusterNS, resetPatch)
			waitForHCPPods(mgmtOC, hcpNamespace, 8*time.Minute)
			waitForGuestOperatorsAfterTLSChange(oc, configChangeCtx, "Intermediate")
			verifyObservedConfigForTargets(oc, configChangeCtx, "VersionTLS12", "Intermediate", guestTargets)
			verifyConfigMapsForTargets(oc, configChangeCtx, "VersionTLS12", "Intermediate", guestTargets)
			verifyHCPConfigMaps(mgmtOC, hcpNamespace, "VersionTLS12", "Intermediate")

			// Phase 3: Modern again
			g.By("patching HostedCluster to Modern again")
			setTLSProfileOnHyperShift(mgmtOC, hostedClusterName, hostedClusterNS, modernPatch)
			waitForHCPPods(mgmtOC, hcpNamespace, 8*time.Minute)
			waitForGuestOperatorsAfterTLSChange(oc, configChangeCtx, "Modern (2nd)")
			verifyObservedConfigForTargets(oc, configChangeCtx, "VersionTLS13", "Modern (2nd)", guestTargets)
			verifyConfigMapsForTargets(oc, configChangeCtx, "VersionTLS13", "Modern (2nd)", guestTargets)
			verifyHCPConfigMaps(mgmtOC, hcpNamespace, "VersionTLS13", "Modern (2nd)")

			// Phase 4: Final Intermediate
			g.By("patching HostedCluster back to Intermediate (final)")
			setTLSProfileOnHyperShift(mgmtOC, hostedClusterName, hostedClusterNS, resetPatch)
			waitForHCPPods(mgmtOC, hcpNamespace, 8*time.Minute)
			waitForGuestOperatorsAfterTLSChange(oc, configChangeCtx, "Intermediate (final)")
			verifyObservedConfigForTargets(oc, configChangeCtx, "VersionTLS12", "Intermediate (final)", guestTargets)
			verifyConfigMapsForTargets(oc, configChangeCtx, "VersionTLS12", "Intermediate (final)", guestTargets)
			verifyHCPConfigMaps(mgmtOC, hcpNamespace, "VersionTLS12", "Intermediate (final)")

			e2e.Logf("PASS: Full TLS propagation cycle verified on HyperShift (Modern → Intermediate → Modern → Intermediate)")
			return
		}
