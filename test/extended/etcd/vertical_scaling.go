...
var _ = g.Describe("[sig-etcd][Feature:EtcdVerticalScaling][Suite:openshift/etcd/scaling][Serial] etcd", func() {
	...
	g.It("is able to vertically scale up and down by replacing a single node in the cluster [Timeout:60m][apigroup:machine.openshift.io]", func() {
		if cpmsActive {
	...
	g.It("is able to vertically scale down when a member is unhealthy [apigroup:machine.openshift.io]", func() {
		...
		err = errors.Wrapf(err, "failed to remove etcd static pod manifest from the node %s", etcdTargetNode.Name)
		o.Expect(err).ToNot(o.HaveOccurred())

		err = e2epod.WaitForPodNotFoundInNamespace(ctx, kubeClient, etcdTargetPod.Name, etcdNamespace, 5*time.Minute)
		err = errors.Wrapf(err, "timed-out waiting for etcd static pod %s on the node %s, to fully terminate", etcdTargetPod.Name, etcdTargetNode.Name)
		o.Expect(err).ToNot(o.HaveOccurred())
