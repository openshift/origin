...
// func ToBeNamed(ctx context.Context, machineClient machinev1beta1client.MachineInterface) error {
// }
...
func StopKubelet(adminKubeClient kubernetes.Interface, node corev1.Node) error {
	...
	pod := applycorev1.Pod("kubelet-stopper-1", "openshift-etcd").WithSpec(podSpec)
	...
	err = wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 3*time.Minute, true, func(ctx context.Context) (done bool, err error) {
		...
		if p.Status.Phase == corev1.PodFailed {
			return true, fmt.Errorf("pod failed with status %v", pod.Status)
		}

		return e2epod.FindPodConditionByType(&p.Status, corev1.PodReady).Status == corev1.ConditionFalse, nil
	})
	return err
}
