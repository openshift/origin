...
`
		testDockerfile2 = `
FROM image-registry.openshift-image-registry.svc:5000/openshift/ruby:2.6
USER 1001
`
		testDockerfile3 = `
FROM scratch
	...
	g.Context("", func() {
		...
		g.Describe("being created from new-build", func() {
			...
			g.It("should create a image via new-build and infer the origin tag", func() {
				...
				_, err = oc.ImageClient().ImageV1().ImageStreamTags(oc.Namespace()).Get(context.Background(), "ruby:2.6", metav1.GetOptions{})
				o.Expect(err).NotTo(o.HaveOccurred())
			})
