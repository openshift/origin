		...
		g.Context("start-build test context", func() {
			...
			g.Describe("binary builds", func() {
				g.It("should be able to run a build that is implicitly pulling from the internal registry [apigroup:build.openshift.io]", func() {
					...
					err := oc.Run("new-build").Args("--binary", "--strategy=docker", "--name=docker", "--env", "BUILD_LOGLEVEL=2").Execute()
					o.Expect(err).NotTo(o.HaveOccurred())
					br, err := exutil.StartBuildAndWait(oc, "docker", fmt.Sprintf("--from-dir=%s", exampleBuild), "--build-loglevel=2")
					br.AssertSuccess()
					g.By("verifying that the build ran in the node's user namespace")
					logs, err := br.Logs()
					o.Expect(err).NotTo(o.HaveOccurred())
					o.Expect(logs).To(o.MatchRegexp(buildInDefaultUserNSRegexp))
				})

				g.It("should be able to run an unprivileged build that is implicitly pulling from the internal registry [apigroup:build.openshift.io]", func() {
					g.By("creating a build")
					err := oc.Run("new-build").Args("--binary", "--strategy=docker", "--name=docker-unprivileged", "--env", "BUILD_LOGLEVEL=2", "--env", buildInUserNSEnvVar).Execute()
					o.Expect(err).NotTo(o.HaveOccurred())
					br, err := exutil.StartBuildAndWait(oc, "docker-unprivileged", fmt.Sprintf("--from-dir=%s", exampleBuild))
					br.AssertSuccess()
					g.By("verifying that the build ran in a user namespace")
					logs, err := br.Logs()
					o.Expect(err).NotTo(o.HaveOccurred())
			...
			g.Describe("pulling from an external authenticated registry", func() {
				...
				g.It("should be able to use a pull secret in a build [apigroup:build.openshift.io]", func() {
					...
					o.Expect(err).NotTo(o.HaveOccurred())
					br.AssertSuccess()
					g.By("verifying that the build with a pull secret ran in the node's user namespace")
					logs, err := br.Logs()
					o.Expect(err).NotTo(o.HaveOccurred())
					o.Expect(logs).To(o.MatchRegexp(buildInDefaultUserNSRegexp))
				})

				g.It("should be able to use a pull secret in an unprivileged build [apigroup:build.openshift.io]", func() {
					g.By("creating build config")
					err := oc.Run("create").Args("-f", pullSecretBuild).Execute()
					o.Expect(err).NotTo(o.HaveOccurred())
					g.By("starting build with a pull secret")
					br, err := exutil.StartBuildAndWait(oc, "pullsecret-nodejs", "--build-loglevel=6", "--env", buildInUserNSEnvVar)
					o.Expect(err).NotTo(o.HaveOccurred())
					br.AssertSuccess()
					g.By("verifying that the build with a pull secret ran in a user namespace")
					logs, err := br.Logs()
					o.Expect(err).NotTo(o.HaveOccurred())
					o.Expect(logs).To(o.MatchRegexp(buildInUserNSRegexp))
				})
				...
				g.It("should be able to use a pull secret linked to the builder service account in an unprivileged build [apigroup:build.openshift.io]", func() {
					g.By("linking pull secret with the builder service account")
					err := oc.Run("secrets").Args("link", "builder", "local-ps").Execute()
					o.Expect(err).NotTo(o.HaveOccurred())
					g.By("creating build config")
					err = oc.Run("create").Args("-f", linkedBuild).Execute()
					o.Expect(err).NotTo(o.HaveOccurred())
					g.By("starting an unprivileged build")
					br, err := exutil.StartBuildAndWait(oc, "linked-nodejs", "--build-loglevel=6", "--env", buildInUserNSEnvVar)
					o.Expect(err).NotTo(o.HaveOccurred())
					br.AssertSuccess()
					g.By("verifying that the unprivileged build ran in a user namespace")
					logs, err := br.Logs()
					o.Expect(err).NotTo(o.HaveOccurred())
					o.Expect(logs).To(o.MatchRegexp(buildInUserNSRegexp))
				})
