	...
	g.Context("", func() {
		...
		g.Describe("build with long names", func() {
			...
			g.It("delete builds with long names without collateral damage", func() {
				...
				// Names can be a maximum of 203 chars. These build config names are 201 (to allow for suffixes appiled during the test process, e.g. -1, -2 for builds and log filenames)
				// and the names differ only in the last character.
				bcA := "123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890a"
				bcB := "123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890b"
