...
var _ = g.Describe("[sig-builds][Feature:Builds] build without output image", func() {
	...
	g.Context("", func() {
		...
		g.Describe("building from templates", func() {
			g.It("should create an image from a docker template without an output image reference defined [apigroup:build.openshift.io]", func() {
			...
			g.It("should create an image from a S2i template without an output image reference defined [apigroup:build.openshift.io]", func() {
