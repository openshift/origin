...
`
	...
	mpsControlPod, err := spec.driver.GetMPSControlDaemonForClaim(ctx, spec.node, claim)
	o.Expect(err).To(o.BeNil())
	o.Expect(mpsControlPod).ToNot(o.BeNil())
	t.Logf("MPS daemon control daemond pod: %s", mpsControlPod.Name)
	logs, err := drae2eutility.GetLogs(ctx, spec.f.ClientSet, mpsControlPod.Namespace, mpsControlPod.Name, "mps-control-daemon")
	o.Expect(err).To(o.BeNil())
	...
	_, err = drae2eutility.ExecIntoPod(ctx, spec.t, spec.f, mpsControlPod.Name, mpsControlPod.Namespace,
		"mps-control-daemon",
