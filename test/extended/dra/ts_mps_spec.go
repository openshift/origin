...
func (spec gpuTimeSlicingAndMPSWithCUDASpec) Test(ctx context.Context, t g.GinkgoTInterface) {
...
`
	...
	template := drae2eutility.NewResourceClaimTemplate(namespace, "multiple-gpus")
	...
	template.Spec.Spec.Devices.Config = []resourceapi.DeviceClaimConfiguration{
		{Requests: []string{"ts-gpu"}, DeviceConfiguration: newDeviceConfiguration(spec.class, []byte(timeSlicingStrategy))},
	...
	ts := []corev1.Container{
		spec.newContainer("ts-ctr0"),
		spec.newContainer("ts-ctr1"),
	}
	mps := []corev1.Container{
		spec.newContainer("mps-ctr0"),
		spec.newContainer("mps-ctr1"),
	}
	for i := range ts {
		ts[i].Resources.Claims = []corev1.ResourceClaim{{Name: "shared-gpus", Request: "ts-gpu"}}
	}
	for i := range mps {
		mps[i].Resources.Claims = []corev1.ResourceClaim{{Name: "shared-gpus", Request: "mps-gpu"}}
	}
	pod.Spec.Containers = append(ts, mps...)
