...
func (spec gpuTimeSlicingWithCUDASpec) Test(ctx context.Context, t g.GinkgoTInterface) {
	...
	// create a resource claim template that uses time slicing
	const (
		timeSlicingStrategy = `{
...
`
	...
	for i := range ts {
		ts[i].Resources.Claims = []corev1.ResourceClaim{{Name: "single-gpu", Request: "ts-gpu"}}
	}
	pod.Spec.Containers = append(pod.Spec.Containers, ts...)
	...
	claim, err := drae2eutility.GetResourceClaimFor(ctx, spec.f.ClientSet, pod)
	o.Expect(err).To(o.BeNil())
