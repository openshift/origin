...
var _ = g.Describe("[sig-node] DRA [Feature:DynamicResourceAllocation] e2e flow", g.Ordered, func() {
	...
	g.BeforeAll(func() {
		...
		// setup nfd
		ns, err := framework.CreateTestingNS(context.TODO(), "nfd", clientset, nil)
		framework.ExpectNoError(err)
		g.DeferCleanup(func(ctx context.Context) {
			f.ClientSet = clientset
			f.DeleteNamespace(context.TODO(), ns.Name)
		...
		nfd := drae2e.NewHelmInstaller(t, drae2e.HelmParameters{
			Namespace:       ns.Name,
			CreateNamespace: false,
		...
		g.By("waiting for nfd to install")
		o.Expect(nfd.Install()).To(o.Succeed(), "nfd install should not fail")

		g.DeferCleanup(func(ctx context.Context) {
			g.By("waiting for nfd cleanup to complete")
...
func commonSpec(f *framework.Framework, holder *holder) {
	g.It("containers in a pod share gpu", func() {
		driver := holder.driver
		// one pod, two containers, each asking for shared access to a single GPU
		helper := drae2e.NewHelper(f, driver.DeviceClassName())
