...
func fetchOnDiskCertificates(ctx context.Context, kubeClient kubernetes.Interface, nodeList *corev1.NodeList, testPullSpec string) (*certgraphapi.PKIList, error) {
	...
	err = createRBACHostAccess(ctx, kubeClient, namespace)
	if err != nil {
		return nil, err
	}
...
func createRBACHostAccess(ctx context.Context, kubeClient kubernetes.Interface, namespace string) error {
	rbacPrivilegedObj := resourceread.ReadClusterRoleBindingV1OrDie(rbacHostAccessYaml)
	rbacPrivilegedObj.Subjects[0].Namespace = namespace

	client := kubeClient.RbacV1().ClusterRoleBindings()
...
func fetchNodePKIList(ctx context.Context, kubeClient kubernetes.Interface, node *corev1.Node) (*certgraphapi.PKIList, error) {
	var pkiList *certgraphapi.PKIList
