...
var (
	operatorResourceToRequiredConditions = map[schema.GroupVersionResource][]string{
		...
		{Group: "operator.openshift.io", Version: "v1", Resource: "storage"}: { // much isn't listed here and varies by platform
			"ConfigObservationDegraded",
		...
		g.It("should have at least the conditions we had in 4.17", func() {
			...
			for gvr, requiredConditions := range operatorResourceToRequiredConditions {
				...
				for _, requiredCondition := range requiredConditions {
					...
					if condition == nil {
						failures = append(failures, fmt.Sprintf("resource=%v is missing condition %q that was present in 4.17.  If this is intentional, update the list in this test.", gvr, condition))
					}
				}
			}
			if len(failures) > 0 {
