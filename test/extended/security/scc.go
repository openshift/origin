...
var _ = g.Describe("[sig-auth][Feature:SecurityContextConstraints] ", func() {
	...
	oc := exutil.NewCLI(exutil.CliOptions{BaseName: "scc", PodSecurityEnforceLevel: admissionapi.LevelPrivileged})
...
var _ = g.Describe("[sig-auth][Feature:SecurityContextConstraints] ", func() {
	defer g.GinkgoRecover()
	oc := exutil.NewCLI(exutil.CliOptions{BaseName: "ssc", PodSecurityEnforceLevel: admissionapi.LevelBaseline})
