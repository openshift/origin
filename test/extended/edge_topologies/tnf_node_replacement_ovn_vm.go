...
// podReadyConditionTrue reports whether the pod's Ready condition is True.
...
// ovnkubeNodePodLiveAndReady is true when the pod is not terminating, Phase is Running, and Ready=True.
// Terminating pods can still report Ready=True briefly; they must not satisfy readiness for OVN-K polls.
func ovnkubeNodePodLiveAndReady(pod *corev1.Pod) bool {
	if pod == nil {
		return false
	}
	...
	return podReadyConditionTrue(pod)
}

// anyOVNKubeNodePodReady returns true if any listed ovnkube-node pod is Running, not deleting, and Ready.
func anyOVNKubeNodePodReady(pods *corev1.PodList) (podName string, ok bool) {
	for i := range pods.Items {
		p := &pods.Items[i]
		if ovnkubeNodePodLiveAndReady(p) {
			return p.Name, true
		}
	}
	return "", false
...
// Returns (true, nil) when the annotation was cleared and all ovnkube-node deletes succeeded; (false, nil) when no
// action was needed; (false, err) on List/Delete failures so callers do not treat recovery as successful.
func removePreReplacementChassisFromNodeAndRecycleOVNKubeNode(oc *exutil.CLI, targetNodeName, preReplacementChassisID string) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("update node %s to remove chassis annotation: %w", targetNodeName, err)
	}
	...
	if lerr != nil {
		return false, fmt.Errorf("list ovnkube-node on %s: %w", targetNodeName, lerr)
	}
	var delErrs []error
	for i := range pods.Items {
		if derr := oc.AdminKubeClient().CoreV1().Pods(ovnKubernetesNamespace).Delete(ctx, pods.Items[i].Name, metav1.DeleteOptions{}); derr != nil {
			if apierrors.IsNotFound(derr) {
				continue
			}
			delErrs = append(delErrs, fmt.Errorf("delete ovnkube-node %s: %w", pods.Items[i].Name, derr))
		}
	}
	if len(delErrs) > 0 {
		return false, errors.Join(delErrs...)
	}
	if len(pods.Items) > 0 {
...
func clearStaleChassisAnnotationOnReplacementIfNeeded(oc *exutil.CLI, targetNodeName, preReplacementChassisID string) {
	...
	_ = core.PollUntil(func() (bool, error) {
		...
		ok, rerr := removePreReplacementChassisFromNodeAndRecycleOVNKubeNode(oc, targetNodeName, preReplacementChassisID)
		if rerr != nil {
			return false, rerr
		}
		if ok {
...
func provisionTargetNodeWithIronic(testConfig *TNFTestConfig, oc *exutil.CLI) {
	...
	err := recreateBMCSecret(testConfig, oc, context.Background())
	o.Expect(err).To(o.BeNil(), "BMC secret from backup: %v", err)
	...
	o.Expect(waitForBMHProvisioning(context.Background(), testConfig, oc)).To(o.BeNil(), "Expected BareMetalHost to finish provisioning")
	reapplyDetachedAnnotation(testConfig, oc, context.Background())
...
func waitForNodeRecovery(testConfig *TNFTestConfig, oc *exutil.CLI, timeout time.Duration, pollInterval time.Duration) (time.Time, error) {
	...
	err := core.PollUntil(func() (bool, error) {
		...
		if errGet == nil {
			ok, rerr := removePreReplacementChassisFromNodeAndRecycleOVNKubeNode(oc, testConfig.TargetNode.Name, testConfig.Execution.PreReplacementChassisID)
			if rerr != nil {
				return false, rerr
			}
			if ok {
...
// shortNodeHostname returns the first label of a Kubernetes Node name. CNO's ConnectivityCheckController
// builds PodNetworkConnectivityCheck names with WithSource("network-check-source-"+shortName) and
// withTarget("network-check-target", shortName) using strings.Split(nodeName, ".")[0], not FQDNs.
func shortNodeHostname(nodeName string) string {
...
func eastWestCheckName(survivingNodeName, targetNodeName string) string {
	surviving := shortNodeHostname(survivingNodeName)
	target := shortNodeHostname(targetNodeName)
	return fmt.Sprintf("network-check-source-%s-to-network-check-target-%s", surviving, target)
}

// waitForNetworkCheckSourcePodReady polls until a network-check-source pod is scheduled, Running, and Ready.
// CNO does not create or update east-west PNCCs while no source pod is available (e.g. after the only replica
// was on a destroyed node and has not yet rescheduled).
func waitForNetworkCheckSourcePodReady(oc *exutil.CLI, timeout time.Duration) error {
	return core.PollUntil(func() (bool, error) {
		...
		if err != nil {
			e2e.Logf("[east-west] list network-check-source pods: %v", err)
			return false, nil
		}
		for i := range pods.Items {
			p := &pods.Items[i]
			...
			for _, c := range p.Status.Conditions {
				if c.Type == corev1.PodReady && c.Status == corev1.ConditionTrue {
					e2e.Logf("[east-west] network-check-source pod %s on node %s is Running and Ready (CNO can maintain PNCCs)", p.Name, p.Spec.NodeName)
					return true, nil
				}
			}
		}
		e2e.Logf("[east-west] waiting for network-check-source pod Running+Ready on a node (have %d pod(s) in namespace %s)", len(pods.Items), networkDiagnosticsNamespace)
		return false, nil
	}, timeout, eastWestConnectivityPollInterval, "network-check-source pod Running and Ready in "+networkDiagnosticsNamespace)
}

// waitForEastWestConnectivity waits until a PodNetworkConnectivityCheck from the
// network-check-source pod to the other node's network-check-target reports
// Reachable=True. After node replacement the source pod may be rescheduled onto
// the replacement node, changing the PNCC name, so this function discovers the
// actual source-pod node and constructs the check name dynamically.
func waitForEastWestConnectivity(oc *exutil.CLI, survivingNodeName, targetNodeName string, timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
	if err := waitForNetworkCheckSourcePodReady(oc, timeout); err != nil {
		return err
	}
	...
	sourceNode, destNode := resolveEastWestNodes(oc, survivingNodeName, targetNodeName)
	checkName := eastWestCheckName(sourceNode, destNode)
	e2e.Logf("[east-west] Waiting up to %v for PodNetworkConnectivityCheck %s (source=%s -> dest=%s)", remaining, checkName, sourceNode, destNode)
	...
	return core.PollUntil(func() (bool, error) {
		...
		return false, nil
	}, remaining, eastWestConnectivityPollInterval, fmt.Sprintf("east-west connectivity %s -> %s", sourceNode, destNode))
}
...
func resolveEastWestNodes(oc *exutil.CLI, survivingNodeName, targetNodeName string) (string, string) {
	...
	if err != nil {
		e2e.Logf("[east-west] failed to list source pods for node resolution: %v, using default", err)
...
func resetStalePNCC(oc *exutil.CLI, survivingNodeName, targetNodeName string) {
	...
	for i := range pods.Items {
		...
		e2e.Logf("[east-west] Deleting network-check-source pod %s to trigger fresh PNCC creation", podName)
...
func waitForOVNKubeNodeReadyOnSingleNode(oc *exutil.CLI, nodeName string, timeout, interval time.Duration) error {
	return core.PollUntil(func() (bool, error) {
		...
		readyName, ready := anyOVNKubeNodePodReady(pods)
		if !ready {
			p0 := &pods.Items[0]
			e2e.Logf("[post-node-delete OVN] no live Running+Ready ovnkube-node on %s yet (e.g. %s phase=%s deleting=%v)", nodeName, p0.Name, p0.Status.Phase, p0.DeletionTimestamp != nil)
			return false, nil
		}
		e2e.Logf("[post-node-delete OVN] ovnkube-node Ready on %s (%s)", nodeName, readyName)
		return true, nil
...
func waitForOVNKubeNodePodsReadyOnNodes(oc *exutil.CLI, survivingNodeName, targetNodeName string, timeout, interval time.Duration) error {
	return core.PollUntil(func() (bool, error) {
		for _, nodeName := range []string{survivingNodeName, targetNodeName} {
			...
			if !ready {
				p0 := &pods.Items[0]
				e2e.Logf("[east-west recovery] no live Running+Ready ovnkube-node on %s yet (e.g. %s phase=%s deleting=%v)", nodeName, p0.Name, p0.Status.Phase, p0.DeletionTimestamp != nil)
				return false, nil
			}
			e2e.Logf("[east-west recovery] ovnkube-node Ready on %s (%s)", nodeName, readyName)
		}
