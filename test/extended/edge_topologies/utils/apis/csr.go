...
func WaitForAndApproveNodeBootstrapperCSR(parentCtx context.Context, oc *exutil.CLI, nodeName string, timeout time.Duration) error {
	...
	var approved int
	err := wait.PollUntilContextTimeout(parentCtx, 10*time.Second, timeout, true, func(ctx context.Context) (bool, error) {
	...
	if err != nil {
		spotCtx, spotCancel := context.WithTimeout(parentCtx, 15*time.Second)
		defer spotCancel()
