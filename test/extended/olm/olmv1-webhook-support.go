...
var _ = g.Describe("[sig-olmv1][OCPFeatureGate:NewOLMWebhookProviderOpenshiftServiceCA][Skipped:Disconnected][Serial] OLMv1 operator with webhooks", g.Ordered, g.Serial, func() {
	...
	g.BeforeAll(func(ctx g.SpecContext) {
		...
		err := oc.AsAdmin().WithoutNamespace().Run("wait").Args("--for=condition=Available", "-n", testNamespace, "deployments/webhook-operator-webhook").Execute()
	...
	g.It("should still work through openshift-service-ca certificate rotation", func(ctx g.SpecContext) {
		file := filepath.Join(webhookTestBaseDir, "mutating-webhook-test.yaml")
	...
	g.It("should still work if tls secret is deleted", func(ctx g.SpecContext) {
		...
		o.Eventually(func(g o.Gomega) {
			err := oc.AsAdmin().WithoutNamespace().Run("get").Args("secret", certificateSecretName, "-n", testNamespace).Execute()
			o.Expect(err).ToNot(o.HaveOccurred())
		}).Should(o.Succeed())

		// delete tls secret
		g.By("checking webhook is responsible through secret recreation")
		err := oc.AsAdmin().WithoutNamespace().Run("delete").Args("secret", certificateSecretName, "-n", testNamespace).Execute()
		o.Expect(err).ToNot(o.HaveOccurred())
