...
var _ = g.Describe("[sig-auth][Feature:SCC][Early]", func() {
	...
	oc := exutil.NewCLI(exutil.CliOptions{BaseName: "working-scc-during-install", WithoutNamespace: true})
...
var _ = g.Describe("[sig-auth][Feature:PodSecurity][Feature:SCC]", func() {
	...
	oc := exutil.NewCLI(exutil.CliOptions{BaseName: "required-scc", PodSecurityEnforceLevel: psapi.LevelPrivileged})
