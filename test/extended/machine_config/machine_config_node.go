...
func ValidateMCNConditionTransitions(oc *exutil.CLI, fixture string) {
	...
	err := waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdated, metav1.ConditionFalse, 1*time.Minute, 2*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateDrained, metav1.ConditionTrue, 2*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateExecuted, metav1.ConditionTrue, 20*time.Second, 3*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateRebooted, metav1.ConditionTrue, 5*time.Minute, 3*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Checking Resumed=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeResumed, metav1.ConditionTrue, 15*time.Second, 3*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Checking UpdateComplete=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateComplete, metav1.ConditionTrue, 10*time.Second, 2*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdated, metav1.ConditionTrue, 1*time.Minute, 2*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
