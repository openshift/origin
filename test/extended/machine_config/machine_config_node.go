...
func ValidateMCNConditionTransitions(oc *exutil.CLI, fixture string) {
	...
	err := waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdated, metav1.ConditionFalse, 1*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for UpdatePrepared=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdatePrepared, metav1.ConditionTrue, 1*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for UpdateExecuted=Unknown")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateExecuted, metav1.ConditionUnknown, 1*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateFilesAndOS, metav1.ConditionTrue, 3*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for UpdateExecuted=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateExecuted, metav1.ConditionTrue, 20*time.Second, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for UpdatePostActionComplete=Unknown")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdatePostActionComplete, metav1.ConditionUnknown, 30*time.Second, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for RebootedNode=Unknown")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateRebooted, metav1.ConditionUnknown, 15*time.Second, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for RebootedNode=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateRebooted, metav1.ConditionTrue, 5*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for Resumed=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeResumed, metav1.ConditionTrue, 15*time.Second, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for UpdateComplete=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateComplete, metav1.ConditionTrue, 10*time.Second, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for Uncordoned=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdateUncordoned, metav1.ConditionTrue, 10*time.Second, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Waiting for Updated=True")
	err = waitForMCNConditionStatus(clientSet, workerNode.Name, mcfgv1alpha1.MachineConfigNodeUpdated, metav1.ConditionTrue, 1*time.Minute, 1*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
