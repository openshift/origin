...
var _ = g.Describe("[Suite:openshift/machine-config-operator/disruptive][sig-mco][OCPFeatureGate:ManagedBootImagesvSphere][Serial]", func() {
	...
	/*
		g.It("Should scale up a new node with the latest bootimage version [apigroup:machineconfiguration.openshift.io]", func() {
			ScaleUpMachineSetTest(oc, partialMachineSetFixture)
		})
	*/
})
