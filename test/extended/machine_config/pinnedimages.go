...
var _ = g.Describe("[sig-mco][OCPFeatureGate:PinnedImages][OCPFeatureGate:MachineConfigNode][Serial]", func() {
	...
	g.It("All Nodes in a custom Pool should have the PinnedImages even after Garbage Collection [apigroup:machineconfiguration.openshift.io]", func() {
		...
		// Add node to pool
		nodes, err := oc.KubeClient().CoreV1().Nodes().List(context.TODO(), metav1.ListOptions{LabelSelector: labels.SelectorFromSet(labels.Set{"node-role.kubernetes.io/worker": ""}).String()})
...
func unlabelNode(oc *exutil.CLI, name string) error {
	err := oc.AsAdmin().Run("label").Args("node", name, "node-role.kubernetes.io/custom-").Execute()
	return err
}
