...
// `UpdateDeletePolicy` updates the provided MachineSet's `deletePolicy` to be the provided value
func UpdateDeletePolicy(oc *exutil.CLI, machineSetName string, updatedDeletionPolicy string) error {
	deletionPolicy := updatedDeletionPolicy
	// If deletionPolicy was not explicitly set before, reset to default
	// Ref: https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/machine_management/manually-scaling-machineset#machineset-delete-policy_manually-scaling-machineset
	if updatedDeletionPolicy == "" {
		framework.Logf("Machine originally did not have `deletePolicy` set. Defaulting to using default 'Random' policy.")
		deletionPolicy = "Random"
	}
	return oc.Run("patch").Args("machinesets.machine.openshift.io", machineSetName, "-n", "openshift-machine-api", "-p", fmt.Sprintf(`{"spec":{"deletePolicy":"%v"}}`, deletionPolicy), "--type=merge").Execute()
}
...
// `CleanupMachineSetScale` cleans up a MachineSet scaled during an e2e test. The `testFailed`
// parameter indicates whether the test completed successfully (false) or not (true).
//   - When a test fails, it is important to wait before checking if the MachineSet replica count
//     matches the original in case the test failed during provisioning.
//   - When a test completes successfully, no cleanup is needed.
func CleanupMachineSetScale(oc *exutil.CLI, machineClient *machineclient.Clientset, machineSetName string, originalReplicaValue int,
	originalDeletionPolicy string, newNodeName string, newMachineName string, testFailed bool) error {
	// Skip cleanup when test has completed successfully
	if !testFailed {
		framework.Logf("Test completed successfully. No further cleanup required.")
		return nil
	}

	// Get MachineSet
	framework.Logf("Getting MachineSet '%v'.", machineSetName)
	machineSet, err := machineClient.MachineV1beta1().MachineSets("openshift-machine-api").Get(context.TODO(), machineSetName, metav1.GetOptions{})
	if err != nil {
		return err
	}

	// Check if MachineSet needs to be scaled
	if int(*machineSet.Spec.Replicas) == originalReplicaValue {
		framework.Logf("MachineSet '%v' does not need to be scaled. Replica value matches original replica value of %v.", machineSet.Name, originalReplicaValue)
		return nil
	}

	// Scale MachineSet to original replica value
	framework.Logf("Scaling MachineSet '%s' to replica value %v.", machineSet.Name, originalReplicaValue)
	scaleErr := ScaleMachineSet(oc, machineSet.Name, fmt.Sprintf("%d", originalReplicaValue))
	if scaleErr != nil {
		return scaleErr
	}

	// Ensure MachineSet finishes scaling and node is deleted
	var scaleDownErr error = nil
	if newNodeName != "" { //check on created node
		framework.Logf("Waiting for node '%v' to be deleted.", newNodeName)
		scaleDownErr = WaitForNodeToBeDeleted(oc, newNodeName)
	} else if newMachineName != "" { //check on created machine
		framework.Logf("Waiting for machine '%v' to be deleted.", newMachineName)
		scaleDownErr = WaitForMachineToBeDeleted(machineClient, newMachineName)
	} else { //get deleting machine and check on deletion
		...
		if deletingMachineErr != nil {
			return deletingMachineErr
		}
		framework.Logf("Waiting for machine '%v' to be deleted.", deletingMachine.Name)
		scaleDownErr = WaitForMachineToBeDeleted(machineClient, deletingMachine.Name)
	}
	if scaleDownErr != nil {
		return scaleDownErr
	}
