...
function copy-container-files() {
  ...
  for container_name in "${CONTAINER_NAMES[@]}"; do
    ...
    sudo os::util::docker cp "${container_name}:${source_path}" "${dest_dir}"
  done
}
...
function save-container-logs() {
  ...
  for container_name in "${CONTAINER_NAMES[@]}"; do
    ...
    sudo os::util::docker exec -t "${container_name}" bash -c "journalctl | \
gzip > ${container_log_file}"
    sudo os::util::docker cp "${container_name}:${container_log_file}" "${dest_dir}"
