...
// No-op if artifactDir is empty.
func startCEOLogCapture(artifactDir string) (stop func()) {
	if artifactDir == "" {
		return func() {}
	}
	logPath := filepath.Join(artifactDir, "cluster-etcd-operator-node-replacement.log")
	return func() {
...
//
// Numbered steps below are integers only. Steps 8 and 9 run only when the current spec has already failed
...
func recoverClusterFromBackup(testConfig *TNFTestConfig, oc *exutil.CLI) {
	...
	if err := recoverBMHFromBackup(testConfig, oc); err != nil {
	...
	if err := recoverMachineFromBackup(testConfig, oc); err != nil {
	...
	if err := apis.WaitForAndApproveNodeBootstrapperCSR(oc, testConfig.TargetNode.Name, csrApprovalWaitTimeout); err != nil {
...
func recoverEtcdSecretsFromBackup(testConfig *TNFTestConfig, oc *exutil.CLI) error {
	...
	for _, secretName := range etcdSecrets {
		...
		// Check if the secret already exists (via API)
		_, err := oc.AdminKubeClient().CoreV1().Secrets(services.EtcdNamespace).Get(context.Background(), secretName, metav1.GetOptions{})
		if err == nil {
			e2e.Logf("Etcd secret %s already exists, skipping recreation", secretName)
			continue
		}

		// Create secret from backup file via API
		secretBytes, readErr := os.ReadFile(secretFile)
		o.Expect(readErr).To(o.BeNil(), "Expected to read secret file %s", secretFile)
		var secret corev1.Secret
		o.Expect(utils.DecodeObject(string(secretBytes), &secret)).To(o.Succeed(), "Expected to decode etcd secret %s", secretName)
		secret.ResourceVersion = ""
		secret.UID = ""
		err = core.RetryWithOptions(func() error {
			_, createErr := oc.AdminKubeClient().CoreV1().Secrets(services.EtcdNamespace).Create(context.Background(), &secret, metav1.CreateOptions{})
			return createErr
		}, core.RetryOptions{
...
func recoverBMHFromBackup(testConfig *TNFTestConfig, oc *exutil.CLI) error {
	waitForBaremetalOperatorWebhookReady(oc, baremetalWebhookWaitTimeout)
	if err := recreateBMCSecret(testConfig, oc); err != nil {
	...
	if bmhExists {
		...
		if getErr != nil {
			...
			for check := 1; check <= recoverBMHTerminatingMaxChecks; check++ {
				...
				time.Sleep(recoverBMHTerminatingPollInterval)
			}
		} else {
	...
	if !bmhExists {
		...
		err = core.RetryWithOptions(func() error {
			_, createErr := dyn.Resource(apis.BMHGVR).Namespace(machineAPINamespace).Create(ctx, &bmhU, metav1.CreateOptions{})
			return createErr
		}, core.RetryOptions{
			Timeout:      etcdThreeMinutePollTimeout,
			PollInterval: utils.ThirtySecondPollInterval,
...
func recoverMachineFromBackup(testConfig *TNFTestConfig, oc *exutil.CLI) error {
	ctx := context.Background()
	dyn, err := dynamic.NewForConfig(oc.AdminConfig())
	if err != nil {
		return fmt.Errorf("create dynamic client: %w", err)
	...
	err = core.RetryWithOptions(func() error {
		_, createErr := dyn.Resource(apis.MachineGVR).Namespace(machineAPINamespace).Create(ctx, &u, metav1.CreateOptions{})
		return createErr
	}, core.RetryOptions{
		Timeout:      etcdThreeMinutePollTimeout,
		PollInterval: utils.ThirtySecondPollInterval,
...
// recreateBMCSecret recreates the BMC secret from backup via the cluster API.
func recreateBMCSecret(testConfig *TNFTestConfig, oc *exutil.CLI) error {
	...
	secretBytes, readErr := os.ReadFile(bmcSecretFile)
	if readErr != nil {
		return fmt.Errorf("read BMC secret file: %w", readErr)
	}
	var secret corev1.Secret
	if err := utils.DecodeObject(string(secretBytes), &secret); err != nil {
		return fmt.Errorf("decode BMC secret: %w", err)
	}
	secret.ResourceVersion = ""
	secret.UID = ""
	err = core.RetryWithOptions(func() error {
		_, createErr := oc.AdminKubeClient().CoreV1().Secrets(machineAPINamespace).Create(context.Background(), &secret, metav1.CreateOptions{})
		return createErr
	}, core.RetryOptions{
		Timeout:      etcdThreeMinutePollTimeout,
		PollInterval: utils.ThirtySecondPollInterval,
...
func backupTargetNodeConfiguration(testConfig *TNFTestConfig, oc *exutil.CLI) string {
	...
	for _, secretName := range etcdSecrets {
		...
		if err != nil {
			e2e.Logf("WARNING: Could not marshal etcd secret %s: %v", secretName, err)
			continue
		}
		secretFile := filepath.Join(backupDir, secretName+".yaml")
		if err := os.WriteFile(secretFile, secretBytes, core.SecureFileMode); err != nil {
			e2e.Logf("WARNING: Could not write etcd secret %s: %v", secretName, err)
			continue
		}
...
// deleteNodeReferences deletes OpenShift resources for the target node: etcd TLS secrets, BMH, Machine, Node;
// clears stale Etcd and KubeAPIServer operator nodeStatuses for same-name replacement; deletes stale app=installer
// Pods for the removed node in static-pod operator namespaces; removes OVN SB chassis for the old node name.
func deleteNodeReferences(testConfig *TNFTestConfig, oc *exutil.CLI) {
	...
	// BMO/Ironic need time to deprovision; we retry regular Delete deleteBMHMachineMaxAttempts times with
	// deleteBMHMachineRetryInterval between attempts (no force-delete). Webhook/BMO readiness was waited above.
	...
	// Delete the Node object last. The replacement node only appears after we create a new BMH and Machine
	// and the host is provisioned; by then we have already completed these deletes, so the name is free
	// when the new node registers and the machine-approver can approve its CSR.
	nodeName := testConfig.TargetNode.Name
	ctxPre, cancelPre := context.WithTimeout(context.Background(), shortK8sClientTimeout)
	nodePre, errPre := oc.AdminKubeClient().CoreV1().Nodes().Get(ctxPre, nodeName, metav1.GetOptions{})
	cancelPre()
	if errPre == nil {
		testConfig.Execution.PreReplacementChassisID = nodeOVNChassisIDFromNode(nodePre)
		testConfig.Execution.PreReplacementNodeUID = string(nodePre.UID)
		e2e.Logf("[chassis provision observe] captured pre-replacement for comparison: node-chassis-id=%q node.uid=%s (after provision, new Node should have new uid; chassis matching pre-replace suggests carry-forward)",
			testConfig.Execution.PreReplacementChassisID, testConfig.Execution.PreReplacementNodeUID)
	} else {
		e2e.Logf("[chassis provision observe] could not capture pre-replacement node before delete: %v", errPre)
	}
