...
func truncateForLog(s string, maxBytes int) (truncated string, totalBytes int) {
	...
	for i := 0; i < len(s); {
		...
		// Only treat invalid UTF-8 as a stop (RuneError with size 1). A valid U+FFFD rune uses size > 1.
...
func CleanupRemoteKnownHostsFile(sshConfig *SSHConfig, localKnownHostsPath string, remoteKnownHostsPath string) error {
	...
	if err != nil {
		e2e.Logf("WARNING: Failed to clean up remote known_hosts file: %v", err)
		return err
	}
...
func CleanupLocalKnownHostsFile(sshConfig *SSHConfig, knownHostsPath string) error {
	...
	if err != nil {
		e2e.Logf("WARNING: Failed to clean up local known_hosts file: %v", err)
		return err
	}
