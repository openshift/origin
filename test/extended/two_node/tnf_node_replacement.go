...
var _ = g.Describe("[sig-etcd][apigroup:config.openshift.io][Suite:openshift/two-node][Slow][Serial][Disruptive][Requires:HypervisorSSHConfig] TNF", func() {
	...
	g.It("cluster recovers when a permanently failed node needing manual recovery is replaced", func() {
		defer startCEOLogCapture(os.Getenv("ARTIFACT_DIR"))()
		...
		e2e.Logf("[stage timing] Deleting node references (BMH/Machine/Node + OVN SB chassis-del + etcd/KAS nodeStatus + installer pods): %v (BMH/Machine delete: %d attempts, %v between)", time.Since(stageStart), deleteBMHMachineMaxAttempts, deleteBMHMachineRetryInterval)
		stageStart = time.Now()
		...
		o.Expect(apis.WaitForAndApproveNodeBootstrapperCSR(oc, testConfig.TargetNode.Name, csrApprovalWaitTimeout)).To(o.Succeed(), "Approve node-bootstrapper CSR for replacement node when machine-approver has not (reused node name)")
