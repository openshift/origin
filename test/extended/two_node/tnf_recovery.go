...
var _ = g.Describe("[sig-etcd][apigroup:config.openshift.io][OCPFeatureGate:DualReplica][Suite:openshift/two-node][Disruptive] Two Node with Fencing etcd recovery", func() {
	...
	g.BeforeEach(func() {
		...
		err := ensureEtcdOperatorHealthy(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred(), "etcd cluster operator should be healthy before starting test")
	...
	g.It("Should recover from ungraceful node shutdown with etcd member re-addition", func() {
		...
		g.By(fmt.Sprintf("Ensuring target's etcd server %s is unreachable (it might take a while)", targetNode.Name))
		o.Eventually(func() error {
			return helpers.EnsureHealthyMember(g.GinkgoT(), etcdClientFactory, targetNode.Name)
		}, memberHasLeftTimeout, pollInterval).Should(o.HaveOccurred(), fmt.Sprintf("Time out waiting for the node %s to leave the etcd cluster", targetNode.Name))

		//g.By(fmt.Sprintf("Ensuring leader %s is a healthy voting member", survivedNode.Name))
		//o.Eventually(func() error {
		//return helpers.EnsureHealthyMember(g.GinkgoT(), etcdClientFactory, survivedNode.Name)
		//}, memberHasLeftTimeout, pollInterval).ShouldNot(o.HaveOccurred(), fmt.Sprintf("Node %s should have become leader", survivedNode.Name))

		g.By(fmt.Sprintf("Ensuring that %s added %s back as learner", survivedNode.Name, targetNode.Name))
		validateEtcdRecoveryState(etcdClientFactory,
			&survivedNode, true, false, // survivedNode expected started == true, learner == false
			&targetNode, true, true, // targetNode expected started == false, learner == true
			memberIsLeaderTimeout, pollInterval)
