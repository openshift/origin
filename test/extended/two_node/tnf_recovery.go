...
const (
	...
	networkDisruptionDuration    = 1 * time.Minute
...
var _ = g.Describe("[sig-etcd][apigroup:config.openshift.io][OCPFeatureGate:DualReplica][Suite:openshift/two-node][Disruptive] Two Node with Fencing etcd recovery", func() {
	...
	g.It("Should recover from network disruption with etcd member re-addition", func() {
		g.By(fmt.Sprintf("Blocking network communication between %s and %s for %v ", targetNode.Name, survivedNode.Name, networkDisruptionDuration))
		command, err := util.TriggerNetworkDisruption(oc.KubeClient(), targetNode, survivedNode, "br-ex", networkDisruptionDuration)
		o.Expect(err).To(o.BeNil(), "Expected to disrupt network without errors")
		framework.Logf("command: '%s'", command)

		g.By(fmt.Sprintf("Ensuring that %s added %s back as learner after fencing recovery", survivedNode.Name, targetNode.Name))
		// Note: Network disruption scenarios differ from graceful/ungraceful shutdown tests.
		// During network disruption, etcd becomes inaccessible until the targetNode is fenced
		// and the Survived Node recovers the etcd cluster. By the time the survived node can
		// restart etcd services, the target node has already been restored and is ready
		// to rejoin, leaving a narrow window to observe the intermediate recovery states
		// compared to shutdown scenarios where the target remains unavailable longer.
		validateEtcdRecoveryState(etcdClientFactory,
			&survivedNode, true, false, // survivedNode expected started == true, learner == false
			&targetNode, true, true, // targetNode expected started == true, learner == true (already running)
			memberRejoinedLearnerTimeout, pollInterval)
...
func validateEtcdRecoveryState(e *helpers.EtcdClientFactoryImpl,
	...
	o.EventuallyWithOffset(1, func() error {
		...
		if err != nil {
			framework.Logf("could not get members: error %v", err)
			return err
		}
		if len(members) != 2 {
		...
		if started, learner, err := getMemberState(survivedNode, members); err != nil {
			framework.Logf("could not get member state of node '%s', error %v\n", survivedNode, err)
			return err
