...
var _ = g.Describe("[sig-cli] oc version", func() {
	...
	g.It("check kubernetes version matches the io.openshift.build.versions label", func() {
		...
		re1 := regexp.MustCompile(`io.openshift.build.versions=kubernetes=[0-9]+.[0-9]+.[0-9]+`)
		...
		re := regexp.MustCompile(`Kubernetes Version: v[0-9]+.[0-9]+.[0-9]+($|\+)`)
		...
		if match, _ := regexp.MatchString(kversionNum, result[0]); !match {
			e2e.Failf("Failed to match Kubernetes version %v & oc Version %v", kversionNum, result[0])
		}
