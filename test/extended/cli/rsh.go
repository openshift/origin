...
var _ = g.Describe("[cli]oc rsh[Conformance]", func() {
	...
	var (
		...
		podsLabel              = exutil.ParseLabelsOrDie("name=hello-openshift")
	)
	...
	g.Describe("rsh specific flags", func() {
		g.It("should work well when access to a remote shell", func() {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			o.Expect(out).To(o.ContainSubstring("Defaulting container name to hello-openshift"))

			g.By("running the rsh command with specify container name and shell")
			_, err = oc.Run("rsh").Args("--container=hello-openshift-fedora", "--shell=/bin/sh", pods[0], "mkdir", "/tmp/test3").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
		})
	})
})
