...
var _ = g.Describe("[Serial][sig-cli] oc adm upgrade recommend", g.Ordered, func() {
	...
	g.Context("When the update service has no recommendations", func() {
		g.BeforeAll(func() {
			...
			time.Sleep(5 * time.Second) // Give the CVO time to retrieve recommendations and push to status
		})
...
//Channel: test-channel (available channels: other-channel, test-channel)  FIXME: why isn't this matching?
//No updates available. You may still upgrade to a specific release image.*`))
  ...
  "nodes": [
    ...
    {"version": "4.{{.NextMinor}}.0","payload": "example.com/test@sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"}
  ],
  "edges": [[0, 1], [0, 2]],
  "conditionalEdges": [
    {
...
Updates to 4.[0-9]*:
  ...
  Version: 4[.][0-9]*[.]0
  Image: quay.io/openshift-release-dev/ocp-release@sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  Reason: TestReason
