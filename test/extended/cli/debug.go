...
`).Execute()
	...
	g.It("ensure that the label is set for node debug", func() {
		...
		err = oc.AsAdmin().Run("debug").Args("node/"+readyWorkerNode, "--image="+image.ShellImage(), "--keep-labels=true", "--preserve-pod=true", "--", "sleep", "1").Execute()
		...
		err = oc.AsAdmin().Run("debug").Args("node/"+readyWorkerNode, "--image="+image.ShellImage(), "--preserve-pod=true", "--", "sleep", "1").Execute()
