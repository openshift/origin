...
func waitForKeycloakAvailable(ctx context.Context, client *exutil.CLI, namespace string) error {
	timeoutCtx, cancel := context.WithDeadline(ctx, time.Now().Add(30*time.Minute))
	defer cancel()
	err := wait.PollUntilContextCancel(timeoutCtx, 30*time.Second, true, func(ctx context.Context) (done bool, err error) {
