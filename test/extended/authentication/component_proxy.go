...
func testHotReloadCAFileChange(ctx context.Context, oc *exutil.CLI, caCertPEM []byte, kcSetup *keycloakProxySetup, httpsProxyURL, proxyNamespace string) {
	...
	err = wait.PollUntilContextTimeout(ctx, 10*time.Second, 3*time.Minute, true, func(ctx context.Context) (done bool, err error) {
		return podFileContentMatches(oc, squidPods.Items[0], "/etc/squid/tls/tls.crt", newServerCertPEM)
	})
	...
	err = wait.PollUntilContextTimeout(ctx, 10*time.Second, 5*time.Minute, true, func(ctx context.Context) (bool, error) {
		for _, pod := range oauthServerPodList.Items {
			success, err := podFileContentMatches(oc, pod, caFilePath, newCACertPEM)
			if success {
				continue
			}
			return success, err
...
func podFileContentMatches(oc *exutil.CLI, pod v1.Pod, caFilePath string, newCACertPEM []byte) (bool, error) {
	g.GinkgoHelper()
