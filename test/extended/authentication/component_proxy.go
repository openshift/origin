...
var _ = g.Describe("[sig-auth][Suite:openshift/conformance/serial][OCPFeatureGate:AuthenticationComponentProxy][Serial]", func() {
	...
	g.It("should perform full OIDC login flow through the proxy when auth proxy config is applied", func() {
		testProxyConfigPerformOIDCLogin(ctx, oc, kcSetup, httpProxyURL, proxyNamespace)
	})
	g.It("should hot-reload mounted CA file on change when spec.proxy.trustedCA is set", func() {
		testHotReloadCAFileChange(ctx, oc, caCertPEM, kcSetup, httpsProxyURL, proxyNamespace)
	})
	g.It("should bypass proxy by directly connecting to idp to perform OIDC login flow when spec.proxy.noProxy contains idp", func() {
...
func testHotReloadCAFileChange(ctx context.Context, oc *exutil.CLI, caCertPEM []byte, kcSetup *keycloakProxySetup, httpsProxyURL, proxyNamespace string) {
	...
	err = wait.PollUntilContextTimeout(ctx, 10*time.Second, 5*time.Minute, true, func(ctx context.Context) (bool, error) {
		for _, pod := range podNamesBefore.UnsortedList() {
			output, execErr := oc.AsAdmin().Run("exec").Args(
	...
	oauthServerPodListAfter, err := kubeClient.CoreV1().Pods("openshift-authentication").List(ctx, metav1.ListOptions{LabelSelector: "app=oauth-openshift"})
	o.Expect(err).NotTo(o.HaveOccurred())
