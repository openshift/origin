...
var _ = g.Describe("[sig-etcd][Feature:DisasterRecovery][Disruptive]", func() {
	...
	g.It("[dr-etcd-snapshot] Cluster should restore itself from etcd snapshot", func() {
		...
		execOnNodeOrFail(firstMaster, "sudo -i /bin/bash -x /usr/local/bin/etcd-snapshot-backup.sh /root/assets/backup")
		execOnNodeOrFail(firstMaster, "sudo -i /bin/bash -c '/bin/tar -xzf /root/assets/backup/snapshot* -C /root/assets/backup snapshot.db'")
		execOnNodeOrFail(firstMaster, "sudo -i install -o core -g core /root/assets/backup/snapshot.db /tmp/snapshot.db")
