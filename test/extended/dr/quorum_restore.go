	...
	g.It("[dr-quorum-restore] Cluster should restore itself after quorum loss", func() {
		...
		disruption.Run("Quorum Loss and Restore", "quorum_restore",
			...
			func() {
				...
				_, err = oc.AdminOperatorClient().OperatorV1().Etcds().Patch("cluster", types.MergePatchType, []byte(`{"spec": {"forceRedeploymentReason": "recover-etcd"}}`), "spec")
				o.Expect(err).NotTo(o.HaveOccurred())

				framework.Logf("Force new revision of kube-apiserver")
				_, err = oc.AdminOperatorClient().OperatorV1().KubeAPIServers().Patch("cluster", types.MergePatchType, []byte(`{"spec": {"forceRedeploymentReason": "recover-kube-apiserver"}}`), "spec")
				o.Expect(err).NotTo(o.HaveOccurred())
