...
var _ = g.Describe("[sig-network][Feature:Router]", func() {
	...
	g.Describe("The HAProxy router", func() {
		...
		g.It("should expose prometheus metrics for a route [apigroup:route.openshift.io]", func(ctx g.SpecContext) {
			...
			err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 240*time.Second, true, func(context.Context) (bool, error) {
			...
			if err == context.DeadlineExceeded && lastMetricsErr != nil {
				err = fmt.Errorf("last metrics scrape failed: %w", lastMetricsErr)
			}
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			o.Expect(reloadSeconds).NotTo(o.BeEmpty())
			if len(reloadSeconds) > 0 {
				o.Expect(reloadSeconds[0].Summary.GetSampleSum()).To(o.BeNumerically(">", 0))
			}
			writeConfigSeconds := findMetricsWithLabels(metrics["template_router_write_config_seconds"], nil)
			o.Expect(writeConfigSeconds).NotTo(o.BeEmpty())
			if len(writeConfigSeconds) > 0 {
				o.Expect(writeConfigSeconds[0].Summary.GetSampleSum()).To(o.BeNumerically(">", 0))
			}
