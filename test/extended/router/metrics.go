	...
	g.Describe("The HAProxy router", func() {
		...
		g.It("should expose prometheus metrics for a route [apigroup:route.openshift.io]", func() {
			...
			err = wait.PollImmediate(2*time.Second, 240*time.Second, func() (bool, error) {
				results, err = getBearerTokenURLViaPod(oc, execPodName, fmt.Sprintf("http://%s/metrics", net.JoinHostPort(host, strconv.Itoa(int(metricsPort)))), bearerToken)
				o.Expect(err).NotTo(o.HaveOccurred())
			...
			updatedResults, err := getBearerTokenURLViaPod(oc, execPodName, fmt.Sprintf("http://%s/metrics", net.JoinHostPort(host, strconv.Itoa(int(metricsPort)))), bearerToken)
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("should enable openshift-monitoring to pull metrics", func(ctx g.SpecContext) {
			...
			o.Expect(wait.PollImmediate(10*time.Second, 5*time.Minute, func() (bool, error) {
				contents, err := getBearerTokenURLViaPod(oc, execPod.Name, fmt.Sprintf("%s/api/v1/targets?state=active", url), token)
				o.Expect(err).NotTo(o.HaveOccurred())
