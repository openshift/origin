...
// is a multi-hop chain with a pinned version, (for NonDefault) no safe non-default version is available,
// or (for Unset) the cluster has an unsupported HAProxy version override annotation set.
func (h *HAProxyVersionUpgradeTest) Skip(upgctx upgrades.UpgradeContext) bool {
	...
	if h.Mode == HAProxyUpgradeModeUnset {
		...
		if err != nil {
			h.precheckErr = fmt.Errorf("error reading Ingress cluster config for HAProxy version override check: %w", err)
			return false
		}
		if ingCluster.Annotations["unsupported.ingress.openshift.io/default-haproxy-version"] != "" {
			framework.Logf("skipping: unset mode and HAProxy version is being overridden")
			return true
		}
	}
