...
var _ = g.Describe("[sig-network-edge][OCPFeatureGate:GatewayAPIController][Feature:Router][apigroup:gateway.networking.k8s.io]", g.Ordered, g.Serial, func() {
	...
	g.AfterEach(func() {
		if !checkAllTestsDone(oc) {
			...
			o.Expect(err).NotTo(o.HaveOccurred(), "Failed to create the dynamic client")
			...
			operator, err := client.Resource(gvr).Get(context.Background(), serviceMeshOperatorName, metav1.GetOptions{})
