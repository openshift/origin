...
var _ = g.Describe("[sig-network-edge][OCPFeatureGate:GatewayAPIController][Feature:Router][apigroup:gateway.networking.k8s.io]", g.Ordered, g.Serial, func() {
	...
	g.It("Ensure LB, service, and dnsRecord are created for a Gateway object", func() {
		...
		// check LB service
		lbExternalIP, lberr := oc.AsAdmin().WithoutNamespace().Run("get").Args("-n", "openshift-ingress", "dnsrecord", dnsRecordName, `-o=jsonpath={.spec.targets[0]}`).Output()
		...
		o.Expect(lbExternalIP).To(o.Equal(gwlist.Status.Addresses[0].Value))
	})
...
func checkGatewayClass(oc *exutil.CLI, name string) error {
	...
	waitErr := wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 2*time.Minute, false, func(context context.Context) (bool, error) {
...
func createHttpRoute(oc *exutil.CLI, routeName, hostname, gwName, backendRefname string) (*gatewayapiv1.HTTPRoute, error) {
	gwapiClient := gatewayapiclientset.NewForConfigOrDie(oc.AdminConfig())
	namespace := oc.Namespace()
	ingressNameSpace := "openshift-ingress"
