...
var _ = g.Describe("[sig-cluster-lifecycle][Feature:KubeletAuthenticationProviders][Serial] KubeletAuthenticationProviders should", func() {
	...
	g.It("be able to pull images from ECR", func() {
		...
		password, err := ecrClient.GetAuthorizationToken()
		o.Expect(err).ToNot(o.HaveOccurred(), "unable to get token")
		o.Expect(password).NotTo(o.BeEmpty(), "token should not be empty")
	...
	g.It("be able to pull images from ACR", func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred(), "unable to create a new container registry")
		defer exutil.DeleteAzureContainerRegistry(az, registryName, resourceGroup)
		user, password, err := exutil.GetAzureContainerRepositoryCredential(az, registryName, resourceGroup)
		o.Expect(err).NotTo(o.HaveOccurred(), "unable to get container reposity authorization token")
