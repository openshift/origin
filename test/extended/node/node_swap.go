...
var _ = g.Describe("[Jira:Node][sig-node] Node non-cnv swap configuration", func() {
	...
	g.It("should reject user override of swap settings via KubeletConfig API [OCP-86395]", func(ctx context.Context) {
		...
		workerMC, err := mcClient.MachineconfigurationV1().MachineConfigs().Get(ctx, "99-worker-generated-kubelet", metav1.GetOptions{})
		initialResourceVersion := ""
		if err == nil {
			initialResourceVersion = workerMC.ResourceVersion
			framework.Logf("Initial 99-worker-generated-kubelet resourceVersion: %s", initialResourceVersion)
		}
		...
		err = wait.Poll(2*time.Second, 30*time.Second, func() (bool, error) {
			...
			for _, condition := range kc.Status.Conditions {
				if condition.Type == "Failure" && condition.Status == "False" {
		...
		o.Expect(err).NotTo(o.HaveOccurred(), "Error while polling KubeletConfig status")
		o.Expect(foundExpectedError).To(o.BeTrue(), "Expected to find error message about failSwapOn not being allowed in KubeletConfig status")
		...
		workerMC, err = mcClient.MachineconfigurationV1().MachineConfigs().Get(ctx, "99-worker-generated-kubelet", metav1.GetOptions{})
		if err == nil {
			o.Expect(workerMC.ResourceVersion).To(o.Equal(initialResourceVersion), "Machine config 99-worker-generated-kubelet should not be updated when failSwapOn is rejected")
			framework.Logf("Verified: 99-worker-generated-kubelet was not updated (resourceVersion: %s)", workerMC.ResourceVersion)
		}
