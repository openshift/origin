...
var _ = g.Describe("[sig-node][DRA][OCPFeatureGate:DynamicResourceAllocation]", func() {
	...
	g.Context("Dynamic Resource Allocation", func() {
		...
		g.It("should verify beta and alpha DRA APIs are disabled [apigroup:resource.k8s.io]", func(ctx context.Context) {
			...
			g.By("verifying v1alpha2 API is not available")
			_, err = discoveryClient.ServerResourcesForGroupVersion("resource.k8s.io/v1alpha2")
			o.Expect(err).To(o.HaveOccurred(), "v1alpha2 API should not be available")
			o.Expect(apierrors.IsNotFound(err)).To(o.BeTrue(), "v1alpha2 should return NotFound error")
			framework.Logf("Confirmed v1alpha2 API is disabled: %v", err)

			g.By("verifying v1beta1 API is not available")
			_, err = discoveryClient.ServerResourcesForGroupVersion("resource.k8s.io/v1beta1")
			o.Expect(err).To(o.HaveOccurred(), "v1beta1 API should not be available")
			o.Expect(apierrors.IsNotFound(err)).To(o.BeTrue(), "v1beta1 should return NotFound error")
			framework.Logf("Confirmed v1beta1 API is disabled: %v", err)

			g.By("verifying v1alpha1 API is not available")
			_, err = discoveryClient.ServerResourcesForGroupVersion("resource.k8s.io/v1alpha1")
			o.Expect(err).To(o.HaveOccurred(), "v1alpha1 API should not be available")
			o.Expect(apierrors.IsNotFound(err)).To(o.BeTrue(), "v1alpha1 should return NotFound error")
			framework.Logf("Confirmed v1alpha1 API is disabled: %v", err)
