...
func (s *StargzStoreSetup) Cleanup(ctx context.Context) error {
	...
	// Remove SA from privileged SCC before deleting namespace
	// to prevent dangling references when namespace is re-created
	framework.Logf("Removing SA from privileged SCC...")
	...
	if err == nil {
		...
		filtered := make([]string, 0, len(scc.Users))
		for _, u := range scc.Users {
			if u != saUser {
				filtered = append(filtered, u)
			}
		}
		if len(filtered) != len(scc.Users) {
			scc.Users = filtered
			...
			if err != nil {
				...
				framework.Logf("Successfully removed SA from privileged SCC")
			}
		}
	} else {
		framework.Logf("Warning: failed to get privileged SCC: %v", err)
	}
...
# NOTE: --hotfix unlocks do not survive reboots. MCP rollouts (which reboot nodes) will revert this.
# If cleanup fails mid-test, the node stays unlocked until the next reboot.
...
# Wait for service to be ready (poll instead of fixed sleep)
echo "Waiting for stargz-store to be ready..."
for i in $(seq 1 30); do
  if nsenter -t 1 -m -u -i -n -p -- systemctl is-active stargz-store >/dev/null 2>&1; then
    echo "DEBUG: Service became active after ${i} seconds"
    break
  fi
  sleep 1
done
...
func (s *StargzStoreSetup) verifyStargzStoreRunning(ctx context.Context) error {
	...
	for _, node := range pureWorkers {
		// Check service is-active first
		output, err := ExecOnNodeWithChroot(s.oc, node.Name, "systemctl", "is-active", "stargz-store")
		isActive := strings.TrimSpace(output) == "active"

		if !isActive {
			// Only log detailed diagnostics on failure
			framework.Logf("=== DEBUG: stargz-store verification FAILED on node %s ===", node.Name)
			framework.Logf("DEBUG: Service is-active output: %s (error: %v)", strings.TrimSpace(output), err)

			binaryCheck, _ := ExecOnNodeWithChroot(s.oc, node.Name, "ls", "-la", "/usr/local/bin/stargz-store")
			framework.Logf("DEBUG: Binary check: %s", binaryCheck)
		...
		if !strings.Contains(mountOutput, "stargz") {
			...
			framework.Logf("DEBUG: Mount output:")
		...
		framework.Logf("✓ stargz-store is active and mounted on node %s", node.Name)
	}
