...
var _ = g.Describe("[Skipped:Disconnected][apigroup:config.openshift.io][apigroup:machineconfiguration.openshift.io][Jira:Node/CRI-O][sig-node][Feature:AdditionalStorageSupport][OCPFeatureGate:AdditionalStorageConfig][Serial][Disruptive][Suite:openshift/disruptive-longrunning] Additional Image Stores E2E", func() {
	...
	// NEW CONSOLIDATED TEST: Replaces TC8+TC9+TC10 with single-node MCP pattern
	// Reduces MCO rollouts from 3×50min = 150min to 1×45min = 45min
	g.It("should perform comprehensive lifecycle: create, prepopulate, fallback, update, cleanup [TC8+TC9+TC10 consolidated]", func(ctx context.Context) {
		mcClient, err := mcclient.NewForConfig(oc.KubeFramework().ClientConfig())
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		mcpName, testNode, _, mcpCleanup, err := createSingleNodeMCP(ctx, oc, "imagestore-test")
		o.Expect(err).NotTo(o.HaveOccurred())
		g.DeferCleanup(mcpCleanup)
		framework.Logf("Custom MCP %s created, targeting node %s", mcpName, testNode)

		// Create directories on the test node
		g.By("Creating image store directories on test node")
		imageDirs := []string{"/var/lib/imagestore-1", "/var/lib/imagestore-2", "/var/lib/imagestore-3"}
		testNodeList := []corev1.Node{{ObjectMeta: metav1.ObjectMeta{Name: testNode}}}
		err = createDirectoriesOnNodes(oc, testNodeList, imageDirs)
		o.Expect(err).NotTo(o.HaveOccurred())
		g.DeferCleanup(cleanupDirectoriesOnNodes, oc, testNodeList, imageDirs)

		// =====================================================================
		// PHASE 1: Create CRC with single image store (TC8 Part 1)
		// =====================================================================
		g.By("PHASE 1: Creating ContainerRuntimeConfig with one additionalImageStore")
		ctrcfg := &machineconfigv1.ContainerRuntimeConfig{
			ObjectMeta: metav1.ObjectMeta{
				Name: "imagestore-lifecycle-test",
			},
			Spec: machineconfigv1.ContainerRuntimeConfigSpec{
				MachineConfigPoolSelector: &metav1.LabelSelector{
					MatchLabels: map[string]string{
						"machineconfiguration.openshift.io/pool": mcpName, // Target our custom MCP
					},
				},
				ContainerRuntimeConfig: &machineconfigv1.ContainerRuntimeConfiguration{
					AdditionalImageStores: []machineconfigv1.AdditionalImageStore{
		...
		g.By("Waiting for MCP to start updating")
		err = waitForMCPToStartUpdating(ctx, mcClient, mcpName, 5*time.Minute)
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Waiting for MCP rollout (single node, ~10 min)")
		err = waitForMCP(ctx, mcClient, mcpName, 15*time.Minute) // Reduced from 25 min
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Verifying storage.conf contains imagestore-1")
		...
		framework.Logf("Phase 1 PASSED: Single image store configured")

		// =====================================================================
		// PHASE 2: Prepopulate image and test fast pull (TC8 Part 2)
		// =====================================================================
		g.By("PHASE 2: Pre-populating image in imagestore-1")
		testImage := testImageDefault
		err = prepopulateImageOnNode(ctx, oc, testNode, testImage, "/var/lib/imagestore-1")
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Deploying pod using prepopulated image")
		testPod := createTestPod("imagestore-prepop-pod", oc.Namespace(), testImage, testNode)
		startTime := time.Now()
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		pod1Time := time.Since(startTime)
		framework.Logf("Pod started in %v (using prepopulated image)", pod1Time)

		// Verify "already present on machine" event
		g.By("Verifying image was loaded from additional storage")
		var foundAlreadyPresent bool
		err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 2*time.Minute, true, func(ctx context.Context) (bool, error) {
			events, _ := oc.AdminKubeClient().CoreV1().Events(oc.Namespace()).List(ctx, metav1.ListOptions{
				FieldSelector: fmt.Sprintf("involvedObject.name=%s", testPod.Name),
			})
			for _, event := range events.Items {
				if event.Reason == "Pulled" && strings.Contains(event.Message, "already present on machine") {
					foundAlreadyPresent = true
					framework.Logf("SUCCESS: Image loaded from additional storage - %s", event.Message)
					return true, nil
				}
			}
			return false, nil
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(foundAlreadyPresent).To(o.BeTrue())
		framework.Logf("Phase 2 PASSED: Prepopulated image used from additional storage")

		// =====================================================================
		// PHASE 3: Test fallback to registry (TC8 Part 3)
		// =====================================================================
		g.By("PHASE 3: Testing fallback to registry")
		err = oc.AdminKubeClient().CoreV1().Pods(oc.Namespace()).Delete(ctx, testPod.Name, metav1.DeleteOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Waiting for first pod deletion")
		err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 2*time.Minute, true, func(ctx context.Context) (bool, error) {
		...
		g.By("Removing image from additional storage")
		removeCmd := fmt.Sprintf("podman --root /var/lib/imagestore-1 rmi %s", testImage)
		_, err = ExecOnNodeWithChroot(oc, testNode, "sh", "-c", removeCmd)
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Creating second pod to test registry fallback")
		testPod2 := createTestPod("imagestore-fallback-pod", oc.Namespace(), testImage, testNode)
		startTime2 := time.Now()
		...
		framework.Logf("Second pod started in %v (pulled from registry)", pod2Time)

		// Verify "Successfully pulled" event
		g.By("Verifying image was pulled from registry")
		...
		if pod1Time < pod2Time {
			speedup := float64(pod2Time) / float64(pod1Time)
			framework.Logf("Performance: Prepopulated %.2fx faster (pod1: %v, pod2: %v)", speedup, pod1Time, pod2Time)
		}
		framework.Logf("Phase 3 PASSED: Fallback to registry works")

		// Cleanup second pod before update
		err = oc.AdminKubeClient().CoreV1().Pods(oc.Namespace()).Delete(ctx, testPod2.Name, metav1.DeleteOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())

		// =====================================================================
		// PHASE 4: Update CRC to add more stores (TC9 + TC10 combined)
		// =====================================================================
		g.By("PHASE 4: Updating CRC to add imagestore-2 and imagestore-3")
		...
		g.By("Waiting for MCP to update after CRC modification")
		err = waitForMCPToStartUpdating(ctx, mcClient, mcpName, 5*time.Minute)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.By("Verifying all three image stores in storage.conf")
		output, err = ExecOnNodeWithChroot(oc, testNode, "cat", "/etc/containers/storage.conf")
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("/var/lib/imagestore-1"))
		o.Expect(output).To(o.ContainSubstring("/var/lib/imagestore-2"))
		o.Expect(output).To(o.ContainSubstring("/var/lib/imagestore-3"))
		framework.Logf("Phase 4 PASSED: Updated configuration with 3 image stores")

		// =====================================================================
		// PHASE 5: Cleanup and verify removal (best-effort, don't fail test)
		// =====================================================================
		g.By("PHASE 5: Deleting CRC and verifying cleanup (best-effort)")
		err = mcClient.MachineconfigurationV1().ContainerRuntimeConfigs().Delete(ctx, ctrcfg.Name, metav1.DeleteOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Waiting for MCP to start updating after deletion (best-effort)")
		err = waitForMCPToStartUpdating(ctx, mcClient, mcpName, 5*time.Minute)
		if err != nil {
			framework.Logf("Warning: MCP did not start updating after CRC deletion: %v", err)
		} else {
			g.By("Waiting for MCP rollout after deletion (best-effort, max 15 min)")
			err = waitForMCP(ctx, mcClient, mcpName, 15*time.Minute)
			if err != nil {
				framework.Logf("Warning: MCP rollout did not complete after deletion (this is acceptable in cleanup): %v", err)
			} else {
				g.By("Verifying image stores removed from storage.conf")
				output, err = ExecOnNodeWithChroot(oc, testNode, "cat", "/etc/containers/storage.conf")
				if err == nil {
					if !strings.Contains(output, "/var/lib/imagestore-1") &&
						!strings.Contains(output, "/var/lib/imagestore-2") &&
						!strings.Contains(output, "/var/lib/imagestore-3") {
						framework.Logf("Phase 5 PASSED: Cleanup verified")
					} else {
						framework.Logf("Warning: Image stores still present in storage.conf after deletion (cleanup incomplete)")
					}
				}
			}
		}
		...
		// =====================================================================
		framework.Logf("========================================")
		framework.Logf("CONSOLIDATED E2E TEST SUMMARY")
		framework.Logf("========================================")
		framework.Logf("Phase 1: Single image store configuration - PASSED")
		framework.Logf("Phase 2: Prepopulated image fast pull - PASSED")
		framework.Logf("Phase 3: Fallback to registry - PASSED")
		framework.Logf("Phase 4: Update to 3 image stores - PASSED")
		framework.Logf("Phase 5: Cleanup - BEST EFFORT (warnings logged if incomplete)")
		framework.Logf("========================================")
		framework.Logf("Single-node MCP reduced rollout time from ~75 min to ~45 min (3 rollouts × 15 min)")
		framework.Logf("Test PASSED: Complete lifecycle validation (TC8+TC9+TC10)")
	})

	/* COMMENTED OUT - Replaced by consolidated test above
	...
	g.It("should perform complete E2E lifecycle test with prepopulated images and fallback validation [TC8]", func(ctx context.Context) {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		g.DeferCleanup(cleanupDirectoriesOnNodes, oc, pureWorkers, imageDirs)
		...
		testPod := createTestPod("imagestore-test-pod", oc.Namespace(), testImageDefault, testNode)
		startTime := time.Now()
		_, err = oc.AdminKubeClient().CoreV1().Pods(oc.Namespace()).Create(ctx, testPod, metav1.CreateOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		g.DeferCleanup(deletePodAndWait, ctx, oc, oc.Namespace(), testPod.Name)
		...
		testPod2 := createTestPod("imagestore-fallback-pod", oc.Namespace(), testImageDefault, testNode)
		startTime2 := time.Now()
		_, err = oc.AdminKubeClient().CoreV1().Pods(oc.Namespace()).Create(ctx, testPod2, metav1.CreateOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		g.DeferCleanup(deletePodAndWait, ctx, oc, oc.Namespace(), testPod2.Name)
