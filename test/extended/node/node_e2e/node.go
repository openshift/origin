...
var _ = g.Describe("[sig-node] Kubelet, CRI-O, CPU manager", func() {
	...
	g.It("validate KUBELET_LOG_LEVEL", func() {
		...
		waitErr := wait.Poll(10*time.Second, 1*time.Minute, func() (bool, error) {
			...
			for _, node := range nodes {
				...
				if nodeStatus == "True" {
					...
					kubeservice, err = oc.AsAdmin().WithoutNamespace().Run("debug").Args("node/"+node, "--", "chroot", "/host", "/bin/bash", "-c", "systemctl show kubelet.service | grep KUBELET_LOG_LEVEL").Output()
					o.Expect(err).NotTo(o.HaveOccurred())

					g.By("Checking kubelet process for --v=2 flag on node " + node)
					kublet, err = oc.AsAdmin().WithoutNamespace().Run("debug").Args("node/"+node, "--", "chroot", "/host", "/bin/bash", "-c", "ps aux | grep kubelet").Output()
					o.Expect(err).NotTo(o.HaveOccurred())
