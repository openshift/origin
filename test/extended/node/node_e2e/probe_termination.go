...
// Tolerance applied around the expected termination time: -minToleranceSec for timing
// precision, +maxToleranceSec for container cleanup/restart overhead.
const (
	minToleranceSec = 3
	maxToleranceSec = 10
)
...
var _ = g.Describe("[sig-node] Probe configuration", func() {
	...
	g.It("[OTP] Liveness probe should respect probe-level terminationGracePeriodSeconds [OCP-44493]", func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred(), "failed to observe probe termination and restart")
		assertTerminationWithinTolerance(timeDiff, expectedSec)
	})

	//author: bgudi@redhat.com
	g.It("[OTP] Startup probe should respect probe-level terminationGracePeriodSeconds [OCP-44493]", func() {
		ctx := context.Background()
	...
	//author: bgudi@redhat.com
	g.It("[OTP] Liveness probe should fall back to pod-level terminationGracePeriodSeconds when probe-level is not set [OCP-44493]", func() {
		ctx := context.Background()
...
// assertTerminationWithinTolerance asserts that timeDiff falls within the tolerance window
// around expectedSec (see minToleranceSec/maxToleranceSec).
func assertTerminationWithinTolerance(timeDiff, expectedSec int) {
	minSec := expectedSec - minToleranceSec
	maxSec := expectedSec + maxToleranceSec
...
// verifyProbeTermination measures the time between the "Killing" event (kill decision, via
// FirstTimestamp) and the container's restart (via pod.Status, not a "Started" event, since
// LastTimestamp on repeated events is unreliable). Restart is gated to RestartCount==1 so the
// two signals always refer to the same cycle. Returns the difference in seconds.
func verifyProbeTermination(ctx context.Context, oc *exutil.CLI, namespace, podName, containerName string, expectedTerminationSec int) (int, error) {
	...
	err := wait.PollUntilContextTimeout(ctx, 5*time.Second, 6*time.Minute, true, func(ctx context.Context) (bool, error) {
		...
		e2e.Logf("Container %q: probe failure detected at %v, restarted at %v, time difference: %d seconds (expected: %d, -%d/+%ds tolerance)",
			containerName, killedAt, startedAt, timeDiff, expectedTerminationSec, minToleranceSec, maxToleranceSec)
