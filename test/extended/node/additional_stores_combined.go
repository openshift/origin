...
var _ = g.Describe("[Skipped:Disconnected][apigroup:config.openshift.io][apigroup:machineconfiguration.openshift.io][Jira:Node/CRI-O][sig-node][Feature:AdditionalStorageSupport][OCPFeatureGate:AdditionalStorageConfig][Serial][Disruptive][Suite:openshift/disruptive-longrunning] Combined Additional Stores E2E", func() {
	...
	g.It("should functionally verify all three storage types work together [TC9]", func(ctx context.Context) {
		...
		// Use skopeo to copy image to additional image store
		skopeoCmd := fmt.Sprintf("skopeo copy docker://%s dir:%s/prepopulated-image", testImage, imageStorePath)
		_, err = ExecOnNodeWithChroot(oc, testNode.Name, "bash", "-c", skopeoCmd)
		o.Expect(err).NotTo(o.HaveOccurred())

		// Verify image was copied
		lsOutput, err := ExecOnNodeWithChroot(oc, testNode.Name, "ls", "-la", imageStorePath+"/prepopulated-image")
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		snapshotsAfterFirst := getStargzSnapshotCount(oc, testNode.Name)
		...
		err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 30*time.Second, true, func(ctx context.Context) (bool, error) {
			currentSnapshots := getStargzSnapshotCount(oc, testNode.Name)
		...
		snapshotsAfterSecond := getStargzSnapshotCount(oc, testNode.Name)
