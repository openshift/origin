...
var _ = g.Describe("[sig-node][Feature:NVIDIA-DRA][Suite:openshift/nvidia-dra][Serial]", func() {
	...
	g.BeforeEach(func(ctx context.Context) {
		...
		ensurePrerequisites(ctx, NewPrerequisitesInstaller(oc.KubeFramework()))
	...
	g.Context("ResourceClaim Lifecycle on Real Hardware", func() {
		g.It("should transition claim through Pending -> Allocated -> Reserved -> Released", func(ctx context.Context) {
			...
			g.By("Verifying claim is released (deallocated and no longer reserved)")
			// With structured parameters the resourceclaim controller clears status.allocation
			// once the last consumer is dropped from reservedFor, so the claim returns to the
			// pending state rather than staying allocated.
			err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 30*time.Second, true, func(ctx context.Context) (bool, error) {
				status, err = validator.ValidateClaimStatus(ctx, oc.Namespace(), claimName)
				if err != nil {
					return false, err
				}
				return len(status.ReservedFor) == 0 && !status.Allocated, nil
			})
			...
			o.Expect(status.Allocated).To(o.BeFalse(), "Claim allocation should be cleared once the last consumer is removed")
			framework.Logf("Claim %s is released (deallocated, no reservations)", claimName)
		})
		...
		g.It("should allocate GPU using CEL selector on device attributes", func(ctx context.Context) {
			deviceClassName := "test-cel-selector-" + oc.Namespace()
			...
			claim := builder.BuildResourceClaimWithCELSelector(claimName, deviceClassName, `device.driver == "gpu.nvidia.com"`, 1)
			err = createResourceClaim(ctx, oc.KubeFramework().DynamicClient, oc.Namespace(), claim)
