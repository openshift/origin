	...
	g.Describe("when installed on the cluster", func() {
		...
		g.It("should have important platform topology metrics", func() {
			...
			tests := map[string][]metricTest{
				...
				`count(cluster_infrastructure_provider{type!=""}) == 1`: {{success: true}},
				`count(cluster_feature_set) ==  1`:                      {{success: true}},

				// track installer type
				`count(cluster_installer{type!="",invoker!=""}) == 1`: {{success: true}},

				// track sum of etcd
				`count(instance:etcd_object_counts:sum) == 1`: {{success: true}},

				// track cores and sockets across node types
				`sum(node_role_os_version_machine:cpu_capacity_cores:sum{label_kubernetes_io_arch!="",label_node_role_kubernetes_io_master!=""}) > 0`:                                      {{success: true}},
				`sum(node_role_os_version_machine:cpu_capacity_sockets:sum{label_kubernetes_io_arch!="",label_node_hyperthread_enabled!="",label_node_role_kubernetes_io_master!=""}) > 0`: {{success: true}},
			}
