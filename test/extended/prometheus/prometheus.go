...
var _ = g.Describe("[sig-instrumentation] Prometheus [apigroup:image.openshift.io]", func() {
	...
	g.Describe("when installed on the cluster", func() {
		...
		g.It("shouldn't report any alerts in firing state apart from Watchdog and AlertmanagerReceiversNotConfigured [Early][apigroup:config.openshift.io]", func() {
			...
			// Exclude this alert for single node openshift; if we have an error getting the topology, then don't exclude.
			jobType, err := platformidentification.GetJobType(ctx, oc.AdminConfig())
			if err != nil && jobType.Topology == "single" {
				allowedAlertNames = append(allowedAlertNames, "KubeMemoryOvercommit")
			}
