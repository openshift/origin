...
import (
	...
	"github.com/openshift/origin/test/extended/apiserver/jsonpatch"
	"github.com/openshift/origin/test/extended/testdata"
	exutil "github.com/openshift/origin/test/extended/util"
)

var _ = g.Describe("[sig-api-machinery] JSON Patch", func() {
	var (
	...
	g.BeforeEach(func() {
		...
		err = resourceClient.Delete(ctx, "test-apiserver", metav1.DeleteOptions{})
	...
	g.It("should delete an entry from an array with a test precondition provided", func() {
		...
		jsonPatch := jsonpatch.New().WithTest("/status/nodeStatuses/1/nodeName", "master-2").WithRemove("/status/nodeStatuses/1")
		kasOperator = applyJSONPatch(ctx, kasOperator.Name, jsonPatch, resourceClient)
	...
	g.It("should delete an entry from an array with multiple field owners", func() {
		...
		jsonPatch := jsonpatch.New().WithTest("/status/nodeStatuses/0/nodeName", "master-1").WithRemove("/status/nodeStatuses/0")
		kasOperator = applyJSONPatch(ctx, kasOperator.Name, jsonPatch, resourceClient)
		o.Expect(kasOperator.Status.NodeStatuses).To(o.Equal([]operatorv1.NodeStatus{
			{NodeName: "master-2"},
		}))
...
func applyJSONPatch(ctx context.Context, name string, jsonPatch *jsonpatch.PatchSet, resourceClient dynamic.ResourceInterface) *operatorv1.KubeAPIServer {
	...
	o.Expect(err).NotTo(o.HaveOccurred())
	return unstructuredToKubeAPIServerOperator(unstructuredKasOperator.Object)
}
