...
var _ = g.Describe("[sig-api-machinery][Feature:APIServer][Feature:PullSecret]", func() {
	...
	g.It("User can pull a private image from a registry when a pull secret is defined [Serial][apigroup:build.openshift.io][apigroup:apps.openshift.io]", func() {
	...
	g.It("Use well-formed pull secret with incorrect credentials will fail to build and deploy [Serial][apigroup:build.openshift.io][apigroup:apps.openshift.io]", func() {
		skipIfBaselineCapsMissingCapabilities(oc, "Build", "DeploymentConfig")
		skipIfProxyCluster(oc)
		architecture.SkipArchitectures(oc, architecture.MULTI)
	...
	g.It("Deploy will fail with incorrectly formed pull secrets [apigroup:build.openshift.io][apigroup:apps.openshift.io][apigroup:image.openshift.io]", func() {
	...
	g.It("Use bound service account tokens when generating pull secrets [apigroup:image.openshift.io]", func() {
