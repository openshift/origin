...
var _ = g.Describe("[sig-api-machinery][Feature:APIServer][Late]", func() {
	...
	oc := exutil.NewCLI(exutil.CliOptions{BaseName: "terminating-kube-apiserver", WithoutNamespace: true})
	...
	g.It("API LBs follow /readyz of kube-apiserver and stop sending requests before server shutdowns for external clients", func() {
		...
		if *controlPlaneTopology == configv1.ExternalTopologyMode {
			mgmtClusterOC := exutil.NewCLI(exutil.CliOptions{BaseName: "default", WithoutNamespace: true}).AsAdmin()
