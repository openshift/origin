...
EOF`, serverconf)
	...
	g.It("When using webhooks fails to rollout latest deploymentconfig [Disruptive][apigroup:apps.openshift.io]", func() {
...
EOF`, dcpolicyrepo)
	...
	g.It("HPA/oc scale and DeploymentConfig should be working with OPA webhooks [Disruptive][apigroup:apps.openshift.io]", func() {
		randomStr := compat_otp.GetRandomString()
	...
	g.It("Kubernetes validating admission webhook bypass [Serial][apigroup:admissionregistration.k8s.io]", func() {
...
var _ = g.Describe("[sig-api-machinery][Feature:APIServer][Feature:UpgradeWebhooks]", func() {
	...
	g.It("Prepare upgrade checks when cluster has bad admission webhooks [Serial][apigroup:config.openshift.io][apigroup:admissionregistration.k8s.io][apigroup:apiextensions.k8s.io]", func() {
		var (
			namespace                  = "ocp-50362"
		...
		g.By("Capture current context and verify kube-apiserver operator status")
		origContext, contextErr := oc.Run("config").Args("current-context").Output()
		o.Expect(contextErr).NotTo(o.HaveOccurred())
		defer func() {
			o.Expect(oc.Run("config").Args("use-context", origContext).Execute()).To(o.Succeed())
		}()
		...
		if !reflect.DeepEqual(kasStatusBefore, kubeApiserverCoStatus) && !reflect.DeepEqual(kasStatusBefore, kasRolloutStatus) {
			g.Skip("kube-apiserver operator is not in a stable status before upgrade preparation")
		}

		g.By("Create namespace and bad admission webhook configurations")
		...
		g.By("Verify kube-apiserver reports webhook configuration errors")
	...
	g.It("Post upgrade checks when cluster has bad admission webhooks [Serial][apigroup:config.openshift.io][apigroup:admissionregistration.k8s.io][apigroup:apiextensions.k8s.io]", func() {
		var (
			namespace                  = "ocp-50362"
			badValidatingWebhookName   = "test-validating-cfg"
			badMutatingWebhookName     = "test-mutating-cfg"
		...
		kasStatusBefore := getCoStatus(oc, "kube-apiserver", kubeApiserverCoStatus)
		if !reflect.DeepEqual(kasStatusBefore, kubeApiserverCoStatus) && !reflect.DeepEqual(kasStatusBefore, kasRolloutStatus) {
			g.Skip("kube-apiserver operator is not in a stable status before post-upgrade verification")
		}

		g.By("Verify pre-upgrade webhook resources still exist")
		for _, check := range []struct {
			kind  string
			names []string
		}{
			{"ValidatingWebhookConfiguration", []string{badValidatingWebhookName}},
			{"MutatingWebhookConfiguration", []string{badMutatingWebhookName}},
			{"crd", []string{badCrdWebhookName}},
		} {
			output, available := checkIfResourceAvailable(oc, check.kind, check.names, "")
			if !available && strings.Contains(output, "not found") {
				g.Skip(fmt.Sprintf("resource %s not found in post-upgrade check; pre-upgrade step likely did not run", check.kind))
			}
		}
		...
		currentKAStatus := getCoStatus(oc, "kube-apiserver", kubeApiserverCoStatus)
		if !(reflect.DeepEqual(currentKAStatus, kasStatusBefore) || reflect.DeepEqual(currentKAStatus, kubeApiserverCoStatus)) {
			e2e.Failf("kube-apiserver operator status changed after upgrade with bad webhooks present")
		}

		g.By("Delete bad webhooks and verify errors clear")
		_ = oc.Run("delete").Args("ValidatingWebhookConfiguration", badValidatingWebhookName, "--ignore-not-found").Execute()
		_ = oc.Run("delete").Args("MutatingWebhookConfiguration", badMutatingWebhookName, "--ignore-not-found").Execute()
		_ = oc.Run("delete").Args("crd", badCrdWebhookName, "--ignore-not-found").Execute()
	...
	g.It("Checks on different bad admission webhook errors and kube-apiserver status [Serial][apigroup:admissionregistration.k8s.io][apigroup:apiextensions.k8s.io]", func() {
		var (
	...
	g.It("Prepare upgrade cluster under APF stress [Slow][Disruptive][apigroup:config.openshift.io]", func() {
		...
		g.By("Verify clusterbuster pods complete and cluster remains healthy")
		errPod := wait.PollUntilContextTimeout(context.Background(), 15*time.Second, 900*time.Second, false, func(ctx context.Context) (bool, error) {
			_, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("pods", "-A").OutputToFile(dirname + "pod.log")
			o.Expect(err).NotTo(o.HaveOccurred())
			cmd := fmt.Sprintf(`cat %s | grep -iE 'cpuload|memload' | grep -ivE 'Running|Completed|namespace|pending' || true`, dirname+"pod.log")
			podLogs, err := exec.Command("bash", "-c", cmd).Output()
			o.Expect(err).NotTo(o.HaveOccurred())
			return len(podLogs) == 0, nil
		})
		compat_otp.AssertWaitPollNoErr(errPod, "clusterbuster pods did not complete successfully")
		...
		for _, kasPod := range podList {
			_, _ = oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", "openshift-kube-apiserver", string(kasPod)).OutputToFile(dirname + "kas.log." + string(kasPod))
		}
		...
		if cpuAvgVal > 70 || memAvgVal > 75 || len(noRouteLogs) > 0 || len(panicLogs) > 0 {
			e2e.Failf("pre-upgrade APF stress preparation failed")
		}
	})

	g.It("Post upgrade cluster under APF stress [Slow][apigroup:config.openshift.io]", func() {
		dirname := "/tmp/-OCP-40667/"
		exceptions := "panicked: false"
		defer os.RemoveAll(dirname)
		defer func() {
			for _, cmd := range []string{
				`clusterbuster --cleanup -B cpuload -w cpusoaker`,
				`clusterbuster --cleanup -B memload -w memory`,
			} {
		...
		o.Expect(clusterNodesHealthcheck(oc, 500, dirname+"log")).To(o.Succeed())
		o.Expect(clusterOperatorHealthcheck(oc, 500, dirname+"log")).To(o.Succeed())

		kasPods, err := compat_otp.GetAllPodsWithLabel(oc, "openshift-kube-apiserver", "apiserver")
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		if cpuAvgVal > 75 || memAvgVal > 85 || len(noRouteLogs) > 0 || len(panicLogs) > 0 {
			e2e.Failf("post-upgrade APF stress verification failed")
		}
	})

	g.It("ClusterResourceQuota objects validation [apigroup:quota.openshift.io][apigroup:monitoring.coreos.com]", func() {
