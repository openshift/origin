...
os::log::info "Pre-pulling and pushing ruby-27-centos7"
os::cmd::expect_success 'docker pull centos/ruby-27-centos7:latest'
os::log::info "Pulled ruby-27-centos7"
...
os::log::info "Tagging and pushing ruby-27-centos7 to ${DOCKER_REGISTRY}/cache/ruby-27-centos7:latest"
os::cmd::expect_success "docker tag centos/ruby-27-centos7:latest ${DOCKER_REGISTRY}/cache/ruby-27-centos7:latest"
os::cmd::expect_success "docker push ${DOCKER_REGISTRY}/cache/ruby-27-centos7:latest"
os::log::info "Pushed ruby-27-centos7"

# get image's digest
rubyimagedigest="$(oc get -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].image}' is/ruby-27-centos7)"
os::log::info "Ruby image digest: ${rubyimagedigest}"
# get a random, non-empty blob
rubyimageblob="$(oc get isimage -o go-template='{{range .image.dockerImageLayers}}{{if gt .size 1024.}}{{.name}},{{end}}{{end}}' "ruby-27-centos7@${rubyimagedigest}" | cut -d , -f 1)"
...
os::cmd::expect_success "curl \
    ...
    'http://${DOCKER_REGISTRY}/v2/cache/ruby-27-centos7/manifests/latest'"
os::cmd::expect_success "curl --head                                                \
    ...
    'http://${DOCKER_REGISTRY}/v2/verify-manifest/ruby-27-centos7/manifests/latest' \
...
os::cmd::expect_success "oc project ${CLUSTER_ADMIN_CONTEXT}"
os::cmd::expect_success_and_text 'oc whoami' 'system:admin'
os::cmd::expect_success "oc tag --source docker centos/ruby-27-centos7:latest -n custom ruby-27-centos7:latest"
...
os::cmd::expect_success_and_text "curl -I -X HEAD -u 'pusher:${pusher_token}' '${DOCKER_REGISTRY}/v2/cache/ruby-27-centos7/blobs/$rubyimageblob'" "200 OK"
os::cmd::try_until_text "oc get -n custom is/ruby-27-centos7 -o 'jsonpath={.status.tags[*].tag}'" "latest" $((20*TIME_SEC))
...
os::cmd::expect_success_and_text "curl -I -X POST -u 'pusher:${pusher_token}' '${DOCKER_REGISTRY}/v2/crossmount/repo/blobs/uploads/?mount=$rubyimageblob&from=cache/ruby-27-centos7'" "201 Created"
...
os::cmd::expect_success_and_text "curl -I -X POST -u 'pusher:${pusher_token}' '${DOCKER_REGISTRY}/v2/crossmount/repo/blobs/uploads/?mount=$rubyimageblob&from=cache/ruby-27-centos7'" "202 Accepted"
...
os::cmd::expect_success "oc process -n node-selector -p NODE_NAME='$(oc get node -o jsonpath='{.items[0].metadata.name}')' -f test/testdata/node-selector/pods.json | oc create -n node-selector -f -"
