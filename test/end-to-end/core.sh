...
oc run cli-with-token --attach --image="${IMAGE_PREFIX}:${TAG}" --restart=Never --command -- oc status --loglevel=4 > "${LOG_DIR}/cli-with-token.log" 2>&1
...
oc run cli-with-token-2 --attach --image="${IMAGE_PREFIX}:${TAG}" --restart=Never --command -- oc whoami --loglevel=4 > "${LOG_DIR}/cli-with-token2.log" 2>&1
