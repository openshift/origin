...
func TestModifyHostConfig(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:     "full pod.Spec.SecurityContext",
			podSc:    fullValidPodSecurityContext(),
			expected: fullValidHostConfig(),
		},
		{
		...
		{
			name: "pod.Spec.SecurityContext.Privileged",
			podSc: &api.PodSecurityContext{
				Privileged: &priv,
			},
			expected: setPrivHC,
		...
		{
			name: "pod.Spec.SecurityContext.Capabilities",
			podSc: &api.PodSecurityContext{
				Capabilities: inputCapabilities(),
			},
			expected: setCapsHC,
...
func overridePodSecurityContext() *api.PodSecurityContext {
	...
	return &api.PodSecurityContext{
		...
		Capabilities: &api.Capabilities{
			Add:  []api.Capability{"addCapC", "addCapD"},
			Drop: []api.Capability{"dropCapC", "dropCapD"},
		},
