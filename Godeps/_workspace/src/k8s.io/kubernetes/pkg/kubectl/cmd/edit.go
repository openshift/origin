...
func (r *editResults) addError(err error, info *resource.Info) string {
	...
	case errors.IsInvalid(err):
		...
		return fmt.Sprintf("Error: the %s %q is invalid", info.Mapping.Resource, info.Name)
	case errors.IsNotFound(err):
		r.notfound++
		return fmt.Sprintf("Error: the %s %q could not be found on the server", info.Mapping.Resource, info.Name)
	default:
		r.retryable++
		return fmt.Sprintf("Error: the %s %q could not be patched: %v", info.Mapping.Resource, info.Name, err)
	}
}
