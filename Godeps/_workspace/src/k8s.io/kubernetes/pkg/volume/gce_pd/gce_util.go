...
func (util *GCEDiskUtil) DeleteVolume(d *gcePersistentDiskDeleter) error {
	...
	if err := cloud.(*gce_cloud.GCECloud).DeleteDisk(d.pdName); err != nil {
...
func (gceutil *GCEDiskUtil) CreateVolume(c *gcePersistentDiskCreater) (string, int, error) {
	...
	err := cloud.(*gce_cloud.GCECloud).CreateDisk(name, int64(volSize))
	if err != nil {
