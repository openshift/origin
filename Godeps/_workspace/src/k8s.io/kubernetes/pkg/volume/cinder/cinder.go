...
func (plugin *cinderPlugin) NewCreater(options volume.VolumeOptions) (volume.Creater, error) {
	if len(options.AccessModes) == 0 {
		options.AccessModes = plugin.GetAccessModes()
	}
	return plugin.newCreaterInternal(options, &CinderDiskUtil{})
}

func (plugin *cinderPlugin) newCreaterInternal(options volume.VolumeOptions, manager cdManager) (volume.Creater, error) {
	return &cinderVolumeCreater{
		cinderVolume: &cinderVolume{
			manager: manager,
			plugin:  plugin,
		},
...
type cdManager interface {
	...
	// Creates a volume
	CreateVolume(creater *cinderVolumeCreater) (string, int, error)
	// Deletes a volume
...
type cinderVolumeCreater struct {
	*cinderVolume
	options volume.VolumeOptions
}

var _ volume.Creater = &cinderVolumeCreater{}

func (c *cinderVolumeCreater) Provision(pv *api.PersistentVolume) error {
...
func (c *cinderVolumeCreater) NewPersistentVolumeTemplate() (*api.PersistentVolume, error) {
	// Provide dummy api.PersistentVolume.Spec, it will be filled in
	// cinderVolumeCreater.Provision()
	return &api.PersistentVolume{
		ObjectMeta: api.ObjectMeta{
			GenerateName: "pv-cinder-",
			Labels: map[string]string{
				"createdby": "cinder_dynamic_provisioner",
			},
