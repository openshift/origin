...
Related discussion:
- https://k8s.io/kubernetes/issues/442
- https://k8s.io/kubernetes/issues/443
...
- any features that help users avoid use of privileged containers (https://k8s.io/kubernetes/issues/391)
...
The API should have a `quota` concept (see https://k8s.io/kubernetes/issues/442).  A quota object relates a namespace (and optionally a label selector) to a maximum quantity of resources that may be used (see [resources design doc](resources.md)).
