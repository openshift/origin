...
2.  Configure that node's `kube-proxy` to decorate HTTP requests from that pod to the
    ...
    (see the [LOAS Daemon](https://k8s.io/kubernetes/issues/2209) proposal)
...
in [#1553](https://k8s.io/kubernetes/issues/1553).  When such a store
...
[#4441](https://k8s.io/kubernetes/issues/4441)), the capacity calculation
