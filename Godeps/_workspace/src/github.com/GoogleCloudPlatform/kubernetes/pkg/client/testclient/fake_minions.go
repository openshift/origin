...
func (c *FakeNodes) Get(name string) (*api.Node, error) {
	obj, err := c.Fake.Invokes(FakeAction{Action: "get-minion", Value: name}, &api.Node{})
	return obj.(*api.Node), err
}

func (c *FakeNodes) List() (*api.NodeList, error) {
	obj, err := c.Fake.Invokes(FakeAction{Action: "list-minions"}, &api.NodeList{})
	return obj.(*api.NodeList), err
}

func (c *FakeNodes) Create(minion *api.Node) (*api.Node, error) {
	obj, err := c.Fake.Invokes(FakeAction{Action: "create-minion", Value: minion}, &api.Node{})
	return obj.(*api.Node), err
}

func (c *FakeNodes) Delete(name string) error {
	_, err := c.Fake.Invokes(FakeAction{Action: "delete-minion", Value: name}, &api.Node{})
	return err
}

func (c *FakeNodes) Update(minion *api.Node) (*api.Node, error) {
	obj, err := c.Fake.Invokes(FakeAction{Action: "update-minion", Value: minion}, &api.Node{})
	return obj.(*api.Node), err
}
