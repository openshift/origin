...
func printPod(pod *api.Pod, w io.Writer, withNamespace bool) error {
	...
	for i := len(pod.Status.ContainerStatuses) - 1; i >= 0; i-- {
		...
		} else if container.State.Termination != nil && container.State.Termination.Reason != "" {
			reason = container.State.Termination.Reason
		} else if container.State.Termination != nil && container.State.Termination.Reason == "" {
			if container.State.Termination.Signal != 0 {
				reason = fmt.Sprintf("Signal:%d", container.State.Termination.Signal)
			} else {
				reason = fmt.Sprintf("ExitCode:%d", container.State.Termination.ExitCode)
			}
