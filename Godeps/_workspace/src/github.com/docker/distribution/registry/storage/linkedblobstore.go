...
// Enumerate returns an array of digests pointing to blobs.
func (lbs *linkedBlobStore) Enumerate(ctx context.Context) ([]digest.Digest, error) {
	res := []digest.Digest{}
	...
	for _, fn := range lbs.blobsRootPathFns {
		...
		dgsts, err := enumerateBlobDigests(ctx, lbs.driver, blobsRootPath, false)
		if err != nil {
			return nil, err
		}
