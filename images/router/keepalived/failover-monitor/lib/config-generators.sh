...
function generate_global_config() {
  ...
  echo "   }"
  echo ""
  echo "   notification_email_from ${EMAIL_FROM:-"router@openshift.local"}"
...
#
#  Examples:
#      generate_vrrp_sync_groups "router-1" "10.1.1.1 10.1.2.2"
#
...
function generate_vrrp_sync_groups() {
  ...
  echo ""
  echo "vrrp_sync_group group_${routername} {"
  echo "   group {"
...
#
#      generate_vrrpd_instance_config router-1 4 "10.1.2.3-4" enp0s8 "7"
#
function generate_vrrpd_instance_config() {
  ...
  local instance_name=$(generate_vrrp_instance_name "$routername" "$iid")

  local auth_section=$(generate_authentication_info "$routername")
...
function generate_failover_config() {
  local vips=$(expand_ip_ranges "$ROUTER_VIPS")
...
$(generate_global_config "$ROUTER_NAME")
$(generate_script_config "$ipaddr")
$(generate_vrrp_sync_groups "$ROUTER_NAME" "$vips")
"

  local ipkey=$(echo "$ipaddr" | cut -f 4 -d '.')
  local ipslot=$((ipkey % 128))
  local idx=$((ipslot % $NUM_REPLICAS))
  idx=$((idx + 1))
  ...
  for vip in ${vips}; do
    ...
    generate_vrrpd_instance_config "$ROUTER_NAME" "$counter" "$vip"  \
