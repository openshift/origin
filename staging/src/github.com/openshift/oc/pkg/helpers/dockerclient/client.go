...
// getRepositoryV1 returns a repository implementation for a imagev1 registry by asking for
...
func (c *connection) getRepositoryV1(name string) (repository, error) {
	...
	base := c.url
	base.Path = path.Join(base.Path, fmt.Sprintf("/imagev1/repositories/%s/images", name))
	req, err := http.NewRequest("GET", base.String(), nil)
	if err != nil {
...
func (repo *v2repository) unmarshalImageManifest(c *connection, body []byte) (*DockerImage, error) {
...
func (repo *v1repository) getTags(c *connection) (map[string]string, error) {
	endpoint := repo.endpoint
	endpoint.Path = path.Join(endpoint.Path, fmt.Sprintf("/imagev1/repositories/%s/tags", repo.name))
	req, err := http.NewRequest("GET", endpoint.String(), nil)
	if err != nil {
		return nil, fmt.Errorf("error creating request: %v", err)
	}
...
func (repo *v1repository) getTaggedImage(c *connection, tag, userTag string) (*Image, []byte, error) {
	endpoint := repo.endpoint
	endpoint.Path = path.Join(endpoint.Path, fmt.Sprintf("/imagev1/repositories/%s/tags/%s", repo.name, tag))
	req, err := http.NewRequest("GET", endpoint.String(), nil)
	if err != nil {
		return nil, nil, fmt.Errorf("error creating request: %v", err)
	}
	...
	case code == http.StatusNotFound:
		...
		body, _ := ioutil.ReadAll(resp.Body)
		klog.V(4).Infof("unable to find imagev1 tagged image at %q, %d %v: %s", req.URL.String(), resp.StatusCode, resp.Header, body)
		return nil, nil, errTagNotFound{len(userTag) == 0, tag, repo.name}
	case code >= 300 || resp.StatusCode < 200:
		// token might have expired - evict repo from cache so we can get a new one on retry
		delete(c.cached, repo.name)
...
func (repo *v1repository) getImage(c *connection, image, userTag string) (*Image, []byte, error) {
	endpoint := repo.endpoint
	endpoint.Path = path.Join(endpoint.Path, fmt.Sprintf("/imagev1/images/%s/json", image))
	req, err := http.NewRequest("GET", endpoint.String(), nil)
	if err != nil {
		return nil, nil, fmt.Errorf("error creating request: %v", err)
	}
...
func unmarshalDockerImage(body []byte) (*DockerImage, error) {
	var (
		imagePre012     docker.ImagePre012
		config          DockerConfig
	...
	if imagePre012.Config != nil {
		convertLegacyConfigToDockerConfig(imagePre012.Config, &config)
	}

	convertLegacyConfigToDockerConfig(&imagePre012.ContainerConfig, &containerConfig)
