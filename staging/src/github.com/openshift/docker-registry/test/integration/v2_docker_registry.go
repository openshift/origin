...
import (
	...
	registryutil "github.com/openshift/docker-registry/pkg/dockerregistry/testutil"
...
func TestV2RegistryGetTags(t *testing.T, clusterAdminKubeConfig, namespace, user, token string) {
	_, clusterAdminClientConfig, err := configapi.GetInternalKubeClient(clusterAdminKubeConfig, nil)
	if err != nil {
		t.Fatalf("error getting cluster admin client config: %v", err)
	}

	userClientConfig := clientcmd.AnonymousClientConfig(clusterAdminClientConfig)
	userClientConfig.BearerToken = token
	adminClient, err := client.New(&userClientConfig)
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	if _, err := adminClient.ImageStreams(namespace).Create(&stream); err != nil {
		t.Fatalf("error creating image stream: %s", err)
	}

	tags, err := getTags(namespace, stream.Name, user, token)
	if err != nil {
		t.Fatal(err)
	}
	if len(tags) > 0 {
	...
	err = putEmptyBlob(namespace, stream.Name, user, token)
	if err != nil {
		t.Fatal(err)
	}

	dgst, err := putManifest(namespace, stream.Name, user, token)
	if err != nil {
		t.Fatal(err)
	}
	...
	// test get by tag
	url := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/manifests/%s", namespace, stream.Name, imageapi.DefaultImageTag)
	...
	// test get by digest
	url = fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/manifests/%s", namespace, stream.Name, dgst.String())
	req, err = http.NewRequest("GET", url, nil)
	if err != nil {
		t.Fatalf("error creating request: %v", err)
	}
	...
	image, err := adminClient.ImageStreamImages(namespace).Get(stream.Name, dgst.String())
	if err != nil {
	...
	if e, a := fmt.Sprintf("127.0.0.1:5000/%s/%s@%s", namespace, stream.Name, dgst.String()), image.Image.DockerImageReference; e != a {
	...
	otherStream, err := adminClient.ImageStreams(namespace).Get("otherrepo", metav1.GetOptions{})
	t.Logf("otherStream=%#v, err=%v", otherStream, err)
	if err == nil {
		t.Fatalf("expected error getting otherrepo")
	}

	err = putEmptyBlob(namespace, "otherrepo", user, token)
	if err != nil {
		t.Fatal(err)
	}

	otherDigest, err := putManifest(namespace, "otherrepo", user, token)
	if err != nil {
		t.Fatal(err)
	}
...
func putManifest(namespace, name, user, token string) (digest.Digest, error) {
	creds := registryutil.NewBasicCredentialStore(user, token)
	desc, _, err := registryutil.UploadRandomTestBlob(&url.URL{Host: "127.0.0.1:5000", Scheme: "http"}, creds, namespace+"/"+name)
	if err != nil {
		return "", err
	}

	putUrl := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/manifests/%s", namespace, name, imageapi.DefaultImageTag)
...
func putEmptyBlob(namespace, name, user, token string) error {
	putUrl := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/blobs/uploads/", namespace, name)
	method := "POST"
...
func getTags(namespace, streamName, user, token string) ([]string, error) {
	url := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/tags/list", namespace, streamName)
	client := http.DefaultClient
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
